"""
Updated S3 backend implementation for the Unified Storage Manager.
...
class S3Backend(BackendStorage):
    ...
    def add_content(self, content: Union[str, bytes, BinaryIO], metadata: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
        ...
        object_key = self._generate_object_id()

        # Set up metadata
        s3_metadata = {"mcp_added": str(int(time.time())), "mcp_backend": self.get_name()}

        # Add custom metadata from options
        if metadata:
            for k, v in metadata.items():
                if isinstance(v, (str, int, float, bool)):
                    s3_metadata[f"mcp_{k}"] = str(v)
        ...
        try:
            ...
            if data_size > self.chunk_size:
                ...
                result = {
                    ...
                    "backend": self.get_name(),
                    "container": bucket,
                    "details": {"bucket": bucket, "key": object_key, "metadata": s3_metadata},
                }

            return result

        except self.ClientError as e:
            logger.error(f"S3 add_content error: {str(e)}")
            return {
                "success": False,
                "error": str(e),
                "backend": self.get_name(),
            ...
            logger.error(f"Unexpected error in S3 add_content: {str(e)}")
            return {"success": False, "error": str(e), "backend": self.get_name()}
    ...
    def get_content(self, content_id: str) -> Dict[str, Any]:
        ...
        in_cache, cache_path = self._is_in_cache(bucket, content_id)
        if in_cache:
            try:
        ...
        try:
            # Get S3 object
            with self.connection_pool as client:
                response = client.get_object(Bucket=bucket, Key=content_id)
                ...
                if len(data) < self.chunk_size:
                    self._add_to_cache(bucket, content_id, data, metadata)
            ...
            if error_code == "NoSuchKey":
                logger.warning(f"Object {content_id} not found in bucket {bucket}")
                return {
                    "success": False,
                    "error": f"Object {content_id} not found",
                    "backend": self.get_name(),
                    "details": {"code": "NoSuchKey"},
                }

            logger.error(f"S3 get_content error: {str(e)}")
            return {
                "success": False,
                "error": str(e),
                "backend": self.get_name(),
            ...
            logger.error(f"Unexpected error in S3 get_content: {str(e)}")
            return {"success": False, "error": str(e), "backend": self.get_name()}
    ...
    def remove_content(self, content_id: str) -> Dict[str, Any]:
        ...
        try:
            # Delete object
            with self.connection_pool as client:
                client.delete_object(Bucket=bucket, Key=content_id)

            # Remove from cache if present
            cache_key = f"{bucket}:{content_id}"
            if cache_key in self._metadata_cache:
                del self._metadata_cache[cache_key]

            # Remove from local cache if present
            cache_path = self._cache_path(bucket, content_id)
            meta_path = f"{cache_path}.meta"
            ...
            logger.error(f"S3 remove_content error: {str(e)}")
            return {
                "success": False,
                "error": str(e),
                "backend": self.get_name(),
            ...
            logger.error(f"Unexpected error in S3 remove_content: {str(e)}")
            return {"success": False, "error": str(e), "backend": self.get_name()}
    ...
    def get_metadata(self, content_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            with self.connection_pool as client:
                response = client.head_object(Bucket=bucket, Key=content_id)
...
def store(self, data, container=None, path=None, options=None):
    """Legacy method for backward compatibility"""
    logger.warning("The 'store' method is deprecated. Use 'add_content' instead.")
...
S3Backend.retrieve = retrieve
S3Backend.delete = delete
