...
class ContentSearchService:
    ...
    async def _create_embedding(
        ...
        try:
            ...
            async with self.db_lock: # Lock for DB and index modification
    ...
    async def _text_search(
        ...
        sql = f'''SELECT m.cid, m.name, m.description, m.tags, m.content_type, m.size, m.created, m.author, m.license, m.extra, m.indexed_at, fts.rank
                  FROM content_fts fts JOIN content_metadata m ON fts.cid = m.cid
                  WHERE content_fts MATCH ? ORDER BY fts.rank LIMIT ?'''
    ...
    async def get_stats(self) -> Dict[str, Any]:
        ...
        try:
            ...
            db_size = await anyio.to_thread.run_sync(self._get_file_size, self.db_path) or 0
            index_file = os.path.join(self.vector_index_path, "index.faiss")
            vector_index_size = await anyio.to_thread.run_sync(self._get_file_size, index_file) or 0
