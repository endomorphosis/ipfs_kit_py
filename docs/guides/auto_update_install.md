...
- `scripts/auto_update_and_restart.sh` — script that pulls `main`, installs requirements, installs package in editable mode, and restarts the service. Logs to `logs/auto_update.log`.
...
- Because these operations modify code and install packages automatically, ensure you trust the `main` branch contents and backups are in place.
