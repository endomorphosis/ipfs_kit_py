...
- MCP server metrics and status
- IPFS Kit Prometheus metrics
- System telemetry
...
class DataCollector:
    ...
    def __init__(self, config: DashboardConfig):
        ...
        self.mcp_collection_interval = 10  # seconds
        self.ipfs_collection_interval = 15  # seconds
    ...
    async def _collection_loop(self) -> None:
        ...
        last_mcp_collection = 0
        last_ipfs_collection = 0
        ...
        try:
            while self.is_running:
                ...
                # Collect MCP server data
                if current_time - last_mcp_collection >= self.mcp_collection_interval:
                    try:
                        await self._collect_mcp_data()
                        last_mcp_collection = current_time
                    except Exception as e:
                        logger.error(f"Error collecting MCP data: {e}")
                        self._track_error()
                
                # Collect IPFS Kit data
                if (self.config.ipfs_kit_enabled and 
                    current_time - last_ipfs_collection >= self.ipfs_collection_interval):
                    try:
                        await self._collect_ipfs_data()
                        last_ipfs_collection = current_time
                    except Exception as e:
                        logger.error(f"Error collecting IPFS data: {e}")
                        self._track_error()
                ...
                if (self.has_vfs_analytics and 
                    current_time - last_vfs_collection >= self.vfs_collection_interval):
                    try:
    ...
    async def _collect_mcp_data(self) -> None:
        """Collect data from MCP server."""
        if not self.session:
            return
        
        base_url = self.config.get_mcp_server_url()
        
        # Collect health data
        try:
            health_url = f"{base_url}{self.config.mcp_health_endpoint}"
            async with self.session.get(health_url) as response:
                if response.status == 200:
                    health_data = await response.json()
                    await self._process_mcp_health_data(health_data)
                else:
                    logger.warning(f"MCP health endpoint returned {response.status}")
                    self._add_metric("mcp_server_health_status", 0, {"status": "unhealthy"})
        except Exception as e:
            logger.debug(f"Failed to collect MCP health data: {e}")
            self._add_metric("mcp_server_health_status", 0, {"status": "error"})
        
        # Collect metrics data
        try:
            metrics_url = f"{base_url}{self.config.mcp_metrics_endpoint}"
            async with self.session.get(metrics_url) as response:
                if response.status == 200:
                    metrics_data = await response.json()
                    await self._process_mcp_metrics_data(metrics_data)
                else:
                    logger.warning(f"MCP metrics endpoint returned {response.status}")
        except Exception as e:
            logger.debug(f"Failed to collect MCP metrics data: {e}")
    
    async def _process_mcp_health_data(self, health_data: Dict[str, Any]) -> None:
        """Process MCP server health data."""
        # Overall health status
        status = health_data.get("status", "unknown")
        health_value = 1 if status == "healthy" else 0
        self._add_metric("mcp_server_health_status", health_value, {"status": status})
        ...
        self._add_metric("mcp_server_uptime_seconds", uptime)
        
        # Last check timestamp
        timestamp = health_data.get("timestamp", time.time())
        self._add_metric("mcp_server_last_check", timestamp)
    
    async def _process_mcp_metrics_data(self, metrics_data: Dict[str, Any]) -> None:
        """Process MCP server metrics data."""
        # Performance metrics
        perf_metrics = metrics_data.get("performance_metrics", {})
        for backend, backend_metrics in perf_metrics.items():
            for operation, operation_data in backend_metrics.items():
                # Operation counts
                count = operation_data.get("count", 0)
                self._add_metric(
                ...
                # Average time
                avg_time = operation_data.get("avg_time", 0)
                self._add_metric(
                ...
                # Success rate
                success_rate = operation_data.get("success_rate", 0)
                self._add_metric(
        ...
        # Capacity metrics
        capacity_metrics = metrics_data.get("capacity_metrics", {})
        backends = capacity_metrics.get("backends", {})
        for backend, backend_data in backends.items():
            # Storage usage
            used = backend_data.get("used", 0)
            available = backend_data.get("available", 0)
            
            self._add_metric("mcp_storage_used_bytes", used, {"backend": backend})
            self._add_metric("mcp_storage_available_bytes", available, {"backend": backend})
            
            # Content count
            content_count = backend_data.get("content_count", 0)
            self._add_metric("mcp_content_count", content_count, {"backend": backend})
        
        # API metrics
        api_metrics = metrics_data.get("api_metrics", {})
        if api_metrics:
            for endpoint, count in api_metrics.get("request_counts", {}).items():
                self._add_metric("mcp_api_requests_total", count, {"endpoint": endpoint})
    
    async def _collect_ipfs_data(self) -> None:
        """Collect data from IPFS Kit Prometheus endpoint."""
        if not self.session:
            return
        
        try:
            # Try to get data from localhost Prometheus endpoint
            prometheus_url = f"http://localhost:9100{self.config.prometheus_endpoint}"
            async with self.session.get(prometheus_url) as response:
                if response.status == 200:
                    prometheus_data = await response.text()
                    await self._process_prometheus_data(prometheus_data)
                else:
                    logger.debug(f"IPFS Prometheus endpoint returned {response.status}")
        except Exception as e:
            logger.debug(f"Failed to collect IPFS Prometheus data: {e}")
        
        # Try to collect from IPFS Kit instance directly
        try:
            await self._collect_ipfs_kit_data_directly()
        except Exception as e:
            logger.debug(f"Failed to collect IPFS Kit data directly: {e}")
    
    async def _process_prometheus_data(self, prometheus_text: str) -> None:
        """Process Prometheus format metrics."""
        ...
        for line in lines:
            ...
            try:
                # Simple Prometheus parsing
                # Format: metric_name{labels} value timestamp
                if '{' in line:
                    metric_name = line.split('{')[0]
                    rest = line.split('{')[1]
                    labels_part = rest.split('}')[0]
                ...
                tags = {}
                if labels_part:
                    # Simple label parsing (not fully compliant with Prometheus format)
                    label_pairs = labels_part.split(',')
                ...
                # Add metric with ipfs_ prefix to distinguish from MCP metrics
                self._add_metric(f"ipfs_{metric_name}", value, tags)
                
            except Exception as e:
                logger.debug(f"Failed to parse Prometheus line '{line}': {e}")
    
    async def _collect_ipfs_kit_data_directly(self) -> None:
        """Collect data directly from IPFS Kit instances."""
        try:
            # Try to import and get IPFS Kit metrics directly
            from ipfs_kit_py.ipfs_kit import IPFSKit
            from ipfs_kit_py.performance_metrics import PerformanceMetrics
            
            # This is a placeholder - in practice, we would need access to
            # the actual IPFS Kit instance or a registry of instances
            # For now, we'll simulate some basic metrics
            
            # Virtual filesystem metrics
            self._add_metric("ipfs_vfs_operations_total", 0, {"operation": "read"})
            self._add_metric("ipfs_vfs_operations_total", 0, {"operation": "write"})
            self._add_metric("ipfs_vfs_operations_total", 0, {"operation": "list"})
            
            # Cache metrics
            self._add_metric("ipfs_cache_hit_ratio", 0.85)
            self._add_metric("ipfs_cache_size_bytes", 1024 * 1024 * 100)
            
        except ImportError:
            logger.debug("IPFS Kit not available for direct data collection")
    ...
    def _generate_vfs_performance_insights(self, analytics: Dict[str, Any]) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            backend_health = analytics.get('backend_health', {})
            if backend_health and 'summary' in backend_health:
                health_pct = backend_health['summary'].get('health_percentage', 100)
                unhealthy_count = backend_health['summary'].get('unhealthy_backends', 0)
                ...
                if unhealthy_count > 0:
                    insights.append({
                        ...
                        'title': 'Backend Health Issues',
                        'description': f'{unhealthy_count} backend(s) are unhealthy',
                        'impact': 'Reduced system reliability and potential service disruption',
                        'metrics': {
                            'unhealthy_backends': unhealthy_count,
                            'backend_health_percentage': health_pct
                        }
                    })
            ...
            insights.append({
                ...
                'description': f'Error analyzing VFS performance: {e}',
                'impact': 'Limited visibility into VFS performance',
                'metrics': {}
            })
        
        return insights
    
    def _generate_vfs_optimization_recommendations(self, analytics: Dict[str, Any]) -> List[Dict[str, Any]]:
        """Generate VFS optimization recommendations."""
        recommendations = []
        ...
        try:
            ...
            if cache and 'current_metrics' in cache:
                ...
                if hit_rate < 0.8:
                    recommendations.append({
                        ...
                        'expected_impact': 'Reduced latency and backend load',
                        'metrics': {'current_hit_rate': hit_rate, 'target_hit_rate': 0.85}
                    })
                ...
                if utilization > 0.9:
                    recommendations.append({
                        ...
                        'priority': 'medium',
                        'title': 'Scale Cache Resources',
                        'description': 'Cache memory pressure is high',
                        'actions': [
                        ...
                        'expected_impact': 'Improved cache efficiency and reduced evictions',
                        'metrics': {'current_utilization': utilization, 'target_utilization': 0.8}
                    })
            ...
            if bandwidth and 'interfaces' in bandwidth:
                for interface, stats in bandwidth['interfaces'].items():
                    peak_in = stats.get('peak_in_bps', 0)
                    avg_in = stats.get('average_in_bps', 0)
                    
                    if peak_in > avg_in * 10 and avg_in > 0:  # High peak-to-average ratio
                        recommendations.append({
                            'category': 'bandwidth_optimization',
                            'priority': 'medium',
                            'title': f'Optimize {interface} Bandwidth Usage',
                            'description': f'High bandwidth spikes detected on {interface}',
                            'actions': [
                                'Implement bandwidth throttling',
                                'Add load balancing across interfaces',
                                'Optimize data transfer patterns',
                                'Consider bandwidth QoS policies'
                            ],
                            'expected_impact': 'More consistent bandwidth usage and reduced congestion',
                            'metrics': {
                                'interface': interface,
                                'peak_to_avg_ratio': peak_in / avg_in if avg_in > 0 else 0
                            }
                        })
            ...
            if replication and 'current_status' in replication:
                health_pct = replication['current_status'].get('health_percentage', 100)
                sync_lag = replication['current_status'].get('sync_lag_seconds', 0)
                failed_ops = replication['current_status'].get('failed_operations', 0)
                
                if health_pct < 90:
                    recommendations.append({
                        'category': 'replication_reliability',
                        'priority': 'high',
                        'title': 'Improve Replication Health',
                        'description': 'Replication health is below acceptable threshold',
                        'actions': [
                            'Investigate and repair failed replicas',
                            'Check network connectivity between replicas',
                            'Review replication configuration',
                            'Implement replica health monitoring'
                        ],
                        'expected_impact': 'Improved data redundancy and availability',
                        'metrics': {'current_health': health_pct, 'target_health': 95}
                    })
                
                if sync_lag > 60:  # 1 minute
                    recommendations.append({
                        'category': 'replication_performance',
                        'priority': 'medium',
                        'title': 'Reduce Replication Lag',
                        'description': 'Replication synchronization is too slow',
                        'actions': [
                            'Optimize replication worker performance',
                            'Increase replication bandwidth',
                            'Review replication batch sizes',
                            'Implement priority-based replication'
                        ],
                        'expected_impact': 'Faster data consistency and reduced sync lag',
                        'metrics': {'current_lag': sync_lag, 'target_lag': 30}
                    })
            ...
            if backend_health and 'backends' in backend_health:
                unhealthy_backends = [
                    name for name, status in backend_health['backends'].items()
                ...
                if unhealthy_backends:
                    recommendations.append({
                        ...
                        'priority': 'critical',
                        'title': 'Restore Failed Backends',
                        'description': f'Backend services are not responding: {", ".join(unhealthy_backends)}',
                        'actions': [
                            'Investigate backend connectivity issues',
                            'Restart failed backend services',
                            'Check backend resource utilization',
                            'Implement backend health checks and auto-recovery'
                        ],
                        'expected_impact': 'Restored system functionality and improved reliability',
                        'metrics': {'failed_backends': unhealthy_backends}
                    })
                
                # Check for high latency backends
                high_latency_backends = []
                for name, status in backend_health['backends'].items():
                    avg_latency = status.get('average_latency_ms', 0)
                    if avg_latency > 1000:  # 1 second
                        high_latency_backends.append((name, avg_latency))
                
                if high_latency_backends:
                    recommendations.append({
                        'category': 'performance_optimization',
                        'priority': 'medium',
                        'title': 'Optimize High-Latency Backends',
                        'description': 'Some backends have high response times',
                        'actions': [
                            'Investigate backend performance bottlenecks',
                            'Optimize backend resource allocation',
                            'Consider backend caching strategies',
                            'Implement backend load balancing'
                        ],
                        'expected_impact': 'Improved response times and user experience',
                        'metrics': {'high_latency_backends': high_latency_backends}
                    })
            ...
            recommendations.append({
                'category': 'monitoring',
                'priority': 'low',
                'title': 'Fix Recommendation Analysis',
                'description': f'Error generating recommendations: {e}',
                'actions': ['Check dashboard logs for detailed error information'],
                'expected_impact': 'Restored recommendation functionality',
                'metrics': {}
            })
        
        return recommendations
    
    def _analyze_vfs_trends(self) -> Dict[str, Any]:
        """Analyze VFS performance trends from historical data."""
        trends = {}
        
        try:
            # Analyze recent trends in key metrics
            current_time = time.time()
            one_hour_ago = current_time - 3600
            
            # VFS operations trend
            ops_metric = self.get_metric("vfs_operations_per_second")
            if ops_metric:
                recent_points = ops_metric.get_range(one_hour_ago, current_time)
            ...
            # Error rate trend
            error_metric = self.get_metric("vfs_error_rate")
            if error_metric:
                recent_points = error_metric.get_range(one_hour_ago, current_time)
                if len(recent_points) > 1:
                    values = [p.value for p in recent_points]
                    ...
                    trends['error_rate'] = {
                        'average_percentage': avg_error_rate * 100,
                        'trend_direction': trend_direction,
                        'data_points': len(recent_points),
                        'time_range_hours': 1
                    }
            ...
            cache_metric = self.get_metric("vfs_cache_hit_rate")
            if cache_metric:
                recent_points = cache_metric.get_range(one_hour_ago, current_time)
                if len(recent_points) > 1:
                    values = [p.value for p in recent_points]
                    avg_hit_rate = sum(values) / len(values)
                    trend_direction = "increasing" if values[-1] > values[0] else "decreasing"
                    
                    trends['cache_hit_rate'] = {
                        'average_percentage': avg_hit_rate * 100,
                        'trend_direction': trend_direction,
                        'data_points': len(recent_points),
                        'time_range_hours': 1
                    }
            ...
            logger.error(f"Error analyzing VFS trends: {e}")
            trends['error'] = str(e)
        
        return trends
    
    def _generate_vfs_health_summary(self, analytics: Dict[str, Any]) -> Dict[str, Any]:
        """Generate a comprehensive VFS health summary."""
        try:
            # Overall health
            overall_health = analytics.get('overall_health', {})
            health_score = overall_health.get('score', 0)
            ...
            components['operations'] = {
                ...
                'status': 'healthy' if error_rate < 0.05 else 'warning' if error_rate < 0.2 else 'critical',
                'metrics': {'error_rate': error_rate, 'ops_per_second': ops_per_sec}
            }
            ...
            if cache and 'current_metrics' in cache:
                ...
                if utilization > 0.9:
                    cache_score *= 0.8  # Penalize high utilization
                
                components['cache'] = {
                    'score': cache_score,
                    'status': 'healthy' if hit_rate > 0.8 else 'warning' if hit_rate > 0.6 else 'critical',
                    'metrics': {'hit_rate': hit_rate, 'utilization': utilization}
                }
            ...
            if replication and 'current_status' in replication:
                ...
                components['replication'] = {
                    ...
                    'metrics': replication['current_status']
                }
            
            # Backend health
            backend_health = analytics.get('backend_health', {})
            if backend_health and 'summary' in backend_health:
                backend_health_pct = backend_health['summary'].get('health_percentage', 100)
                
                components['backends'] = {
                    'score': backend_health_pct,
                    'status': 'healthy' if backend_health_pct > 90 else 'warning' if backend_health_pct > 70 else 'critical',
                    'metrics': backend_health['summary']
                }
            ...
            logger.error(f"Error generating VFS health summary: {e}")
            return {
                'error': str(e),
                'timestamp': time.time()
            }
