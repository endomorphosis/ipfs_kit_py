...
class TestClusterHealthMonitoring:
    ...
    def test_node_metrics_collection(self, monitoring_cluster):
        ...
        master.ipfs_cluster_ctl.ipfs_cluster_ctl_status = MagicMock(return_value={
            "success": True,
            "peer_statuses": [
                {
                    "id": "QmMasterNodeID",
        ...
        def collect_metrics(**kwargs):
            ...
            for peer in status.get("peer_statuses", []):
                ...
                if "freespace" in peer_metrics and "reposize" in peer_metrics:
                    ...
                    disk_usage_percent = (peer_metrics["reposize"] / total_space) * 100 if total_space > 0 else 0
                ...
                if "memory_used_mb" in peer_metrics and "memory_total_mb" in peer_metrics:
                    ...
                    memory_usage_percent = (peer_metrics["memory_used_mb"] / peer_metrics["memory_total_mb"]) * 100 if peer_metrics["memory_total_mb"] > 0 else 0
        ...
        assert metrics["nodes"]["QmWorker2ID"]["metrics"]["disk_usage_percent"] > 80  # Worker2 has high disk usage
    ...
    def test_alert_generation(self, monitoring_cluster):
        ...
        def check_thresholds(metrics_data, **kwargs):
            ...
            config = master.metadata.get("config", {}).get("Monitoring", {}).get("AlertThresholds", {})
        ...
        assert any(alert["type"] == "cpu_usage_high" and alert["node_id"] == "QmWorker1ID" for alert in alerts)
        assert any(alert["type"] == "memory_usage_high" and alert["node_id"] == "QmWorker2ID" for alert in alerts)
        assert any(alert["type"] == "disk_usage_high" and alert["node_id"] == "QmWorker2ID" for alert in alerts)
    ...
    def test_automatic_recovery_actions(self, monitoring_cluster):
        ...
        assert any(action["action"] == "throttle_operations" and action["node_id"] == "QmWorker1ID" for action in actions)
        assert any(action["action"] == "reallocate_pins" and action["node_id"] == "QmWorker2ID" for action in actions)
        assert any(action["action"] == "run_garbage_collection" and action["node_id"] == "QmWorker2ID" for action in actions)
    ...
    def test_metrics_aggregation(self, monitoring_cluster):
        ...
        def aggregate_metrics(time_range="24h", interval="1h", **kwargs):
            ...
            for metrics in filtered_metrics:
                ...
                for node_id, node_data in metrics["nodes"].items():
                    ...
                    if node_id in aggregated["nodes"]:
                        ...
                        node_series["cpu_usage_percent"].append(node_metrics.get("cpu_usage_percent", 0))
                        node_series["memory_usage_percent"].append(node_metrics.get("memory_usage_percent", 0))
                        node_series["disk_usage_percent"].append(node_metrics.get("disk_usage_percent", 0))
                        node_series["peers_connected"].append(node_metrics.get("peers_connected", 0))
                        node_series["pins_total"].append(node_metrics.get("pins_total", 0))
                        node_series["pins_in_progress"].append(node_metrics.get("pins_in_progress", 0))
                ...
                aggregated["cluster"]["cpu_usage_percent"].append(sum(cpu_values) / len(cpu_values) if cpu_values else 0)
                aggregated["cluster"]["memory_usage_percent"].append(sum(memory_values) / len(memory_values) if memory_values else 0)
                aggregated["cluster"]["disk_usage_percent"].append(sum(disk_values) / len(disk_values) if disk_values else 0)
            ...
            aggregated["summary"] = {
                ...
                "cluster": {
                    "cpu_usage_percent_avg": sum(aggregated["cluster"]["cpu_usage_percent"]) / len(aggregated["cluster"]["cpu_usage_percent"]) if aggregated["cluster"]["cpu_usage_percent"] else 0,
                    "memory_usage_percent_avg": sum(aggregated["cluster"]["memory_usage_percent"]) / len(aggregated["cluster"]["memory_usage_percent"]) if aggregated["cluster"]["memory_usage_percent"] else 0,
                    "disk_usage_percent_avg": sum(aggregated["cluster"]["disk_usage_percent"]) / len(aggregated["cluster"]["disk_usage_percent"]) if aggregated["cluster"]["disk_usage_percent"] else 0,
                    "pins_total_current": aggregated["cluster"]["pins_total"][-1] if aggregated["cluster"]["pins_total"] else 0
                }
            }
    ...
    def test_metrics_export_formats(self, monitoring_cluster):
        ...
        def export_csv(**kwargs):
            ...
            for i, ts in enumerate(aggregated_metrics["timestamps"]):
                csv_lines.append(f"{ts},cluster,{aggregated_metrics['cluster']['cpu_usage_percent'][i]},{aggregated_metrics['cluster']['memory_usage_percent'][i]},{aggregated_metrics['cluster']['disk_usage_percent'][i]}")
            
            # Add node-specific data
            for node_id, node_data in aggregated_metrics["nodes"].items():
                for i, ts in enumerate(aggregated_metrics["timestamps"]):
                    csv_lines.append(f"{ts},{node_id},{node_data['cpu_usage_percent'][i]},{node_data['memory_usage_percent'][i]},{node_data['disk_usage_percent'][i]}")
        ...
        assert len(csv_lines) == 1 + 24 + (24 * len(aggregated_metrics["nodes"]))  # Header + cluster lines + node lines
...
class TestConfigurationManagement:
    ...
    def test_configuration_validation(self, monitoring_cluster):
        ...
        def validate_config(config, **kwargs):
            ...
            known_keys = [
                ...
                "pin_recovery_timeout", "pinning_timeout",
                "pin_tracker"
            ]
    ...
    def test_configuration_distribution(self, monitoring_cluster):
        ...
        def distribute_config(config, **kwargs):
            ...
            accepted_peers = sum(1 for resp in peer_responses.values() if resp["status"] == "accepted")
