#!/usr/bin/env python3
# test/test_wal_api_anyio.py

"""
Unit tests for the WAL API AnyIO module.

These tests validate the REST API endpoints for the Write-Ahead Log system with AnyIO support:
1. WAL operation listing and filtering
2. WAL operation status and management
3. WAL configuration retrieval and updates
4. WAL metrics and monitoring with AnyIO task groups
"""
...
class TestWALAPIAnyIO(unittest.TestCase):
    ...
    def test_get_metrics(self, mock_create_stream, mock_create_task_group):
        """Test getting WAL metrics with AnyIO task groups."""
        # Setup mock task group and streams for AnyIO
