"""
Fixed test server for MCP API testing.
This script creates a standalone FastAPI server with all the endpoints needed for the MCP API tests.
"""
...
app = FastAPI(title="Fixed MCP Test Server")
...
# Define the API prefix to match test script
api_prefix = "/api/v0"

# Health endpoint - the most important one for tests to pass
...
async def ipfs_cat(cid: str):
    ...
    return PlainTextResponse(f"Simulated content for {cid}")
...
async def root():
    ...
    return {
        "message": "Fixed MCP Test Server is running",
