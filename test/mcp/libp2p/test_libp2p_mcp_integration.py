...
try:
    ...
    from ipfs_kit_py.libp2p.enhanced_content_routing import EnhancedContentRouter, apply_to_peer
    ...
    try:
        ...
        logger.error("Failed to import install_libp2p module")
        HAS_LIBP2P = False


# Skip tests if libp2p is not available
libp2p_not_available = not HAS_LIBP2P
skip_if_no_libp2p = pytest.mark.skipif(
    libp2p_not_available,
    reason="libp2p is not available"
)
