    ...
    def test_register_routes(self):
        ...
        for route in expected_routes:
            ...
            for call in add_route_calls:
                # First argument (args[0]) is the path
                if call.args and call.args[0] == route:
                    registered = True
                    break
        ...
        for call in add_route_calls:
            ...
            if len(call.args) >= 2:
                ...
                # In our new implementation, the handler is passed as a positional argument
                # Check if it's in kwargs instead
                for key, value in call.kwargs.items():
                    if callable(value) and key in ("endpoint", "route", "handler"):
                        handler = value
                        handler_name = handler.__name__
                        assert "async" in handler_name, f"Handler {handler_name} is not an async method"
