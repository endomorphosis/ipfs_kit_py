...
from ipfs_kit_py.ipld_knowledge_graph import (
    IPLDGraphDB,
    KnowledgeGraphQuery,
    GraphRAG
)
...
class TestIPLDKnowledgeGraph(unittest.TestCase):
    ...
    def test_query_related(self):
        ...
        results = self.graph_db.query_related("doc1", relationship_type="authored", direction="incoming")
        self.assertEqual(len(results), 1)
        self.assertEqual(results[0]["relationship_type"], "authored")
    ...
    def test_graphrag_retrieve(self):
        ...
        with patch('ipfs_kit_py.ipld_knowledge_graph.EMBEDDINGS_AVAILABLE', False):
            ...
            prompt = rag.generate_llm_prompt(
                user_query="Tell me about knowledge graphs",
                context=context
            )
        ...
        with patch('ipfs_kit_py.ipld_knowledge_graph.EMBEDDINGS_AVAILABLE', True), \
