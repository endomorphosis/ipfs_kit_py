...
try:
    ...
    def _patch_table_from_arrays(monkeypatch):
        """Helper function to patch Table.from_arrays during tests."""
        original_table_from_arrays = pa.Table.from_arrays
        
        def patched_table_from_arrays(arrays, schema=None, names=None, metadata=None):
        ...
        # This will be called by the patch_arrow_table fixture
        monkeypatch.setattr(pa.Table, 'from_arrays', patched_table_from_arrays)
...
def reset_globals():
    ...
    try:
        ...
        if hasattr(ipfs_kit_py.ipfs_kit, '_initialized_instances'):
            original_values['initialized_instances'] = getattr(ipfs_kit_py.ipfs_kit, '_initialized_instances').copy()
    ...
    try:
        ...
        if 'initialized_instances' in original_values and hasattr(ipfs_kit_py.ipfs_kit, '_initialized_instances'):
            getattr(ipfs_kit_py.ipfs_kit, '_initialized_instances').update(original_values['initialized_instances'])
...
def mock_ipfs():
    ...
    with patch('ipfs_kit_py.ipfs.IPFS') as mock:
...
def mock_cluster_state():
    ...
    with patch('ipfs_kit_py.cluster_state.ArrowClusterState') as mock:
        ...
        tasks_data = [
            [{"id": "task1", "status": "completed", "type": "process"}]
        ]
        tasks_array = pa.array(tasks_data)
        
        content_data = [
            [{"cid": "QmTestCid", "size": 1024}]
        ]
        ...
        real_table = pa.Table.from_arrays(
            [cluster_id_array, master_id_array, nodes_array, tasks_array, content_array, updated_at_array], 
            schema=schema
        )
