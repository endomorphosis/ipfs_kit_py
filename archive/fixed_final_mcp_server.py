#!/usr/bin/env python3
"""
Fixed Final MCP Server Implementation
...
# Global state
PORT = 3002  # Changed to 3002 to avoid port conflicts
...
def import_required_modules():
    ...
    try:
        ...
        # Create FastMCP server with explicit tool dictionary
        server = FastMCP(
            name=f"final-mcp-server",
            instructions="Unified MCP server with comprehensive IPFS tool coverage"
        )
        
        # Initialize tools dictionary if not present
        ...
        try:
            try:
                ...
                logger.info('Virtual filesystem tools registered')
        except Exception as e:
            logger.error(f"Error registering virtual filesystem tools: {e}")
            logger.error(traceback.format_exc())
...
def register_all_tools():
    ...
    try:
        ...
        if hasattr(server, "_tools"):
            ...
            logger.warning("Server has no _tools attribute. Tool registration may have failed.")
        return True
    except Exception as e:
...
def main():
    ...
    global PORT, server_start_time, app
    ...
    async def on_startup():
        ...
        if hasattr(server, "_tools"):
            ...
            logger.warning("No tools were registered during startup.")
