...
try:
    ...
    if hasattr(_real_module, "__all__"):
        ...
        for name in __all__:
            try:
                ...
                logger.warning(f"Could not import {name} from ipfs_kit_py.mcp_server.server")
    else:
    ...
    logger.debug("Successfully imported from ipfs_kit_py.mcp_server.server")
except ImportError as e:
    logger.warning(f"Failed to import from ipfs_kit_py.mcp_server.server: {e}")
    __all__ = []
