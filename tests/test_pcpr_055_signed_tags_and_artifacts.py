"""PCPR-055: produce Kit signed tags and artifacts."""
...
from ipfs_kit_py.assurance.signed_tags_and_artifacts import (
...
def test_closed_vocabularies_match_pcpr_055_requirements() -> None:
    ...
    assert INTERFACE == "KitSignedTagsAndArtifacts@1"
    assert SCHEMA == "ipfs_kit_py/assurance/signed-tags-and-artifacts@1"
    assert TAG_KIND == "declared_signed_tag_policy"
    assert CHECKSUM_KIND == "declared_artifact_checksum_manifest"
    assert INTENDED_TAG_NAME == "ipfs_kit_py-v0.3.0"
...
def test_committed_files_match_generator() -> None:
    ...
    assert 'interface = "KitSignedTagsAndArtifacts@1"' in pyproject
