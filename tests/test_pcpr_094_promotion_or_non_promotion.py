"""PCPR-094: Kit binding to Accelerate promotion or non-promotion receipt."""
...
from ipfs_kit_py.assurance.promotion_or_non_promotion import (
...
def test_closed_vocabularies_match_pcpr_094_requirements() -> None:
    ...
    assert INTERFACE == "KitPromotionOrNonPromotionBinding@1"
    assert SCHEMA == "ipfs_kit_py/assurance/promotion-or-non-promotion-binding@1"
    ...
    assert binding["storage"]["survives_promotion_or_non_promotion"] is True
    ...
    assert client["kind"] == "promotion_or_non_promotion"
    ...
    with pytest.raises(KitPromotionOrNonPromotionError, match="remints"):
        refuse_pack_cid_remint(
            "baguqeerabbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbb"
        )
    with pytest.raises(KitPromotionOrNonPromotionError, match="cannot complete"):
        refuse_model_completion("frontier_model")
    with pytest.raises(KitPromotionOrNonPromotionError, match="DuckDB"):
        refuse_database_edit(edited=True)
    with pytest.raises(KitPromotionOrNonPromotionError, match="memory"):
        refuse_memory_reconstruction(from_memory=True)
    with pytest.raises(KitPromotionOrNonPromotionError, match="live durable"):
...
def test_static_probes_show_declared_binding_constraints() -> None:
    ...
    assert probes["gate_owned_by_accelerate"].present is True
    assert probes["current_root_survives_gate_from_disk"].present is True
    assert probes["gate_not_stored_as_live_bytes"].present is True
...
def test_committed_files_match_generator() -> None:
    ...
    assert 'interface = "KitPromotionOrNonPromotionBinding@1"' in pyproject


def test_simulated_live_probe_is_rejected() -> None:
    dummy = "baguqeerabbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbb"
    with pytest.raises(KitPromotionOrNonPromotionError, match="simulated"):
        qualify_promotion_or_non_promotion(
            (
                OutcomeProbe(
                    probe_id="bogus",
    ...
    with pytest.raises(KitPromotionOrNonPromotionError, match="measured_live"):
        qualify_promotion_or_non_promotion(
            (
                OutcomeProbe(
                    probe_id="bogus",
