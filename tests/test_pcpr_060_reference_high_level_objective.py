"""PCPR-060: Kit binding to the reference high-level objective."""
...
from ipfs_kit_py.assurance.reference_high_level_objective import (
...
def test_closed_vocabularies_match_pcpr_060_requirements() -> None:
    ...
    assert INTERFACE == "KitReferenceHighLevelObjectiveBinding@1"
    assert SCHEMA == "ipfs_kit_py/assurance/reference-high-level-objective-binding@1"
    ...
    with pytest.raises(KitReferenceHighLevelObjectiveError, match="remints"):
        refuse_objective_remint(
...
def test_committed_files_match_generator() -> None:
    ...
    assert 'interface = "KitReferenceHighLevelObjectiveBinding@1"' in pyproject


def test_simulated_live_probe_is_rejected() -> None:
    with pytest.raises(KitReferenceHighLevelObjectiveError, match="simulated"):
        qualify_reference_high_level_objective(
            (
                OutcomeProbe(
                    probe_id="bogus",
