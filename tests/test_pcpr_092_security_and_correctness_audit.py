"""PCPR-092: Kit binding to Accelerate security and correctness audit package."""
...
from ipfs_kit_py.assurance.security_and_correctness_audit import (
...
def test_closed_vocabularies_match_pcpr_092_requirements() -> None:
    ...
    assert INTERFACE == "KitSecurityAndCorrectnessAuditPackageBinding@1"
    assert SCHEMA == (
    ...
    assert binding["storage"]["survives_audit_package"] is True
    ...
    assert client["kind"] == "security_and_correctness_audit_package"
    ...
    with pytest.raises(KitSecurityAndCorrectnessAuditError, match="remints"):
        refuse_pack_cid_remint(
            "baguqeerabbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbb"
        )
    with pytest.raises(KitSecurityAndCorrectnessAuditError, match="cannot complete"):
        refuse_model_completion("frontier_model")
    with pytest.raises(KitSecurityAndCorrectnessAuditError, match="DuckDB"):
        refuse_database_edit(edited=True)
    with pytest.raises(KitSecurityAndCorrectnessAuditError, match="memory"):
        refuse_memory_reconstruction(from_memory=True)
    with pytest.raises(KitSecurityAndCorrectnessAuditError, match="live durable"):
...
def test_static_probes_show_declared_binding_constraints() -> None:
    ...
    assert probes["audit_package_owned_by_accelerate"].present is True
    assert probes["current_root_survives_audit_package_from_disk"].present is True
    assert probes["audit_package_not_stored_as_live_bytes"].present is True
...
def test_committed_files_match_generator() -> None:
    ...
    assert 'interface = "KitSecurityAndCorrectnessAuditPackageBinding@1"' in pyproject


def test_simulated_live_probe_is_rejected() -> None:
    dummy = "baguqeerabbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbb"
    with pytest.raises(KitSecurityAndCorrectnessAuditError, match="simulated"):
        qualify_audit_package(
            (
                OutcomeProbe(
                    probe_id="bogus",
    ...
    with pytest.raises(KitSecurityAndCorrectnessAuditError, match="measured_live"):
        qualify_audit_package(
            (
                OutcomeProbe(
                    probe_id="bogus",
