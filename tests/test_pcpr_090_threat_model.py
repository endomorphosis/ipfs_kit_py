"""PCPR-090: Kit binding to Accelerate threat model."""
...
from ipfs_kit_py.assurance.threat_model import (
...
def test_closed_vocabularies_match_pcpr_090_requirements() -> None:
    ...
    assert INTERFACE == "KitThreatModelBinding@1"
    assert SCHEMA == "ipfs_kit_py/assurance/threat-model-binding@1"
    ...
    assert binding["storage"]["survives_threat_model"] is True
    ...
    assert client["kind"] == "threat_model"
    ...
    with pytest.raises(KitThreatModelError, match="remints"):
        refuse_pack_cid_remint(
            "baguqeerabbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbb"
        )
    with pytest.raises(KitThreatModelError, match="cannot complete"):
        refuse_model_completion("frontier_model")
    with pytest.raises(KitThreatModelError, match="DuckDB"):
        refuse_database_edit(edited=True)
    with pytest.raises(KitThreatModelError, match="memory"):
        refuse_memory_reconstruction(from_memory=True)
    with pytest.raises(KitThreatModelError, match="live durable"):
...
def test_static_probes_show_declared_binding_constraints() -> None:
    ...
    assert probes["threat_model_owned_by_accelerate"].present is True
    assert probes["current_root_survives_threat_model_from_disk"].present is True
    assert probes["threat_model_not_stored_as_live_bytes"].present is True
...
def test_simulated_live_probe_is_rejected() -> None:
    ...
    with pytest.raises(KitThreatModelError, match="measured_live"):
        qualify_threat_model(
            (
                OutcomeProbe(
                    probe_id="bogus",
