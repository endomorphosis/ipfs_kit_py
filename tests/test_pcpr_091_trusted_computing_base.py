"""PCPR-091: Kit binding to Accelerate trusted computing base."""
...
from ipfs_kit_py.assurance.trusted_computing_base import (
...
def test_closed_vocabularies_match_pcpr_091_requirements() -> None:
    ...
    assert INTERFACE == "KitTrustedComputingBaseBinding@1"
    assert SCHEMA == "ipfs_kit_py/assurance/trusted-computing-base-binding@1"
    ...
    assert binding["storage"]["survives_tcb_inventory"] is True
    ...
    assert client["kind"] == "trusted_computing_base"
    ...
    with pytest.raises(KitTrustedComputingBaseError, match="remints"):
        refuse_pack_cid_remint(
            "baguqeerabbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbb"
        )
    with pytest.raises(KitTrustedComputingBaseError, match="cannot complete"):
        refuse_model_completion("frontier_model")
    with pytest.raises(KitTrustedComputingBaseError, match="DuckDB"):
        refuse_database_edit(edited=True)
    with pytest.raises(KitTrustedComputingBaseError, match="memory"):
        refuse_memory_reconstruction(from_memory=True)
    with pytest.raises(KitTrustedComputingBaseError, match="live durable"):
...
def test_static_probes_show_declared_binding_constraints() -> None:
    ...
    assert probes["tcb_inventory_owned_by_accelerate"].present is True
    assert probes["current_root_survives_tcb_inventory_from_disk"].present is True
    assert probes["tcb_inventory_not_stored_as_live_bytes"].present is True
...
def test_committed_files_match_generator() -> None:
    ...
    assert 'interface = "KitTrustedComputingBaseBinding@1"' in pyproject


def test_simulated_live_probe_is_rejected() -> None:
    dummy = "baguqeerabbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbb"
    with pytest.raises(KitTrustedComputingBaseError, match="simulated"):
        qualify_tcb_inventory(
            (
                OutcomeProbe(
                    probe_id="bogus",
    ...
    with pytest.raises(KitTrustedComputingBaseError, match="measured_live"):
        qualify_tcb_inventory(
            (
                OutcomeProbe(
                    probe_id="bogus",
