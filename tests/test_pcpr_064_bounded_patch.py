"""PCPR-064: Kit binding to the Accelerate bounded PatchPlan."""
...
from ipfs_kit_py.assurance.bounded_patch import (
...
def test_closed_vocabularies_match_pcpr_064_requirements() -> None:
    ...
    assert INTERFACE == "KitBoundedPatchBinding@1"
    assert SCHEMA == "ipfs_kit_py/assurance/bounded-patch-binding@1"
    ...
    assert HERMETIC_CANDIDATE_SUITES[-1].endswith("test_pcpr_064_bounded_patch.py")
    ...
    with pytest.raises(KitBoundedPatchError, match="remints"):
        refuse_pack_cid_remint(
            "baguqeeraaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa"
        )
    with pytest.raises(KitBoundedPatchError, match="cannot complete"):
...
def test_committed_files_match_generator() -> None:
    ...
    assert 'interface = "KitBoundedPatchBinding@1"' in pyproject


def test_simulated_live_probe_is_rejected() -> None:
    with pytest.raises(KitBoundedPatchError, match="simulated"):
        qualify_bounded_patch(
            (
                OutcomeProbe(
                    probe_id="bogus",
    ...
    with pytest.raises(KitBoundedPatchError, match="measured_live"):
        qualify_bounded_patch(
            (
                OutcomeProbe(
                    probe_id="bogus",
