"""PCPR-081: Kit binding to Accelerate generic MCP client."""
...
from ipfs_kit_py.assurance.generic_mcp_client import (
...
def test_closed_vocabularies_match_pcpr_081_requirements() -> None:
    ...
    assert INTERFACE == "KitGenericMcpClientBinding@1"
    assert SCHEMA == "ipfs_kit_py/assurance/generic-mcp-client-binding@1"
    ...
    assert client["kind"] == "generic_mcp_client_demonstration"
    ...
    with pytest.raises(KitGenericMcpClientError, match="remints"):
        refuse_pack_cid_remint(
            "baguqeerabbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbb"
        )
    with pytest.raises(KitGenericMcpClientError, match="cannot complete"):
        refuse_model_completion("frontier_model")
    with pytest.raises(KitGenericMcpClientError, match="DuckDB"):
        refuse_database_edit(edited=True)
    with pytest.raises(KitGenericMcpClientError, match="memory"):
        refuse_memory_reconstruction(from_memory=True)
    with pytest.raises(KitGenericMcpClientError, match="live durable"):
...
def test_committed_files_match_generator() -> None:
    ...
    assert 'interface = "KitGenericMcpClientBinding@1"' in pyproject


def test_simulated_live_probe_is_rejected() -> None:
    dummy = "baguqeerabbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbb"
    with pytest.raises(KitGenericMcpClientError, match="simulated"):
        qualify_generic_mcp_client(
            (
                OutcomeProbe(
                    probe_id="bogus",
    ...
    with pytest.raises(KitGenericMcpClientError, match="measured_live"):
        qualify_generic_mcp_client(
            (
                OutcomeProbe(
                    probe_id="bogus",
