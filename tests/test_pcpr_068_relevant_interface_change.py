"""PCPR-068: Kit binding to Accelerate unrelated state change."""
...
from ipfs_kit_py.assurance.relevant_interface_change import (
...
def test_closed_vocabularies_match_pcpr_068_requirements() -> None:
    ...
    assert INTERFACE == "KitRelevantInterfaceChangeBinding@1"
    assert SCHEMA == "ipfs_kit_py/assurance/relevant-interface-change-binding@1"
    ...
    assert binding["lock_cid"] == "baguqeerawsekbbbt5ccctjt4tydzeahfkahsatb6q5x7k6cy4inrii5lhqmq"
    ...
    with pytest.raises(KitRelevantInterfaceChangeError, match="remints"):
        refuse_pack_cid_remint(
            "baguqeeraaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa"
        )
    with pytest.raises(KitRelevantInterfaceChangeError, match="cannot complete"):
        refuse_model_completion("frontier_model")
    with pytest.raises(KitRelevantInterfaceChangeError, match="PCPR-069"):
...
def test_static_probes_show_declared_binding_constraints() -> None:
    ...
    assert probes["change_not_stored_as_live_bytes"].present is True
...
def test_committed_files_match_generator() -> None:
    ...
    assert 'interface = "KitRelevantInterfaceChangeBinding@1"' in pyproject


def test_simulated_live_probe_is_rejected() -> None:
    with pytest.raises(KitRelevantInterfaceChangeError, match="simulated"):
        qualify_relevant_interface_change(
            (
                OutcomeProbe(
                    probe_id="bogus",
    ...
    with pytest.raises(KitRelevantInterfaceChangeError, match="measured_live"):
        qualify_relevant_interface_change(
            (
                OutcomeProbe(
                    probe_id="bogus",
