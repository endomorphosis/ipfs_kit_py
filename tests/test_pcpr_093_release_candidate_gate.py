"""PCPR-093: Kit binding to Accelerate release-candidate gate."""
...
from ipfs_kit_py.assurance.release_candidate_gate import (
...
def test_closed_vocabularies_match_pcpr_093_requirements() -> None:
    ...
    assert INTERFACE == "KitReleaseCandidateGateBinding@1"
    assert SCHEMA == "ipfs_kit_py/assurance/release-candidate-gate-binding@1"
    ...
    assert binding["storage"]["survives_release_candidate_gate"] is True
    ...
    assert client["kind"] == "release_candidate_gate"
    ...
    with pytest.raises(KitReleaseCandidateGateError, match="remints"):
        refuse_pack_cid_remint(
            "baguqeerabbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbb"
        )
    with pytest.raises(KitReleaseCandidateGateError, match="cannot complete"):
        refuse_model_completion("frontier_model")
    with pytest.raises(KitReleaseCandidateGateError, match="DuckDB"):
        refuse_database_edit(edited=True)
    with pytest.raises(KitReleaseCandidateGateError, match="memory"):
        refuse_memory_reconstruction(from_memory=True)
    with pytest.raises(KitReleaseCandidateGateError, match="live durable"):
...
def test_committed_files_match_generator() -> None:
    ...
    assert 'interface = "KitReleaseCandidateGateBinding@1"' in pyproject


def test_simulated_live_probe_is_rejected() -> None:
    dummy = "baguqeerabbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbb"
    with pytest.raises(KitReleaseCandidateGateError, match="simulated"):
        qualify_release_candidate_gate(
            (
                OutcomeProbe(
                    probe_id="bogus",
    ...
    with pytest.raises(KitReleaseCandidateGateError, match="measured_live"):
        qualify_release_candidate_gate(
            (
                OutcomeProbe(
                    probe_id="bogus",
