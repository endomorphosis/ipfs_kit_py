"""PCPR-057: Kit binding to branch and release gates."""
...
from ipfs_kit_py.assurance.branch_and_release_gates import (
...
def test_closed_vocabularies_match_pcpr_057_requirements() -> None:
    ...
    assert INTERFACE == "KitBranchAndReleaseGatesBinding@1"
    assert SCHEMA == "ipfs_kit_py/assurance/branch-and-release-gates-binding@1"
    ...
    with pytest.raises(KitBranchAndReleaseGatesError, match="remints"):
        refuse_policy_remint(
...
def test_committed_files_match_generator() -> None:
    ...
    assert 'interface = "KitBranchAndReleaseGatesBinding@1"' in pyproject
...
def test_simulated_live_probe_is_rejected() -> None:
    with pytest.raises(KitBranchAndReleaseGatesError, match="simulated"):
        qualify_branch_and_release_gates(
            (
                OutcomeProbe(
                    probe_id="bogus",
