"""PCPR-071: Kit binding to Accelerate recovery and idempotency."""
...
from ipfs_kit_py.assurance.recovery_and_idempotency import (
...
def test_closed_vocabularies_match_pcpr_071_requirements() -> None:
    ...
    assert INTERFACE == "KitRecoveryAndIdempotencyBinding@1"
    assert SCHEMA == "ipfs_kit_py/assurance/recovery-and-idempotency-binding@1"
    ...
    assert binding["storage"]["survives_recovery"] is True
    ...
    assert recovery["kind"] == "recovery_and_idempotency"
    assert recovery["classified_by"] == "ipfs_accelerate_py"
    assert recovery["stored"] is False
    ...
    with pytest.raises(KitRecoveryAndIdempotencyError, match="remints"):
        refuse_pack_cid_remint(
            "baguqeeraaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa"
        )
    with pytest.raises(KitRecoveryAndIdempotencyError, match="cannot complete"):
        refuse_model_completion("frontier_model")
    with pytest.raises(KitRecoveryAndIdempotencyError, match="memory"):
        refuse_memory_reconstruction(from_memory=True)
    with pytest.raises(KitRecoveryAndIdempotencyError, match="live durable"):
        refuse_live_storage(stored=True)
    with pytest.raises(KitRecoveryAndIdempotencyError, match="DuckDB"):
...
def test_static_probes_show_declared_binding_constraints() -> None:
    ...
    assert probes["current_root_survives_recovery_from_disk"].present is True
    assert probes["recovery_not_stored_as_live_bytes"].present is True
...
def test_committed_files_match_generator() -> None:
    ...
    assert 'interface = "KitRecoveryAndIdempotencyBinding@1"' in pyproject


def test_simulated_live_probe_is_rejected() -> None:
    dummy = "baguqeeraaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa"
    with pytest.raises(KitRecoveryAndIdempotencyError, match="simulated"):
        qualify_recovery_and_idempotency(
            (
                OutcomeProbe(
                    probe_id="bogus",
    ...
    with pytest.raises(KitRecoveryAndIdempotencyError, match="measured_live"):
        qualify_recovery_and_idempotency(
            (
                OutcomeProbe(
                    probe_id="bogus",
