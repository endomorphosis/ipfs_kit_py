"""PCPR-096: Kit binding to Accelerate next-bounded-pilot recommendation receipt."""
...
from ipfs_kit_py.assurance.next_bounded_pilot import (
...
def test_closed_vocabularies_match_pcpr_094_requirements() -> None:
    ...
    assert INTERFACE == "KitNextBoundedPilotBinding@1"
    assert SCHEMA == "ipfs_kit_py/assurance/next-bounded-pilot-binding@1"
    ...
    assert binding["storage"]["survives_next_bounded_pilot"] is True
    ...
    assert client["kind"] == "next_bounded_pilot"
    ...
    with pytest.raises(KitNextBoundedPilotError, match="remints"):
        refuse_pack_cid_remint(
            "baguqeerabbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbb"
        )
    with pytest.raises(KitNextBoundedPilotError, match="cannot complete"):
        refuse_model_completion("frontier_model")
    with pytest.raises(KitNextBoundedPilotError, match="DuckDB"):
        refuse_database_edit(edited=True)
    with pytest.raises(KitNextBoundedPilotError, match="memory"):
        refuse_memory_reconstruction(from_memory=True)
    with pytest.raises(KitNextBoundedPilotError, match="live durable"):
...
def test_static_probes_show_declared_binding_constraints() -> None:
    ...
    assert probes["report_owned_by_accelerate"].present is True
    assert probes["current_root_survives_report_from_disk"].present is True
    assert probes["report_not_stored_as_live_bytes"].present is True
...
def test_committed_files_match_generator() -> None:
    ...
    assert 'interface = "KitNextBoundedPilotBinding@1"' in pyproject


def test_simulated_live_probe_is_rejected() -> None:
    dummy = "baguqeerabbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbb"
    with pytest.raises(KitNextBoundedPilotError, match="simulated"):
        qualify_next_bounded_pilot(
            (
                OutcomeProbe(
                    probe_id="bogus",
    ...
    with pytest.raises(KitNextBoundedPilotError, match="measured_live"):
        qualify_next_bounded_pilot(
            (
                OutcomeProbe(
                    probe_id="bogus",
