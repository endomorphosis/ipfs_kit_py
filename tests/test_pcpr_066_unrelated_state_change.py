"""PCPR-066: Kit binding to Accelerate unrelated state change."""
...
from ipfs_kit_py.assurance.unrelated_state_change import (
...
def test_closed_vocabularies_match_pcpr_066_requirements() -> None:
    ...
    assert INTERFACE == "KitUnrelatedStateChangeBinding@1"
    assert SCHEMA == "ipfs_kit_py/assurance/unrelated-state-change-binding@1"
    ...
    assert change["kind"] == "unrelated_documentation"
    ...
    with pytest.raises(KitUnrelatedStateChangeError, match="remints"):
        refuse_pack_cid_remint(
            "baguqeeraaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa"
        )
    with pytest.raises(KitUnrelatedStateChangeError, match="cannot complete"):
        refuse_model_completion("frontier_model")
    with pytest.raises(KitUnrelatedStateChangeError, match="PCPR-067"):
...
def test_committed_files_match_generator() -> None:
    ...
    assert 'interface = "KitUnrelatedStateChangeBinding@1"' in pyproject


def test_simulated_live_probe_is_rejected() -> None:
    with pytest.raises(KitUnrelatedStateChangeError, match="simulated"):
        qualify_unrelated_state_change(
            (
                OutcomeProbe(
                    probe_id="bogus",
    ...
    with pytest.raises(KitUnrelatedStateChangeError, match="measured_live"):
        qualify_unrelated_state_change(
            (
                OutcomeProbe(
                    probe_id="bogus",
