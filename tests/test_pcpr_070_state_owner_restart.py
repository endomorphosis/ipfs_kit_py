"""PCPR-070: Kit binding to Accelerate state-owner restart."""
...
from ipfs_kit_py.assurance.state_owner_restart import (
...
def test_closed_vocabularies_match_pcpr_070_requirements() -> None:
    ...
    assert INTERFACE == "KitAuthoritativeStateOwnerRestartBinding@1"
    assert SCHEMA == "ipfs_kit_py/assurance/state-owner-restart-binding@1"
    ...
    assert binding["storage"]["survives_restart"] is True
    ...
    assert restart["kind"] == "authoritative_state_owner_restart"
    assert restart["classified_by"] == "ipfs_accelerate_py"
    assert restart["stored"] is False
    ...
    with pytest.raises(KitStateOwnerRestartError, match="remints"):
        refuse_pack_cid_remint(
            "baguqeeraaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa"
        )
    with pytest.raises(KitStateOwnerRestartError, match="cannot complete"):
        refuse_model_completion("frontier_model")
    with pytest.raises(KitStateOwnerRestartError, match="memory"):
        refuse_memory_reconstruction(from_memory=True)
    with pytest.raises(KitStateOwnerRestartError, match="live durable"):
        refuse_live_storage(stored=True)
    with pytest.raises(KitStateOwnerRestartError, match="DuckDB"):
...
def test_static_probes_show_declared_binding_constraints() -> None:
    ...
    assert probes["current_root_survives_restart_from_disk"].present is True
    assert probes["restart_not_stored_as_live_bytes"].present is True
...
def test_committed_files_match_generator() -> None:
    ...
    assert 'interface = "KitAuthoritativeStateOwnerRestartBinding@1"' in pyproject


def test_simulated_live_probe_is_rejected() -> None:
    dummy = "baguqeeraaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa"
    with pytest.raises(KitStateOwnerRestartError, match="simulated"):
        qualify_state_owner_restart(
            (
                OutcomeProbe(
                    probe_id="bogus",
    ...
    with pytest.raises(KitStateOwnerRestartError, match="measured_live"):
        qualify_state_owner_restart(
            (
                OutcomeProbe(
                    probe_id="bogus",
