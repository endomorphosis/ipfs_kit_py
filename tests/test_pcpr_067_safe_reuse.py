"""PCPR-067: Kit binding to Accelerate eligible reuse."""
...
from ipfs_kit_py.assurance.safe_reuse import (
...
def test_closed_vocabularies_match_pcpr_067_requirements() -> None:
    ...
    assert INTERFACE == "KitSafeReuseBinding@1"
    assert SCHEMA == "ipfs_kit_py/assurance/safe-reuse-binding@1"
    ...
    with pytest.raises(KitSafeReuseError, match="remints"):
        refuse_pack_cid_remint(
            "baguqeeraaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa"
        )
    with pytest.raises(KitSafeReuseError, match="cannot complete"):
        refuse_model_completion("frontier_model")
    with pytest.raises(KitSafeReuseError, match="must demonstrate"):
        refuse_reuse_not_demonstrated(demonstrated=False)
    with pytest.raises(KitSafeReuseError, match="live reuse"):
...
def test_static_probes_show_declared_binding_constraints() -> None:
    ...
    assert probes["reuse_not_stored_as_live_bytes"].present is True
...
def test_simulated_live_probe_is_rejected() -> None:
    ...
    with pytest.raises(KitSafeReuseError, match="measured_live"):
        qualify_safe_reuse(
            (
                OutcomeProbe(
                    probe_id="bogus",
