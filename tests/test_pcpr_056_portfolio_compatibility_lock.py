"""PCPR-056: Kit binding to the portfolio compatibility lock."""
...
from ipfs_kit_py.assurance.portfolio_compatibility_lock import (
...
def test_closed_vocabularies_match_pcpr_056_requirements() -> None:
    ...
    assert INTERFACE == "KitPortfolioCompatibilityLockBinding@1"
    assert SCHEMA == "ipfs_kit_py/assurance/portfolio-compatibility-lock-binding@1"
    ...
    with pytest.raises(KitPortfolioCompatibilityLockError, match="remints"):
        refuse_lock_remint(
...
def test_committed_files_match_generator() -> None:
    ...
    assert 'interface = "KitPortfolioCompatibilityLockBinding@1"' in pyproject
...
def test_simulated_live_probe_is_rejected() -> None:
    with pytest.raises(KitPortfolioCompatibilityLockError, match="simulated"):
        qualify_portfolio_compatibility_lock(
            (
                OutcomeProbe(
                    probe_id="bogus",
