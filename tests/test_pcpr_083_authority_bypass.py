"""PCPR-083: Kit binding to Accelerate authority bypass."""
...
from ipfs_kit_py.assurance.authority_bypass import (
...
def test_closed_vocabularies_match_pcpr_083_requirements() -> None:
    ...
    assert INTERFACE == "KitAuthorityBypassBinding@1"
    assert SCHEMA == "ipfs_kit_py/assurance/authority-bypass-binding@1"
    ...
    assert binding["storage"]["survives_bypass"] is True
    ...
    assert client["kind"] == "authority_bypass"
    ...
    with pytest.raises(KitAuthorityBypassError, match="remints"):
        refuse_pack_cid_remint(
            "baguqeerabbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbb"
        )
    with pytest.raises(KitAuthorityBypassError, match="cannot complete"):
        refuse_model_completion("frontier_model")
    with pytest.raises(KitAuthorityBypassError, match="DuckDB"):
        refuse_database_edit(edited=True)
    with pytest.raises(KitAuthorityBypassError, match="memory"):
        refuse_memory_reconstruction(from_memory=True)
    with pytest.raises(KitAuthorityBypassError, match="live durable"):
...
def test_static_probes_show_declared_binding_constraints() -> None:
    ...
    assert probes["bypass_owned_by_accelerate"].present is True
    assert probes["current_root_survives_bypass_from_disk"].present is True
    assert probes["bypass_not_stored_as_live_bytes"].present is True
...
def test_committed_files_match_generator() -> None:
    ...
    assert 'interface = "KitAuthorityBypassBinding@1"' in pyproject


def test_simulated_live_probe_is_rejected() -> None:
    dummy = "baguqeerabbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbb"
    with pytest.raises(KitAuthorityBypassError, match="simulated"):
        qualify_authority_bypass(
            (
                OutcomeProbe(
                    probe_id="bogus",
    ...
    with pytest.raises(KitAuthorityBypassError, match="measured_live"):
        qualify_authority_bypass(
            (
                OutcomeProbe(
                    probe_id="bogus",
