"""PCPR-063: Kit binding to the Accelerate deterministic-first route."""
...
from ipfs_kit_py.assurance.deterministic_first_route import (
...
def test_closed_vocabularies_match_pcpr_063_requirements() -> None:
    ...
    assert INTERFACE == "KitDeterministicFirstRouteBinding@1"
    assert SCHEMA == "ipfs_kit_py/assurance/deterministic-first-route-binding@1"
    ...
    with pytest.raises(KitDeterministicFirstRouteError, match="remints"):
        refuse_pack_cid_remint(
            "baguqeeraaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa"
        )
    with pytest.raises(KitDeterministicFirstRouteError, match="cannot complete"):
...
def test_committed_files_match_generator() -> None:
    ...
    assert 'interface = "KitDeterministicFirstRouteBinding@1"' in pyproject


def test_simulated_live_probe_is_rejected() -> None:
    with pytest.raises(KitDeterministicFirstRouteError, match="simulated"):
        qualify_deterministic_first_route(
            (
                OutcomeProbe(
                    probe_id="bogus",
    ...
    with pytest.raises(KitDeterministicFirstRouteError, match="measured_live"):
        qualify_deterministic_first_route(
            (
                OutcomeProbe(
                    probe_id="bogus",
