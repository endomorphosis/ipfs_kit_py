"""PCPR-082: Kit binding to Accelerate objective identity parity."""
...
from ipfs_kit_py.assurance.objective_identity_parity import (
...
def test_closed_vocabularies_match_pcpr_082_requirements() -> None:
    ...
    assert INTERFACE == "KitObjectiveIdentityParityBinding@1"
    assert SCHEMA == "ipfs_kit_py/assurance/objective-identity-parity-binding@1"
    ...
    assert client["kind"] == "objective_identity_parity"
    ...
    with pytest.raises(KitObjectiveIdentityParityError, match="remints"):
        refuse_pack_cid_remint(
            "baguqeerabbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbb"
        )
    with pytest.raises(KitObjectiveIdentityParityError, match="cannot complete"):
        refuse_model_completion("frontier_model")
    with pytest.raises(KitObjectiveIdentityParityError, match="DuckDB"):
        refuse_database_edit(edited=True)
    with pytest.raises(KitObjectiveIdentityParityError, match="memory"):
        refuse_memory_reconstruction(from_memory=True)
    with pytest.raises(KitObjectiveIdentityParityError, match="live durable"):
...
def test_static_probes_show_declared_binding_constraints() -> None:
    ...
    assert probes["parity_owned_by_accelerate"].present is True
    assert probes["current_root_survives_parity_from_disk"].present is True
    assert probes["parity_not_stored_as_live_bytes"].present is True
...
def test_committed_files_match_generator() -> None:
    ...
    assert 'interface = "KitObjectiveIdentityParityBinding@1"' in pyproject


def test_simulated_live_probe_is_rejected() -> None:
    dummy = "baguqeerabbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbb"
    with pytest.raises(KitObjectiveIdentityParityError, match="simulated"):
        qualify_objective_identity_parity(
            (
                OutcomeProbe(
                    probe_id="bogus",
    ...
    with pytest.raises(KitObjectiveIdentityParityError, match="measured_live"):
        qualify_objective_identity_parity(
            (
                OutcomeProbe(
                    probe_id="bogus",
