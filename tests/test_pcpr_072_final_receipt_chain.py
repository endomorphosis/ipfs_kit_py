"""PCPR-072: Kit binding to Accelerate final receipt chain."""
...
from ipfs_kit_py.assurance.final_receipt_chain import (
...
def test_closed_vocabularies_match_pcpr_072_requirements() -> None:
    ...
    assert INTERFACE == "KitFinalReceiptChainBinding@1"
    assert SCHEMA == "ipfs_kit_py/assurance/final-receipt-chain-binding@1"
    ...
    assert binding["storage"]["survives_chain"] is True
    ...
    with pytest.raises(KitFinalReceiptChainError, match="remints"):
        refuse_pack_cid_remint(
            "baguqeeraaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa"
        )
    with pytest.raises(KitFinalReceiptChainError, match="cannot complete"):
        refuse_model_completion("frontier_model")
    with pytest.raises(KitFinalReceiptChainError, match="memory"):
        refuse_memory_reconstruction(from_memory=True)
    with pytest.raises(KitFinalReceiptChainError, match="live durable"):
        refuse_live_storage(stored=True)
    with pytest.raises(KitFinalReceiptChainError, match="DuckDB"):
...
def test_static_probes_show_declared_binding_constraints() -> None:
    ...
    assert probes["current_root_survives_chain_from_disk"].present is True
    assert probes["chain_not_stored_as_live_bytes"].present is True
...
def test_committed_files_match_generator() -> None:
    ...
    assert 'interface = "KitFinalReceiptChainBinding@1"' in pyproject


def test_simulated_live_probe_is_rejected() -> None:
    dummy = "baguqeeraaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa"
    with pytest.raises(KitFinalReceiptChainError, match="simulated"):
        qualify_final_receipt_chain(
            (
                OutcomeProbe(
                    probe_id="bogus",
    ...
    with pytest.raises(KitFinalReceiptChainError, match="measured_live"):
        qualify_final_receipt_chain(
            (
                OutcomeProbe(
                    probe_id="bogus",
