"""PCPR-095: Kit binding to Accelerate residual-gap report receipt."""
...
from ipfs_kit_py.assurance.residual_gap_report import (
...
def test_closed_vocabularies_match_pcpr_094_requirements() -> None:
    ...
    assert INTERFACE == "KitResidualGapReportBinding@1"
    assert SCHEMA == "ipfs_kit_py/assurance/residual-gap-report-binding@1"
    ...
    assert binding["storage"]["survives_residual_gap_report"] is True
    ...
    assert client["kind"] == "residual_gap_report"
    ...
    with pytest.raises(KitResidualGapReportError, match="remints"):
        refuse_pack_cid_remint(
            "baguqeerabbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbb"
        )
    with pytest.raises(KitResidualGapReportError, match="cannot complete"):
        refuse_model_completion("frontier_model")
    with pytest.raises(KitResidualGapReportError, match="DuckDB"):
        refuse_database_edit(edited=True)
    with pytest.raises(KitResidualGapReportError, match="memory"):
        refuse_memory_reconstruction(from_memory=True)
    with pytest.raises(KitResidualGapReportError, match="live durable"):
...
def test_committed_files_match_generator() -> None:
    ...
    assert 'interface = "KitResidualGapReportBinding@1"' in pyproject


def test_simulated_live_probe_is_rejected() -> None:
    dummy = "baguqeerabbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbb"
    with pytest.raises(KitResidualGapReportError, match="simulated"):
        qualify_residual_gap_report(
            (
                OutcomeProbe(
                    probe_id="bogus",
    ...
    with pytest.raises(KitResidualGapReportError, match="measured_live"):
        qualify_residual_gap_report(
            (
                OutcomeProbe(
                    probe_id="bogus",
