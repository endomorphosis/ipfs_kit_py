"""PCPR-065: Kit binding to Accelerate selected tests and proofs."""
...
from ipfs_kit_py.assurance.selected_tests_and_proofs import (
...
def test_closed_vocabularies_match_pcpr_065_requirements() -> None:
    ...
    assert INTERFACE == "KitSelectedTestsAndProofsBinding@1"
    assert SCHEMA == "ipfs_kit_py/assurance/selected-tests-and-proofs-binding@1"
    ...
    with pytest.raises(KitSelectedTestsAndProofsError, match="remints"):
        refuse_pack_cid_remint(
            "baguqeeraaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa"
        )
    with pytest.raises(KitSelectedTestsAndProofsError, match="cannot complete"):
...
def test_committed_files_match_generator() -> None:
    ...
    assert 'interface = "KitSelectedTestsAndProofsBinding@1"' in pyproject


def test_simulated_live_probe_is_rejected() -> None:
    with pytest.raises(KitSelectedTestsAndProofsError, match="simulated"):
        qualify_selected_tests_and_proofs(
            (
                OutcomeProbe(
                    probe_id="bogus",
    ...
    with pytest.raises(KitSelectedTestsAndProofsError, match="measured_live"):
        qualify_selected_tests_and_proofs(
            (
                OutcomeProbe(
                    probe_id="bogus",
