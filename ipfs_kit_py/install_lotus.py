...
class install_lotus:
    ...
    def ensure_daemon_configured(self) -> bool:
        """Ensure Lotus daemon is minimally configured.

        This is a lightweight hook used by integration smoke tests. It avoids
        invoking external commands; it only checks for the presence of
        `config.toml` and, if missing, writes a placeholder file.
        """
        try:
            ...
            with open(config_file, "w", encoding="utf-8") as f:
                f.write("# lotus config placeholder (generated by ipfs_kit_py)\n")
            return True
        except Exception:
            return False
