...
class install_lotus:
    ...
    def dist_select(self):
        ...
        machine = hardware["machine"].lower() if hardware["machine"] else platform.machine().lower()
        ...
        if machine in ['aarch64', 'arm64']:
            ...
            if "Intel" in hardware["processor"] or "AMD" in hardware["processor"]:
                if "64" in hardware["architecture"]:
                    ...
                    aarch = "x86"
            elif "Qualcomm" in hardware["processor"] or "ARM" in hardware["processor"] or "aarch64" in hardware["processor"]:
                if "64" in hardware["architecture"]:
                    aarch = "arm64"
                elif "32" in hardware["architecture"]:
                    aarch = "arm"
            elif "Apple" in hardware["processor"]:
                if "64" in hardware["architecture"] or "arm64" in machine:
                    aarch = "arm64"
                else:
                    aarch = "x86_64"
            # Default to x86_64 if we can't determine architecture
            else:
                if "64" in hardware["architecture"] or "64" in machine:
                    aarch = "x86_64"
                else:
                    aarch = "x86"
