...
class ipfs_cluster_service:
    def __init__(self, resources=None, metadata=None):
        ...
        try:
            ...
            self.ipfs_cluster_path = self.metadata.get('ipfs_cluster_path', 
                                                   os.path.expanduser("~/.ipfs-cluster"))
            
            # Extract bootstrap peers for IPFS cluster
            self.bootstrap_peers = self.metadata.get('bootstrap_peers', [])
            
            logger.debug(f"Initialized IPFS Cluster Service with role={self.role}, "
    ...
    def run_cluster_service_command(self, cmd_args, check=True, timeout=30, correlation_id=None, shell=False):
        ...
        try:
            ...
            if os.getuid() == 0:
                ...
                if not systemctl_result.get("success", False):
                    logger.warning("Failed to start ipfs-cluster-service via systemctl, will try direct execution")
            else:
            ...
            if ps_result.get("success", False) and ps_result.get("stdout"):
                ...
                if not process_running:
                    logger.warning("ipfs-cluster-service process not found, checking for stale socket")
        ...
        try:
            ...
            if process_running:
                ...
                if status_result.get("success", False):
                    ...
                    result["success"] = True  # Service is running even if we can't get detailed status
            else:
