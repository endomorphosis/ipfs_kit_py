...
class IPLDGraphDB:
    ...
    def _load_indexes(self):
        ...
        try:
            ...
            self.vectors = {"count": 0, "dimension": 0, "index_type": "flat", "entities": {}, "vectors": []}
            self.graph = nx.MultiDiGraph()
            self.change_log = []
    ...
    def vector_search(self, query_vector, top_k=10):
        ...
        if EMBEDDINGS_AVAILABLE and self.vectors["count"] > 10000:
            logger.info("Large vector collection detected. Consider using ipfs_embeddings_py for improved performance.")
        ...
        if FAISS_AVAILABLE and self.vector_index is not None:
            ...
            similarities = np.dot(all_vectors, query_vector.T).squeeze() / (norm_vectors * norm_query)
            ...
            for field in search_fields:
                if field in properties:
                    ...
                    if query in field_value:
                        ...
                        if (field_value == query or 
                            f" {query} " in f" {field_value} " or
                            field_value.startswith(f"{query} ") or
                            field_value.endswith(f" {query}")):
                            score += 0.5
        ...
        try:
            ...
            for rel_id in relationship_ids:
                if rel_id in self.relationships["relationship_cids"]:
                    ...
                    other_entity = relationship["from"] if relationship["to"] == entity_id else relationship["to"]
    ...
    def get_statistics(self):
        ...
        stats = {
            ...
            "relationships": {
                "total": len(self.relationships["relationship_cids"]),
                "by_type": {}
            },
            "vectors": {
                "total": self.vectors["count"],
                "dimension": self.vectors["dimension"]
            },
            "changes": len(self.change_log),
            "storage": {
                "entities_size": 0,
                "relationships_size": 0,
                "total_size": 0
            },
            "graph_metrics": {
    ...
    def get_version_history(self, entity_id=None, limit=10):
        ...
        if entity_id:
            ...
            changes = [
                ...
                if (change.get("entity_id") == entity_id or
                    (change.get("operation") == "add_relationship" and 
                     (change.get("from_entity") == entity_id or change.get("to_entity") == entity_id)) or
                    (change.get("operation") == "delete_relationship" and 
                     (change.get("from_entity") == entity_id or change.get("to_entity") == entity_id)))
            ]
        else:
            # All changes
...
class KnowledgeGraphQuery:
    ...
    def hybrid_search(self, query=None, query_vector=None, entity_type=None, 
                     properties=None, hop_count=1, top_k=10):
...
class GraphRAG:
    ...
    def generate_embedding(self, text):
        ...
        if EMBEDDINGS_AVAILABLE:
            logger.info("Consider using ipfs_embeddings_py.EmbeddingGenerator for production embedding generation")
            
        if self.embedding_model is None:
            raise ValueError("No embedding model available. Either provide an embedding model or use ipfs_embeddings_py")
            
        return self.embedding_model.encode(text)
        
    def retrieve(self, query_text=None, query_vector=None, entity_types=None, 
                top_k=5, hop_count=1):
