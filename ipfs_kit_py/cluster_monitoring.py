...
class ClusterMonitoring:
    ...
    def __init__(self, ipfs_kit_instance):
        ...
        self.alert_thresholds = {
            ...
            "MemoryUsage": self._get_config_value(["Monitoring", "AlertThresholds", "MemoryUsage"], 85),
            "CpuUsage": self._get_config_value(["Monitoring", "AlertThresholds", "CpuUsage"], 90),
            "PinQueueLength": self._get_config_value(["Monitoring", "AlertThresholds", "PinQueueLength"], 100),
            "PeerConnectionLimit": self._get_config_value(["Monitoring", "AlertThresholds", "PeerConnectionLimit"], 5)
        }
        
        # Storage for collected metrics
        self.historical_metrics = []
        self.max_metrics_history = self._get_config_value(["Monitoring", "MaxMetricsHistory"], 1440)  # 24h at 1-minute intervals
    ...
    def start_monitoring(self) -> Dict[str, Any]:
        ...
        try:
            ...
            self.metrics_thread = threading.Thread(
                target=self._metrics_collection_thread,
                name="IPFS-Cluster-Monitoring"
            )
        ...
        try:
            ...
            if self.role == "master" and hasattr(self.ipfs_kit, "ipfs_cluster_ctl"):
                ...
                if status_result.get("success", False):
                    ...
                    logger.warning(f"Failed to collect cluster status: {status_result.get('error', 'Unknown error')}")
    ...
    def check_alert_thresholds(self, metrics_data: Dict[str, Any]) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            for node_id, node_data in metrics_data.get("nodes", {}).items():
                ...
                if "peers_connected" in metrics and metrics.get("peers_connected", 0) < peer_threshold:
                    alerts.append({
                        "level": "warning",
                        "type": "low_peer_count",
                        "node_id": node_id,
        ...
        if is_local:
            ...
            if hasattr(self.ipfs_kit, "ipfs") and hasattr(self.ipfs_kit.ipfs, "ipfs_repo_gc"):
                ...
                if gc_result.get("success", False):
                    return True, f"Successfully ran garbage collection, removed {len(gc_result.get('Keys', []))} objects"
                else:
                    return False, f"Failed to run garbage collection: {gc_result.get('error', 'Unknown error')}"
            else:
            ...
            logger.warning(f"Remote GC on node {node_id} not implemented yet, would use cluster API")
    ...
    def _action_connect_bootstrap_peers(self, node_id: str) -> Tuple[bool, str]:
        ...
        if is_local:
            ...
            if hasattr(self.ipfs_kit, "ipfs") and hasattr(self.ipfs_kit.ipfs, "ipfs_bootstrap_list"):
                ...
                if bootstrap_result.get("success", False):
                    ...
                    return False, f"Failed to get bootstrap list: {bootstrap_result.get('error', 'Unknown error')}"
            else:
                return False, "IPFS instance not available"
    ...
    def _action_adjust_pin_concurrency(self, node_id: str) -> Tuple[bool, str]:
        ...
        if is_local and self.role == "master":
            ...
            # For remote nodes, would use the cluster API
            logger.warning(f"Remote pin concurrency adjustment on node {node_id} not implemented yet")
        ...
        with self.metrics_lock:
            ...
            for key in aggregated["cluster"]:
                ...
                if values:
                    ...
                    # Add latest value
                    aggregated["summary"]["cluster"][f"{key}_current"] = values[-1] if values else None
        ...
        for node_id in node_ids:
            ...
            if node_samples:
                ...
                for key in ["cpu_usage_percent", "memory_usage_percent", "disk_usage_percent", 
                           "peers_connected", "pins_total", "pins_in_progress"]:
                    values = []
    ...
    def export_metrics_csv(self, time_range="24h", interval="1h") -> str:
        ...
        for i, ts in enumerate(metrics["timestamps"]):
            writer.writerow([
                ts,
                "cluster",
                metrics["cluster"]["cpu_usage_percent"][i] if i < len(metrics["cluster"]["cpu_usage_percent"]) else "",
                metrics["cluster"]["memory_usage_percent"][i] if i < len(metrics["cluster"]["memory_usage_percent"]) else "",
                metrics["cluster"]["disk_usage_percent"][i] if i < len(metrics["cluster"]["disk_usage_percent"]) else ""
            ])
        ...
        for node_id, node_data in metrics["nodes"].items():
            for i, ts in enumerate(metrics["timestamps"]):
                writer.writerow([
                    ts,
                    node_id,
                    node_data["cpu_usage_percent"][i] if i < len(node_data["cpu_usage_percent"]) else "",
                    node_data["memory_usage_percent"][i] if i < len(node_data["memory_usage_percent"]) else "",
                    node_data["disk_usage_percent"][i] if i < len(node_data["disk_usage_percent"]) else ""
                ])
