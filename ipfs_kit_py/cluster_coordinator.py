...
class ClusterCoordinator:
    ...
    def _handle_node_failure(self, node_id: str) -> None:
        ...
        for task_id, task in self.tasks.items():
            if task.assigned_to == node_id and task.status in (TaskStatus.ASSIGNED, TaskStatus.PROCESSING):
                ...
                if task_id in self.tasks:
                    ...
                    if result.get("success", False):
                        ...
                        if retry_count < self.max_retries:
                            ...
                            task.assigned_to = None
                            self.task_queue.put(task)
                            logger.info(f"Requeued task {task_id} for retry ({retry_count + 1}/{self.max_retries})")
    ...
    def _check_task_timeouts(self) -> None:
        ...
        for task_id, task in self.tasks.items():
            ...
            if task.timeout is None or task.status not in (TaskStatus.ASSIGNED, TaskStatus.PROCESSING):
                continue
            ...
            if task.updated_at + task.timeout < current_time:
                ...
                if retry_count < self.max_retries:
                    ...
                    task.assigned_to = None
                    self.task_queue.put(task)
                    logger.info(f"Requeued timed out task {task_id} for retry ({retry_count + 1}/{self.max_retries})")
    ...
    def get_tasks(self, status_filter: Optional[List[str]] = None,
    ...
    def get_nodes(self, role_filter: Optional[str] = None,
    ...
    def get_cluster_status(self) -> Dict[str, Any]:
        ...
        node_counts = {
            ...
            "by_status": {
                "online": sum(1 for node in self.nodes.values() if node.status == NodeStatus.ONLINE),
                ...
                "starting": sum(1 for node in self.nodes.values() if node.status == NodeStatus.STARTING),
                "stopping": sum(1 for node in self.nodes.values() if node.status == NodeStatus.STOPPING),
                "unknown": sum(1 for node in self.nodes.values() if node.status == NodeStatus.UNKNOWN)
            }
        }
        ...
        task_counts = {
            ...
            "by_status": {
                "pending": sum(1 for task in self.tasks.values() if task.status == TaskStatus.PENDING),
                "assigned": sum(1 for task in self.tasks.values() if task.status == TaskStatus.ASSIGNED),
                "processing": sum(1 for task in self.tasks.values() if task.status == TaskStatus.PROCESSING),
                "completed": sum(1 for task in self.tasks.values() if task.status == TaskStatus.COMPLETED),
                "failed": sum(1 for task in self.tasks.values() if task.status == TaskStatus.FAILED),
                "cancelled": sum(1 for task in self.tasks.values() if task.status == TaskStatus.CANCELLED),
                "timeout": sum(1 for task in self.tasks.values() if task.status == TaskStatus.TIMEOUT)
            }
        }
        
        # Calculate cluster health
        active_workers = sum(1 for node in self.nodes.values() 
                            if node.role == NodeRole.WORKER and node.status == NodeStatus.ONLINE)
        
        if self.role != NodeRole.MASTER:
            # Non-master nodes have limited information
            health_status = "unknown"
            master_node = next((node for node in self.nodes.values() if node.role == NodeRole.MASTER), None)
