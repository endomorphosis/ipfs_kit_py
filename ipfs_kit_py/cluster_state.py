...
def create_cluster_state_schema():
    ...
    return pa.schema([
        ...
        pa.field('tasks', pa.list_(pa.struct([
            ...
            pa.field('parameters', pa.struct([pa.field('_dummy', pa.string())])),  # With dummy field for Parquet compatibility
...
class ArrowClusterState:
    ...
    def _initialize_empty_state(self):
        ...
        try:
            ...
            self.state_table = pa.table({
                ...
                'nodes': pa.array([], type=pa.list_(pa.struct([
                    pa.field('id', pa.string()),
                    pa.field('peer_id', pa.string()),
                    pa.field('role', pa.string()),
                    pa.field('status', pa.string()),
                ...
                'tasks': pa.array([], type=pa.list_(pa.struct([
                    pa.field('id', pa.string()),
                    pa.field('type', pa.string()),
                    pa.field('status', pa.string()),
                    pa.field('priority', pa.int8()),
                ...
                'content': pa.array([], type=pa.list_(pa.struct([
                    pa.field('cid', pa.string()),
                    pa.field('size', pa.int64()),
                    pa.field('providers', pa.list_(pa.string())),
                    pa.field('replication', pa.int8()),
        ...
        def update_function(current_state):
            ...
            if PANDAS_AVAILABLE:
                try:
                    ...
                    current_time_ms = int(time.time() * 1000)
                    if PANDAS_AVAILABLE:
                        last_seen_ts = pa.scalar(pd.Timestamp(current_time_ms, unit='ms'), type=pa.timestamp('ms'))
                    else:
                    ...
                    if PANDAS_AVAILABLE:
                        ...
                        try:
                            ...
                            timestamp_str = time.strftime('%Y-%m-%d %H:%M:%S', time.localtime(current_ms/1000))
                                ...
                                if field.name == 'tasks':
                                    ...
                                    for task in new_tasks:
                                        ...
                                        if isinstance(task.get('created_at'), pd.Timestamp):
                                            ...
                                            if hasattr(task['created_at'], 'unit') and task['created_at'].unit != 'ms':
                                                task['created_at'] = pd.Timestamp(task['created_at'].value // 10**6, unit='ms')
                                        elif isinstance(task.get('created_at'), (int, float)):
                                            # Convert numeric timestamp to pandas timestamp
                                            task['created_at'] = pd.Timestamp(int(task['created_at']), unit='ms')
                                            
                                        if isinstance(task.get('updated_at'), pd.Timestamp):
                                            # Convert to ms timestamp if needed
                                            if hasattr(task['updated_at'], 'unit') and task['updated_at'].unit != 'ms':
                                                task['updated_at'] = pd.Timestamp(task['updated_at'].value // 10**6, unit='ms')
                                        elif isinstance(task.get('updated_at'), (int, float)):
                                            # Convert numeric timestamp to pandas timestamp
                                            task['updated_at'] = pd.Timestamp(int(task['updated_at']), unit='ms')
                                    ...
                                    try:
                                        ...
                                        task_struct_type = pa.struct([
                                            ...
                                            pa.field('updated_at', pa.timestamp('ms')),
                                            pa.field('assigned_to', pa.string()),
                                            pa.field('parameters', pa.struct([pa.field('_dummy', pa.string())])),
                                            pa.field('result_cid', pa.string())
                                        ])
                                        
                                        # Try to create the array with proper type
                                        arrays.append(pa.array([new_tasks], type=pa.list_(task_struct_type)))
                                        ...
                                        if IN_TEST_ENV:
                                            ...
                                            arrays.append(pa.array([[simple_task]], type=pa.list_(task_struct_type)))
                    ...
                    if tasks_array[0].is_valid():
                        ...
                        for i, field in enumerate(current_state.schema):
                            if field.name == 'tasks':
                                ...
                                arrays.append(pa.array([tasks], type=pa.list_(pa.struct([
                                    pa.field('id', pa.string()),
                                    pa.field('type', pa.string()),
                                    pa.field('status', pa.string()),
                                    pa.field('priority', pa.int8()),
                    ...
                    for i, field in enumerate(current_state.schema):
                        if field.name == 'tasks':
                            ...
                            try:
                                ...
                                logger.error(f"Error creating task array in assign_task: {task_err}")
                            ...
                            try:
                                ...
                                logger.error(f"Error creating node array in assign_task: {node_err}")
    ...
    def get_c_data_interface(self):
        ...
        metadata = {
            "plasma_socket": os.path.join(self.state_path, "plasma.sock"),  # Path to a dummy socket file for API compatibility
            "object_id": hashlib.md5(f"{self.cluster_id}_{self._state_version}_{time.time()}".encode()).hexdigest(),
            "schema": self.schema.to_string(),
            "version": self._state_version,
            "cluster_id": self.cluster_id,
            "updated_at": time.time(),
    ...
    def access_via_c_data_interface(state_path):
        ...
        # Check if metadata file exists for shared memory access
        metadata_path = os.path.join(state_path, 'state_metadata.json')
        
        # First try shared memory access for backward compatibility with tests
        # that mock this functionality
        if os.path.exists(metadata_path):
            try:
                with open(metadata_path, 'r') as f:
                    metadata = json.load(f)
                
                logger.info(f"Found metadata file with plasma socket: {metadata.get('plasma_socket')}")
                
                # Try to use plasma connect if it's available
                try:
        ...
        try:
            # Find parquet files in the state directory
            parquet_files = [f for f in os.listdir(state_path) if f.startswith('state_') and f.endswith('.parquet')]
            
            if not parquet_files:
                result["error"] = "No state files found"
                return result
            ...
            # Get cluster info from the table
            result["success"] = True
            result["table"] = state_table
            result["num_rows"] = state_table.num_rows
            
            # Extract more metadata if the table has rows
            if state_table.num_rows > 0:
                ...
                # Get node and task counts
                nodes_list = state_table.column("nodes")[0].as_py()
                tasks_list = state_table.column("tasks")[0].as_py()
                content_list = state_table.column("content")[0].as_py()
