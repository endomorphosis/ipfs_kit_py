"""Embeddings router for ipfs_kit_py.
...
- Provide a reliable local fallback (Gemini CLI -> HF transformers) via
  `ipfs_kit_py.utils.embedding_adapter`.
...
def _builtin_provider_by_name(name: str, *, deps: RouterDeps) -> Optional[EmbeddingsProvider]:
    ...
    if key in {"ipfs_peer", "ipfs_peers"}:
        return _get_ipfs_peer_provider(deps)
...
def _get_ipfs_peer_provider(deps: RouterDeps) -> Optional[EmbeddingsProvider]:
    """Get embeddings provider that multiplexes requests across IPFS peers."""
    ...
    class _IPFSPeerProvider:
        def embed_texts(
            ...
            **kwargs: object,
            ...
            # Use the IPFS backend to route embeddings requests to peers
            try:
...
def _get_local_adapter_provider(*, deps: Optional[RouterDeps] = None) -> EmbeddingsProvider:
    from ipfs_kit_py.utils.embedding_adapter import embed_texts as _embed_texts
