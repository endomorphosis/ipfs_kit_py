...
class ipfs_cluster_ctl:
    ...
    def run_cluster_command(self, cmd_args, check=True, timeout=30, correlation_id=None, shell=False):
        ...
        try:
            ...
            try:
                ...
                try:
                    ...
                    if not pin.startswith("Qm") and not pin.startswith("baf") and not is_valid_cid(pin):
            ...
            if unpin_result["success"]:
                ...
                if "not pinned" in stderr or "not found" in stderr:
                    ...
                    return handle_error(
                        result, 
                        IPFSError(f"Failed to remove cluster pin: {unpin_result.get('error', 'Unknown error')}"),
    ...
    def ipfs_cluster_ctl_add_pin_recursive(self, path=None, **kwargs):
        ...
        try:
            ...
            if pin_result["success"]:
                ...
                return handle_error(
                    result, 
                    IPFSError(f"Failed to add recursive pin: {pin_result.get('error', 'Unknown error')}"),
    ...
    def ipfs_cluster_ctl_execute(self, args=None, **kwargs):
        ...
        try:
            ...
            if args is None or not isinstance(args, list) or len(args) == 0:
                return handle_error(result, IPFSValidationError("Missing or invalid required parameter: args (must be a non-empty list)"))
            ...
            try:
                ...
                valid_commands = ["id", "peers", "add", "pin", "status", "recover", "version", "health", "ipfs"]
                
                if args[0] not in valid_commands:
                    return handle_error(result, IPFSValidationError(f"Invalid command: {args[0]}. Must be one of: {', '.join(valid_commands)}"))
                
                # Validate subcommands for specific commands
                if len(args) > 1:
                    if args[0] == "peers" and args[1] not in ["ls", "rm"]:
                        return handle_error(result, IPFSValidationError(f"Invalid peers subcommand: {args[1]}. Must be one of: ls, rm"))
                    
                    if args[0] == "pin" and args[1] not in ["add", "rm", "ls", "update"]:
                        return handle_error(result, IPFSValidationError(f"Invalid pin subcommand: {args[1]}. Must be one of: add, rm, ls, update"))
                    
                    if args[0] == "health" and args[1] not in ["graph", "metrics", "alerts"]:
                        return handle_error(result, IPFSValidationError(f"Invalid health subcommand: {args[1]}. Must be one of: graph, metrics, alerts"))
                    
                    if args[0] == "ipfs" and args[1] not in ["gc"]:
                        return handle_error(result, IPFSValidationError(f"Invalid ipfs subcommand: {args[1]}. Must be one of: gc"))
            except ImportError:
    ...
    def ipfs_cluster_get_pinset(self, **kwargs):
        ...
        try:
            ...
            if not cmd_result["success"]:
                ...
                return handle_error(
                    result, 
                    IPFSError(f"Failed to get cluster pinset: {cmd_result.get('error', 'Unknown error')}"),
                    {"cmd_result": cmd_result}
                )
    ...
    def ipfs_cluster_ctl_status(self, **kwargs):
        ...
        try:
            ...
            if not cmd_result["success"]:
                ...
                return handle_error(
                    result, 
                    IPFSError(f"Failed to get cluster status: {cmd_result.get('error', 'Unknown error')}"),
                    {"cmd_result": cmd_result}
                )
        ...
        try:
            ...
            if cmd_result["success"] and cmd_result.get("stdout", "").strip():
                ...
                try:
                    ...
                    version_result = self.run_cluster_command(version_cmd, correlation_id=correlation_id)
                ...
                result["success"] = True  # The test itself succeeded even if cluster-ctl isn't available
    ...
    def test(self, **kwargs):
        ...
        try:
            ...
            if binary_test.get("available", False):
                ...
                try:
                    peer_args = ["peers", "ls"]
                    peer_test = self.ipfs_cluster_ctl_execute(peer_args, correlation_id=correlation_id)
