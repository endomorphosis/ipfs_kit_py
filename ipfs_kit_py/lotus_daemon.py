...
class lotus_daemon:
    ...
    def run_command(self, cmd_args, check=True, timeout=30, correlation_id=None, shell=False):
        ...
        if not daemon_ready:
            try:
                ...
                if daemon_process.poll() is None:
                    ...
                    logger.info(f"Lotus daemon process started, waiting {extra_wait_time} seconds to verify stability")
                    ...
                    if daemon_process.poll() is None:
                        ...
                        result["message"] = "Lotus daemon started via direct invocation"
                        ...
                        error_msg = f"Lotus daemon exited shortly after startup: {stderr}"
                        logger.error(error_msg)
                        return handle_error(result, LotusError(error_msg))
                else:
                    ...
                    return handle_error(result, LotusError(f"Daemon failed to start: {stderr}"))
