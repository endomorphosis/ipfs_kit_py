...
class MetadataSyncHandler:
    ...
    def __init__(self, index, ipfs_client, cluster_id: str = None, node_id: str = None):
        ...
        self.node_id = node_id or (ipfs_client.get_node_id() if hasattr(ipfs_client, 'get_node_id') else None)
        ...
        logger.info(f"Initialized MetadataSyncHandler with node_id={self.node_id}, cluster_id={self.cluster_id}")
                                    ...
                                    if partitions:
                                        ...
                                        for partition_id, metadata in partitions.items():
                                            ...
                                            if partition_id not in self.index.partitions:
                                                ...
                                                logger.info(f"Downloading new partition {partition_id} from peer {peer}")
                                                self.index._download_partition(peer, partition_id, metadata)
                                            elif metadata.get("mtime", 0) > self.index.partitions[partition_id].get("mtime", 0):
                                                # Newer version, download it
                                                logger.info(f"Updating partition {partition_id} from peer {peer} (newer version)")
                                                self.index._download_partition(peer, partition_id, metadata)
