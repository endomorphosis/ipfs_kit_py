...
if FASTAPI_AVAILABLE:
    ...
    app.state.config = {
        ...
        "max_upload_size": int(os.environ.get("IPFS_KIT_MAX_UPLOAD_SIZE", 100 * 1024 * 1024)),  # 100MB
        "rate_limit_enabled": os.environ.get("IPFS_KIT_RATE_LIMIT_ENABLED", "false").lower() == "true",
...
if FASTAPI_AVAILABLE:
    ...
    class ModelMetadata(BaseModel):
        ...
        framework: Optional[str] = Field(None, description="Framework used (e.g., 'pytorch', 'tensorflow', 'sklearn')")
...
async def list_peers(verbose: bool = Query(False), latency: bool = Query(False), direction: bool = Query(False)):
    """
...
async def resolve_name(
    name: str,
    recursive: bool = Query(True, description="Whether to resolve until the result is not an IPNS name"),
...
async def list_names():
    ...
    try:
        ...
        try:
            ...
            elif hasattr(api.kit, "ipfs_key_list"):  # Fallback to key list if name list is not available
...
async def cluster_pin(
    ...
    try:
        ...
        if api.config.get("role") == "leecher":
            raise HTTPException(status_code=403, detail="Cluster operations not available in leecher role")
    ...
    async def ai_models_list(
        ...
        try:
            ...
            if has_arrow_index and (framework or parsed_tags or query):
                ...
                logger.info(f"Searching models with metadata index (framework: {framework}, tags: {parsed_tags}, query: {query})")
                ...
                if query:
                    ...
                    if filters:
                        ...
                        text_result = api.metadata_index.search_text(query, fields=["metadata.description", "metadata.title", "tags"])
    ...
    async def ai_datasets_list(
        ...
        try:
            ...
            if has_arrow_index and (format or parsed_tags or query):
                # Advanced search using Arrow metadata index
                logger.info(f"Searching datasets with metadata index (format: {format}, tags: {parsed_tags}, query: {query})")
...
if FASTAPI_AVAILABLE:
    ...
    async def upload_file(
        ...
        try:
            ...
            logger.info(f"Adding file {filename} to IPFS, size={len(content)}, pin={pin}, wrap={wrap_with_directory}")
