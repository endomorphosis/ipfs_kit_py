    ...
    def from_string(cls, role_str: str) -> 'NodeRole':
        ...
        for role in cls:
            if role.value == role_str:
                return role
        raise ValueError(f"Invalid role: {role_str}. Expected one of: {', '.join([r.value for r in cls])}")
...
role_capabilities = {
    ...
    NodeRole.GATEWAY: {
        ...
        "ipfs_config_overrides": {
            "Addresses": {
                "Gateway": "/ip4/0.0.0.0/tcp/8080"  # Expose gateway to all interfaces
            },
            "Gateway": {
                "HTTPHeaders": {
...
class RoleManager:
    ...
    def __init__(self, 
        ...
        self.logger.info(f"Initialized RoleManager with role: {self.current_role} for cluster: {self.cluster_id}")
    ...
    def _detect_optimal_role(self) -> NodeRole:
        ...
        for role, specs in role_capabilities.items():
            ...
            if (self.resources.get("memory_available_mb", 0) < required["min_memory_mb"] or
                self.resources.get("disk_available_gb", 0) < required["min_storage_gb"]):
            ...
            # Memory score (0-100)
            mem_ratio = min(self.resources.get("memory_available_mb", 0) / required["min_memory_mb"], 3)
            score += min(100, mem_ratio * 33)
            
            # Storage score (0-100)
            storage_ratio = min(self.resources.get("disk_available_gb", 0) / required["min_storage_gb"], 3)
            ...
            if "network_max_speed_mbps" in self.resources:
                net_ratio = min(self.resources.get("network_max_speed_mbps", 0) / required["min_bandwidth_mbps"], 3)
            ...
            if "uptime_hours" in self.resources and required["min_uptime_hours"] > 0:
                uptime_ratio = min(self.resources.get("uptime_hours", 0) / required["min_uptime_hours"], 3)
    ...
    def _check_and_switch_role_if_needed(self):
        ...
        if self.adaptation_enabled and current_time - self.last_adaptation > self.adaptation_cooldown:
        ...
        mem_diff = (self.resources.get("memory_available_mb", 0) - current_specs["min_memory_mb"]) / current_specs["min_memory_mb"]
        storage_diff = (self.resources.get("disk_available_gb", 0) - current_specs["min_storage_gb"]) / current_specs["min_storage_gb"]
        ...
        significant_change = (abs(mem_diff) > self.role_stability_threshold or 
                              abs(storage_diff) > self.role_stability_threshold)
    ...
    def _update_resource_history(self):
        ...
        self.resource_history["memory_available_mb"].append(self.resources.get("memory_available_mb", 0))
        self.resource_history["disk_available_gb"].append(self.resources.get("disk_available_gb", 0))
    ...
    def _adapt_to_resource_changes(self) -> bool:
        ...
        if not self.adaptation_enabled or current_time - self.last_adaptation < self.adaptation_cooldown:
            return False
    ...
    def switch_role(self, new_role: NodeRole) -> bool:
        ...
        try:
            ...
            self.metrics["last_switch_reason"] = f"Resource-based switch from {old_role} to {new_role}"
    ...
    def _on_role_changed(self, old_role: NodeRole, new_role: NodeRole):
        ...
        # If switching from master, need to register with a different master
            ...
            self.registered_with_master = False
            self.logger.info("Node is no longer a master node, will need to register with a new master")
    ...
    def _adjust_connection_limits(self, min_connections: int, max_connections: int):
        ...
        self.logger.debug(f"Adjusting connection limits: min={min_connections}, max={max_connections}")
        # In a real implementation, this would modify the IPFS config or call APIs
        
        # Store in metrics for tracking
        self.metrics["connection_limits"] = {
            "min": min_connections,
            "max": max_connections
        }
    ...
    def _discover_peers_via_master(self) -> List[Dict[str, Any]]:
        ...
        Returns:
            List of peer information dictionaries
        """
        self.logger.debug(f"Attempting to discover peers via master node at {self.master_node_address}")
    ...
    def join_cluster(self, cluster_id: str, master_address: str) -> bool:
        ...
        if not registration_result:
            self.logger.warning(f"Failed to register with master at {master_address}, will retry later")
    ...
    def get_cluster_status(self) -> Dict[str, Any]:
        ...
        if hasattr(self, "cluster_peers"):
            ...
            status["peers"] = [
                {"node_id": peer.get("node_id"), "role": peer.get("role")}
                for peer in peers
            ]
