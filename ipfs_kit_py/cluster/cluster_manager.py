...
class ClusterManager:
    ...
    def __init__(self,
        ...
        logger.info(f"Cluster manager initialized with role {self.initial_role} for cluster {self.cluster_id}")
    ...
    def stop(self) -> Dict[str, Any]:
        ...
        self.metrics["uptime"] = time.time() - self.metrics["start_time"] if self.metrics["start_time"] else 0
        
        logger.info(f"Cluster manager stopped with result: {result}")
        return result
    
    def submit_task(self, task_type: str, payload: Dict[str, Any], 
                   priority: int = 1, timeout: Optional[int] = None) -> Dict[str, Any]:
        """
    ...
    def get_node_roles(self) -> Dict[str, str]:
        ...
        members = self.membership_manager.get_members()
        return {node_id: info.get("role", "unknown") 
                for node_id, info in members.items()}
    
    def register_task_handler(self, task_type: str, 
                             handler_func: Callable[[Dict[str, Any]], Dict[str, Any]]) -> bool:
        """
    ...
    def get_cluster_metrics(self, include_members: bool = True, 
        ...
        try:
            ...
            if include_members and hasattr(self, 'membership_manager'):
                ...
                # Count nodes by role
                roles = set([member_info.get("role", "unknown") for _, member_info in members.items()])
                roles.add(self.current_role)  # Add self
                
                for role in roles:
                    count = len([1 for _, member_info in members.items() 
                ...
                for _, member_info in members.items():
                    ...
                    aggregate["total_resources"]["memory_gb"] += resources.get("memory_total", 0) / (1024 * 1024 * 1024)
                    aggregate["total_resources"]["disk_gb"] += resources.get("disk_total", 0) / (1024 * 1024 * 1024)
                    aggregate["total_resources"]["gpu_count"] += resources.get("gpu_count", 0)
                
                # Add self resources
                if "node_metrics" in result:
                    aggregate["total_resources"]["cpu_cores"] += result["node_metrics"].get("cpu_count", 0)
                    aggregate["total_resources"]["memory_gb"] += result["node_metrics"].get("memory_total", 0) / (1024 * 1024 * 1024)
                    aggregate["total_resources"]["disk_gb"] += result["node_metrics"].get("disk_total", 0) / (1024 * 1024 * 1024)
                    aggregate["total_resources"]["gpu_count"] += result["node_metrics"].get("gpu_count", 0)
    ...
    def _handle_membership_change(self, change_type: str, node_id: str, 
    ...
    def _handle_alert(self, alert_type: str, alert_data: Dict[str, Any]) -> None:
        ...
        if severity == "critical":
            logger.critical(f"CRITICAL ALERT from {node_id}: {alert_type} - {alert_data.get('message', '')}")
        elif severity == "error":
            logger.error(f"ERROR ALERT from {node_id}: {alert_type} - {alert_data.get('message', '')}")
        elif severity == "warning":
            logger.warning(f"WARNING ALERT from {node_id}: {alert_type} - {alert_data.get('message', '')}")
        else:
            logger.info(f"INFO ALERT from {node_id}: {alert_type} - {alert_data.get('message', '')}")
