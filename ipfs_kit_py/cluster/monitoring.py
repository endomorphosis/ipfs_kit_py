...
class MetricsCollector:
    ...
    def _update_time_series(self):
        ...
        try:
            ...
            if "resources" in self.current_metrics:
                ...
                self.time_series["node"]["memory_percent"].append(resources.get("memory_percent", 0))
            ...
            if "content" in self.current_metrics:
                ...
                self.time_series["content"]["retrieved_count"].append(content.get("retrieved_count", 0))
                self.time_series["content"]["pinned_count"].append(content.get("pinned_count", 0))
                self.time_series["content"]["total_size_bytes"].append(content.get("total_size_bytes", 0))
                
            # Network metrics
            if "network" in self.current_metrics:
                network = self.current_metrics["network"]
                self.time_series["network"]["bandwidth_in_bytes"].append(network.get("bandwidth_in_bytes", 0))
                self.time_series["network"]["bandwidth_out_bytes"].append(network.get("bandwidth_out_bytes", 0))
                self.time_series["network"]["peer_connections"].append(network.get("peer_connections", 0))
                self.time_series["network"]["content_requests"].append(network.get("content_requests", 0))
    ...
    def get_historical_metrics(self, timeframe: str = "1h") -> Dict[str, Any]:
        ...
        result = {
            "timestamps": list(self.time_series["timestamps"])[-points:],
            "metrics": {}
        }
...
class ClusterMonitor:
    ...
    def get_alerts(self, count: Optional[int] = None, level: Optional[str] = None) -> List[Dict[str, Any]]:
        """
        Get active alerts.
    ...
    def get_alert_history(self, count: Optional[int] = None, level: Optional[str] = None) -> List[Dict[str, Any]]:
        """
        Get alert history.
