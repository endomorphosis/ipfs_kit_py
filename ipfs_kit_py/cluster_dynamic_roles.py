...
class ClusterDynamicRoles:
    ...
    def detect_available_resources(self) -> Dict[str, Any]:
        ...
        try:
            ...
            # Disk space
            disk = psutil.disk_usage(os.path.expanduser("~/.ipfs") if os.path.exists(os.path.expanduser("~/.ipfs")) else "/")
    ...
    def detect_resource_changes(self) -> Dict[str, Any]:
        ...
        try:
            ...
            for resource_type in ["memory_available", "disk_available", "cpu_available", "bandwidth_available"]:
            ...
            if "network_stability" in previous_resources and "network_stability" in current_resources:
    ...
    def determine_optimal_role(self) -> Dict[str, Any]:
        ...
        result = {
            "success": True,
            "operation": "determine_optimal_role",
            "timestamp": time.time()
        }
        ...
        try:
            ...
            if current_role in role_evaluation and role_evaluation[current_role]["capable"]:
                ...
                network_stability = self.last_resources.get("network_stability", 0) if self.last_resources else 0
                resources_stable = network_stability >= 0.8 and len(self.resources_history) >= 3
                cooldown_passed = (time.time() - self.last_transition_time) > self.config["transition_cooldown"]
                    ...
                    for next_role in role_order[current_index+1:]:
                        if role_evaluation[next_role]["capable"]:
                            return {
                                ...
                                "reason": (f"Node has sufficient resources for {next_role} role "
            ...
            if not best_role:
                ...
                best_role = "leecher"
                action = "downgrade" if role_order.index(current_role) > role_order.index(best_role) else "maintain"
    ...
    def upgrade_to_worker(self, master_address: str, cluster_secret: str, config_overrides: Dict = None) -> Dict[str, Any]:
        ...
        try:
            ...
            if hasattr(self.kit, 'create_cluster_follow_service'):
                ...
                if not follow_result.get("success", False):
                    raise Exception(f"Failed to initialize cluster follow service: {follow_result.get('error', 'Unknown error')}")
            ...
            if hasattr(self.kit, 'ipfs_start'):
                ...
                if not start_result.get("success", False):
                    raise Exception(f"Failed to restart IPFS daemon: {start_result.get('error', 'Unknown error')}")
            ...
            if hasattr(self.kit, 'cluster_follow_join'):
                ...
                if not join_result.get("success", False):
                    raise Exception(f"Failed to join cluster: {join_result.get('error', 'Unknown error')}")
            ...
            # Attempt to recover
            if hasattr(self.kit, 'ipfs_start') and "Restarted IPFS daemon" not in result["actions_performed"]:
                try:
    ...
    def upgrade_to_master(self, cluster_secret: str, config_overrides: Dict = None) -> Dict[str, Any]:
        ...
        try:
            ...
            master_config = {
                ...
                "Pinning": {
                    "RemoteServices": {}  # Will be filled in with actual services
                },
                "Cluster": {
                    "PeerAddresses": [],
                    "ReplicationFactor": 3,
            ...
            if hasattr(self.kit, 'create_cluster_service'):
                ...
                if not service_result.get("success", False):
                    raise Exception(f"Failed to initialize cluster service: {service_result.get('error', 'Unknown error')}")
            ...
            if hasattr(self.kit, 'create_cluster_ctl'):
                ...
                if not ctl_result.get("success", False):
                    raise Exception(f"Failed to initialize cluster control interface: {ctl_result.get('error', 'Unknown error')}")
            ...
            if hasattr(self.kit, 'cluster_service_start'):
                ...
                if not start_result.get("success", False):
                    raise Exception(f"Failed to start cluster service: {start_result.get('error', 'Unknown error')}")
    ...
    def downgrade_to_worker(self, master_address: str = None, cluster_secret: str = None) -> Dict[str, Any]:
        ...
        try:
            ...
            if master_address and cluster_secret and hasattr(self.kit, 'create_cluster_follow_service'):
                follow_result = self.kit.create_cluster_follow_service(
                    master_address=master_address,
                    cluster_secret=cluster_secret
                )
    ...
    def downgrade_to_leecher(self) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.info(f"Successfully downgraded node from {result['previous_role']} to leecher role")
    ...
    def check_and_update_role(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if (time.time() - self.last_transition_time) < self.config.get("transition_cooldown", 1800):
                result.update({
                    "success": True,
                    "role_change_needed": False,
            ...
            if action == "upgrade":
                ...
                elif (self.current_role == "leecher" or self.current_role == "worker") and optimal_role == "master":
                    # Need cluster secret
            ...
            # Need to downgrade
                ...
                elif (self.current_role == "master" or self.current_role == "worker") and optimal_role == "leecher":
                    # Perform downgrade
    ...
    def change_role(self, target_role: str, force: bool = False, 
                    master_address: str = None, cluster_secret: str = None, 
        ...
        try:
            ...
            if not force:
                ...
                if target_role not in role_evals or not role_evals[target_role].get("capable", False):
                    limiting_factor = role_evals[target_role]["limiting_factor"] if target_role in role_evals else "unknown"
                    capability_pct = role_evals[target_role]["capability_percent"] if target_role in role_evals else 0
