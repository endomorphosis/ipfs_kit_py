...
class S3ModelAnyIO(BaseStorageModel):
    ...
    async def upload_file_async(self, file_path: str, bucket: str, key: str, metadata: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
    ...
    def list_objects(self, bucket: str, prefix: Optional[str] = None) -> Dict[str, Any]:
        ...
        if backend:
            # We're in an async context, but this is a sync method
            logger.warning(f"Called sync list_objects() in async context ({backend}). Consider using list_objects_async() instead")
    ...
    def delete_object(self, bucket: str, key: str) -> Dict[str, Any]:
        ...
        if backend:
            # We're in an async context, but this is a sync method
            logger.warning(f"Called sync delete_object() in async context ({backend}). Consider using delete_object_async() instead")
    ...
    async def ipfs_to_s3_async(self, cid: str, bucket: str, key: Optional[str] = None, pin: bool = True) -> Dict[str, Any]:
    ...
    def ipfs_to_s3(self, cid: str, bucket: str, key: Optional[str] = None, pin: bool = True) -> Dict[str, Any]:
        ...
        if backend:
            # We're in an async context, but this is a sync method
            logger.warning(f"Called sync ipfs_to_s3() in async context ({backend}). Consider using ipfs_to_s3_async() instead")
        ...
        try:
            ...
            try:
                ...
                if hasattr(self.ipfs_model, "add_content_async") and callable(getattr(self.ipfs_model, "add_content_async")):
                    # Use async version if available
                    ipfs_result = await self.ipfs_model.add_content_async(content, filename=os.path.basename(key))
                else:
                    # Fall back to sync version
                    ipfs_result = await anyio.to_thread.run_sync(
    ...
    def s3_to_ipfs(self, bucket: str, key: str, pin: bool = True) -> Dict[str, Any]:
        ...
        if backend:
            # We're in an async context, but this is a sync method
            logger.warning(f"Called sync s3_to_ipfs() in async context ({backend}). Consider using s3_to_ipfs_async() instead")
