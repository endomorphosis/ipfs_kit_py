...
class StorachaModelAnyIO(BaseStorageModel):
    ...
    async def upload_file_async(self, file_path: str, space_did: Optional[str] = None) -> Dict[str, Any]:
    ...
    async def upload_car_async(self, car_path: str, space_did: Optional[str] = None) -> Dict[str, Any]:
    ...
    async def list_uploads_async(self, space_did: Optional[str] = None) -> Dict[str, Any]:
        ...
        try:
            ...
            if self.storacha_kit:
                list_result = await anyio.to_thread.run_sync(
                    lambda: self.storacha_kit.w3_list()
                )
    ...
    async def delete_upload_async(self, cid: str, space_did: Optional[str] = None) -> Dict[str, Any]:
    ...
    async def ipfs_to_storacha_async(self, cid: str, space_did: Optional[str] = None, pin: bool = True) -> Dict[str, Any]:
    ...
    async def storacha_to_ipfs_async(self, cid: str, space_did: Optional[str] = None, pin: bool = True) -> Dict[str, Any]:
        ...
        try:
            ...
            try:
                ...
                if hasattr(self.ipfs_model, "add_content_async") and callable(getattr(self.ipfs_model, "add_content_async")):
                    # Use async version if available
                    ipfs_result = await self.ipfs_model.add_content_async(content, filename=os.path.basename(temp_path))
                else:
                    # Fall back to sync version
                    ipfs_result = await anyio.to_thread.run_sync(
                        lambda: self.ipfs_model.add_content(content, filename=os.path.basename(temp_path))
                    )
