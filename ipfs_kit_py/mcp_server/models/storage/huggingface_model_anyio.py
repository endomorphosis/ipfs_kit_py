...
class HuggingFaceModelAnyIO(BaseStorageModel):
    ...
    async def authenticate_async(self, token: str) -> Dict[str, Any]:
        ...
        try:
            ...
            if self.hf_kit:
                ...
                auth_result = await anyio.to_thread.run_sync(
                    lambda: self.hf_kit.login(token=token)
                )
    ...
    def authenticate(self, token: str) -> Dict[str, Any]:
        ...
        if backend:
            ...
            logger.warning(f"Called sync authenticate() in async context ({backend}). Consider using authenticate_async() instead")
    ...
    async def create_repository_async(self, repo_id: str, repo_type: str = "model", private: bool = False) -> Dict[str, Any]:
    ...
    def create_repository(self, repo_id: str, repo_type: str = "model", private: bool = False) -> Dict[str, Any]:
        ...
        if backend:
            # We're in an async context, but this is a sync method
            logger.warning(f"Called sync create_repository() in async context ({backend}). Consider using create_repository_async() instead")
    ...
    async def upload_file_async(self, file_path: str, repo_id: str, path_in_repo: Optional[str] = None, 
    ...
    def upload_file(self, file_path: str, repo_id: str, path_in_repo: Optional[str] = None, 
        ...
        if backend:
            # We're in an async context, but this is a sync method
            logger.warning(f"Called sync upload_file() in async context ({backend}). Consider using upload_file_async() instead")
    ...
    async def download_file_async(self, repo_id: str, filename: str, destination: str, 
        ...
        try:
            ...
            if self.hf_kit:
                ...
                if download_result.get("success", False):
                    # Get file size for statistics
                    file_exists = await anyio.to_thread.run_sync(lambda: os.path.exists(destination))
                    file_size = 0
                    if file_exists:
                        file_size = await anyio.to_thread.run_sync(lambda: os.path.getsize(destination))
    ...
    def download_file(self, repo_id: str, filename: str, destination: str, 
        ...
        if backend:
            # We're in an async context, but this is a sync method
            logger.warning(f"Called sync download_file() in async context ({backend}). Consider using download_file_async() instead")
    ...
    async def list_models_async(self, author: Optional[str] = None, search: Optional[str] = None,
    ...
    def list_models(self, author: Optional[str] = None, search: Optional[str] = None,
        ...
        if backend:
            # We're in an async context, but this is a sync method
            logger.warning(f"Called sync list_models() in async context ({backend}). Consider using list_models_async() instead")
    ...
    async def ipfs_to_huggingface_async(self, cid: str, repo_id: str, path_in_repo: Optional[str] = None,
    ...
    def ipfs_to_huggingface(self, cid: str, repo_id: str, path_in_repo: Optional[str] = None,
        ...
        if backend:
            # We're in an async context, but this is a sync method
            logger.warning(f"Called sync ipfs_to_huggingface() in async context ({backend}). Consider using ipfs_to_huggingface_async() instead")
    ...
    async def huggingface_to_ipfs_async(self, repo_id: str, filename: str, pin: bool = True,
                                 revision: Optional[str] = None, repo_type: str = "model") -> Dict[str, Any]:
        ...
        try:
            ...
            try:
                ...
                if hasattr(self.ipfs_model, "add_content_async") and callable(getattr(self.ipfs_model, "add_content_async")):
                    # Use async version if available
                    ipfs_result = await self.ipfs_model.add_content_async(content, filename=filename)
                else:
                    # Fall back to sync version
                    ipfs_result = await anyio.to_thread.run_sync(
    ...
    def huggingface_to_ipfs(self, repo_id: str, filename: str, pin: bool = True,
        ...
        if backend:
            # We're in an async context, but this is a sync method
            logger.warning(f"Called sync huggingface_to_ipfs() in async context ({backend}). Consider using huggingface_to_ipfs_async() instead")
