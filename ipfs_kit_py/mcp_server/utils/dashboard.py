...
class MCPDashboard:
    ...
    def _draw_dashboard(self):
        ...
        with self.lock:
            ...
            server_status = f"{status_color}{self.server_status}{COLORS['reset']}" if self.use_colors else self.server_status
            ...
            files_section = self._create_box("RECENT FILES", files_lines, width=self.term_width // 2 - 3)
            ...
            for i, error in enumerate(self.recent_errors[:self.max_errors_to_show]):
                ...
                if self.use_colors:
                    status = f"{COLORS['green']}FIXED{COLORS['reset']}" if error['fixed'] else f"{COLORS['red']}ERROR{COLORS['reset']}"
            ...
            errors_section = self._create_box("RECENT ERRORS", errors_lines, width=self.term_width // 2 - 3)
    ...
    def _create_box(self, title: str, content_lines: List[str], width: int = 50) -> List[str]:
        ...
        if title:
            ...
            right_border = BOX_CHARS['h'] * (width - len(left_border) - len(title_str) - 1) + BOX_CHARS['tr']
