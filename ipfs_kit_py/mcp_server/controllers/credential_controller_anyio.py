...
from ipfs_kit_py.mcp.controllers.credential_controller import (
    CredentialBaseRequest, GenericCredentialRequest, S3CredentialRequest, 
    StorachaCredentialRequest, FilecoinCredentialRequest, IPFSCredentialRequest,
    CredentialResponse, CredentialInfoResponse
)
...
class CredentialControllerAnyIO:
    ...
    async def add_generic_credential(self, credential_request: GenericCredentialRequest):
        ...
        supported_services = ["ipfs", "s3", "storacha", "filecoin", "huggingface", "lassie"]
        if credential_request.service not in supported_services:
            raise HTTPException(
                status_code=400,
                detail=f"Unsupported service: {credential_request.service}. Supported services: {', '.join(supported_services)}"
            )
        ...
        try:
            ...
            if hasattr(self.credential_manager.add_credential, "__await__"):
                ...
                result = await self.credential_manager.add_credential(
    ...
    async def add_ipfs_credentials(self, credential_request: IPFSCredentialRequest):
        ...
        if not values:
            raise HTTPException(
                status_code=400,
                detail="At least one credential field (identity, api_address, cluster_secret) must be provided"
            )
    ...
    async def remove_credential(self, service: str, name: str):
        ...
        try:
            ...
            if hasattr(self.credential_manager.remove_credential, "__await__"):
                # Method is already async
                result = await self.credential_manager.remove_credential(
                    service=service,
                    name=name
                )
            else:
