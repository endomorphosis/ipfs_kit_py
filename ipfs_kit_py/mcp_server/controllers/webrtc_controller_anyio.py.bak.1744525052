...
class WebRTCControllerAnyIO:
                        ...
                        for server_id, server_info in list(self.active_streaming_servers.items()):
                            try:
                                ...
                                if current_time - started_at > max_lifetime:
                                    logger.info(f"Cleaning up stale streaming server {server_id} (age: {(current_time - started_at)/60:.1f} minutes)")
                                    ...
                                    try:
                                        ...
                                        logger.error(f"Error stopping stale server {server_id}: {e}")
    ...
    async def shutdown(self):
        ...
        try:
            ...
            connections_result = await anyio.to_thread.run_sync(self.ipfs_model.list_webrtc_connections)
        ...
        try:
            ...
            if isinstance(result, dict) and not result.get("success", False):
                ...
                logger.info(f"Successfully closed all WebRTC connections: {result.get('connections_closed', 0)} closed")
        except Exception as e:
    ...
    def get_resource_stats(self):
        ...
        try:
            ...
            for server_id, server_info in self.active_streaming_servers.items():
                ...
                resource_factor = min(1.0, server_info.get("resources", {}).get("usage", 0.0) / 0.7)  # Max at 70% usage
