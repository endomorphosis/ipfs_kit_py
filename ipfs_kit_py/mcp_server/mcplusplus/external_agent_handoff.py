...
def bind_handoff_operation(
    ...
    *,
    ...
    for name, ident in dict(profiles or {}).items():
        ...
        identifier = str(ident).strip().lower()
        if identifier in {"profile-i", "profile-j", "external-agent"}:
            raise ExternalAgentHandoffBindError(
                "new MCP++ profiles are not admitted",
                reason_code="new_profile_forbidden",
            )
        if identifier not in ADMITTED_PROFILES:
            raise ExternalAgentHandoffBindError(
                f"unknown MCP++ profile identifier: {ident}",
                reason_code="unknown_profile",
            )
        bound[key] = identifier
    normalized_effects = tuple(
        str(effect).strip().lower().replace("-", "_") for effect in effects
    )
    forbidden = [
        effect for effect in normalized_effects if effect in STORAGE_AUTHORITY_EFFECTS
    ]
    if forbidden:
        raise ExternalAgentHandoffBindError(
    ...
    method = str(runtime_method or "").strip().lower()
    if method and (method not in HANDOFF_OPERATIONS or method != op):
        raise ExternalAgentHandoffBindError(
            "runtime method must match the bound handoff operation",
            reason_code="runtime_method_mismatch",
        )
