...
from ipfs_kit_py.error import (
    IPFSError, IPFSConnectionError, IPFSTimeoutError, IPFSValidationError, 
    IPFSConfigurationError, create_result_dict, handle_error
)
...
DEFAULT_ROLE_CAPABILITIES = {
    "master": [
        "pin", "unpin", "pin_ls",
        "peer_add", "peer_rm", "peer_ls",
        "config_get", "config_set",
        "add_content", "get_content",
        "role_get", "role_set",
        "metrics",
        "manage_peers", "manage_pins", "manage_config", "manage_roles",
        "admin"
    ],
    "worker": [
        "pin", "unpin", "pin_ls",
...
class ClusterAuthManager:
    ...
    def __init__(self, 
        ...
        self.security_dir = security_dir or os.path.expanduser(f"~/.ipfs/cluster_security/{cluster_id}")
    ...
    def _initialize_master_security(self):
        ...
        if os.path.exists(node_cert_path) and os.path.exists(node_key_path):
            ...
            self.node_cert, self.node_key = self._load_certificate_and_key(node_cert_path, node_key_path)
            ...
            self._save_certificate_and_key(self.node_cert, self.node_key, node_cert_path, node_key_path)
    ...
    def _generate_ca_certificate(self):
        ...
        try:
            ...
            cert = x509.CertificateBuilder().subject_name(
                ...
                datetime.datetime.utcnow() + datetime.timedelta(days=self.config.get("cert_expiration_days", 365) * 2)
            ).add_extension(
    ...
    def _generate_node_certificate(self):
        ...
        try:
            ...
            subject = x509.Name([
                ...
                x509.NameAttribute(NameOID.ORGANIZATION_NAME, "IPFS Cluster"),
                x509.NameAttribute(NameOID.ORGANIZATIONAL_UNIT_NAME, f"Cluster {self.cluster_id}"),
            ])
            ...
            cert = x509.CertificateBuilder().subject_name(
                ...
                datetime.datetime.utcnow() + datetime.timedelta(days=self.config.get("cert_expiration_days", 365))
            ).add_extension(
            ...
            with self._lock:
                ...
                if self.role == "master":
                    ...
                    if not self.ca_cert or not self.ca_key:
                        result.update({
                            "success": False,
                            "error": "No CA certificate available for signing"
                        })
                        return result
                    ...
                    result.update({
                        "success": False,
                        "error": f"Unknown role: {self.role}"
                    })
            ...
            with self._lock:
                ...
                if self.role == "master":
                    ...
                    if not node_cert or not node_key:
                        result.update({
                            "success": False,
                            "error": "Failed to generate node certificate"
                        })
                        return result
                    ...
                    node_cert_path = os.path.join(self.node_dir, 'node.crt')
                    node_key_path = os.path.join(self.node_dir, 'node.key')
                    self._save_certificate_and_key(node_cert, node_key, node_cert_path, node_key_path)
                    ...
                    # Format results
                    ca_cert_pem = ca_cert.public_bytes(encoding=serialization.Encoding.PEM).decode('utf-8')
                    node_cert_pem = node_cert.public_bytes(encoding=serialization.Encoding.PEM).decode('utf-8')
                    ...
                    result.update({
                        "success": False,
                        "error": "Only master nodes can generate certificates"
                    })
        ...
        try:
            with self._lock:
                ...
                if 'client_cert' not in certificates or 'client_key' not in certificates:
                    result.update({
                        "success": False,
                        "error": "Missing client certificate or key"
                    })
                    return result
                ...
                try:
                    ca_cert = x509.load_pem_x509_certificate(
                        certificates['ca_cert'].encode('utf-8'),
                        default_backend()
                    )
                    
                    client_cert = x509.load_pem_x509_certificate(
                        certificates['client_cert'].encode('utf-8'),
                        default_backend()
                    )
                    ...
                    result.update({
                        "success": False,
                        "error": f"Failed to parse certificates: {str(e)}"
                    })
                    return result
    ...
    def generate_ucan_token(self, audience, capabilities=None, expiration=86400):
        ...
        try:
            with self._lock:
                ...
                token.add_signature(issuer_key, None, {
                    "alg": "EdDSA",
                    "typ": "JWT",
                    "ucan": "1"
                })
        ...
        try:
            with self._lock:
                ...
                if peer_id not in self.peers or not self.peers[peer_id].get('secure_connection'):
                    result.update({
                        "success": False,
                        "error": f"No secure connection to peer {peer_id}"
                    })
                    return result
            ...
            with self._lock:
                ...
                if not capability_result["success"]:
                    ...
                    result.update({
                        ...
                        "operation": operation,
                        "authorized": False,
                        "error": capability_result.get("error", "Error during capability verification")
                    })
                    return result
