"""LLM router for ipfs_kit_py.

This module provides a reusable top-level entrypoint for text generation,
integrated with IPFS Kit's endpoint multiplexing capabilities.
...
- Integrate with ipfs_kit_py endpoint multiplexing for peer-to-peer LLM access.

Environment variables (compatible with ipfs_datasets_py):
- `IPFS_KIT_LLM_PROVIDER` or `IPFS_DATASETS_PY_LLM_PROVIDER`: force provider name
- `IPFS_KIT_ENABLE_IPFS_ACCELERATE` or `IPFS_DATASETS_PY_ENABLE_IPFS_ACCELERATE`: enable accelerate
- `IPFS_KIT_LLM_MODEL` or `IPFS_DATASETS_PY_LLM_MODEL`: default HF model name
...
- `gemini_py`: Python wrapper in `ipfs_kit_py.utils.gemini_cli.GeminiCLI`
- `claude_code`: Claude Code CLI command
    - `IPFS_DATASETS_PY_CLAUDE_CODE_CLI_CMD` (supports `{prompt}` placeholder)
- `claude_py`: Python wrapper in `ipfs_kit_py.utils.claude_cli.ClaudeCLI`
"""
