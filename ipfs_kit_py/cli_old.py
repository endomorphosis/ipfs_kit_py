...
def main(args: Optional[List[str]] = None) -> None:
    ...
    """
    try:
        # Quick check for --help or version to avoid loading everything
        ...
        # If the command is --help, just print help and exit
        if parsed_args.command is None:
            # This is a bit of a hack, but it's the easiest way to
            # get the help message to print without triggering the
        ...
        # Combine config, arguments, and parameters
        kwargs = {}
        if parsed_args.param:
            for p in parsed_args.param:
                kwargs.update(parse_key_value(p))
    ...
    except (IPFSError, IPFSValidationError, ValueError) as e:
...
def parse_args(args: Optional[List[str]] = None) -> argparse.Namespace:
    ...
    # Command registration mapping - lazy loading
    COMMAND_REGISTRY = {
        "wal": "ipfs_kit_py.wal_cli_integration.register_wal_commands",
        "fs-journal": "ipfs_kit_py.fs_journal_cli.register_fs_journal_commands",
        "bucket": "ipfs_kit_py.bucket_vfs_cli.register_bucket_commands",
        "vfs-version": "ipfs_kit_py.vfs_version_cli.create_vfs_version_parser",
        "wal-telemetry": "ipfs_kit_py.wal_telemetry_cli.register_wal_telemetry_commands",
        "parallel-query": "ipfs_kit_py.cli.add_parallel_query_commands",
    ...
    for command in LIGHTWEIGHT_COMMANDS:
        if command in COMMAND_REGISTRY:
            ...
            try:
                ...
                registration_function(subparsers)
    ...
    for command in HEAVY_COMMANDS:
        # Create a minimal parser that will trigger lazy loading
    ...
    state_parser = subparsers.add_parser(
        "state",
        help="Get program state information",
    )
    state_parser.add_argument(
        "--summary",
        action="store_true",
        help="Show overall state summary (default)",
    )
    state_parser.add_argument(
        "--system",
        action="store_true",
    ...
    mcp_master_parser = mcp_subparsers.add_parser(
        "master",
        help="Start the MCP server in master role",
    )
    mcp_master_parser.add_argument(
        "--host",
        default="127.0.0.1",
        help="Host to bind server",
    )
    ...
    mcp_worker_parser = mcp_subparsers.add_parser(
        "worker",
        help="Start the MCP server in worker role",
    )
    mcp_worker_parser.add_argument(
        "--host",
        default="127.0.0.1",
        help="Host to bind server",
    )
    ...
    mcp_leecher_parser = mcp_subparsers.add_parser(
        "leecher",
        help="Start the MCP server in leecher role",
    )
    mcp_leecher_parser.add_argument(
        "--host",
        default="127.0.0.1",
        help="Host to bind server",
    )
...
def _handle_lazy_command(api, args, kwargs, command):
    """
    Handle lazy loading of heavy CLI commands.
    Only imports and registers the command when it's actually invoked.
    """
    try:
        # Define command registry locally if not available
        COMMAND_REGISTRY = {
            "wal": "ipfs_kit_py.wal_cli_integration.register_wal_commands",
            "parallel-query": "ipfs_kit_py.cli.add_parallel_query_commands",
            "dashboard": "ipfs_kit_py.cli.add_dashboard_commands",
...
def parse_kwargs(args: argparse.Namespace) -> Dict[str, Any]:
    ...
    for key, value in args_dict.items():
        ...
        if key not in ['config', 'verbose', 'param', 'format', 'no_color', 'command', 'func'] and not key.startswith('timeout_'):
            ...
            # Check if it's a boolean value and add it if not already present
...
def main(argv: Optional[List[str]] = None) -> None:
    ...
    # Initialize API with config if provided (for non-state commands)
    try:
        from .high_level_api import IPFSSimpleAPI
