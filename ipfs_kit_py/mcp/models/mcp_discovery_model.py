...
class MCPDiscoveryModel:
    ...
    def __init__(
        ...
        logger.info(f"MCP Discovery Model initialized with ID {self.server_id} and role {self.role}")
    ...
    def _detect_available_features(self) -> List[str]:
        ...
        try:
            if HAS_LIBP2P and self.libp2p_model and hasattr(self.libp2p_model, 'is_available'):
        ...
        if self.ipfs_model:
            ...
            try:
                if hasattr(self.ipfs_model, 'ipfs_cluster_service') or hasattr(self.ipfs_model, 'ipfs_cluster_follow'):
    ...
    def announce_server(self) -> Dict[str, Any]:
        ...
        if getattr(self, 'has_libp2p', False) and self.libp2p_model:
            try:
                ...
                if (hasattr(self.libp2p_model, 'libp2p_peer') and 
                    self.libp2p_model.libp2p_peer and 
                    hasattr(self.libp2p_model.libp2p_peer, 'pubsub_publish')):
    ...
    def discover_servers(self, 
        ...
        if "libp2p" in methods and getattr(self, 'has_libp2p', False) and self.libp2p_model:
            try:
                ...
                if hasattr(self.libp2p_model, 'discover_peers'):
                    peers_result = self.libp2p_model.discover_peers(discovery_method="all", limit=100)
        ...
        if "websocket" in methods and self.has_websocket:
            try:
                ...
                # would depend on your WebSocket client implementation
                # For now, we'll just log it
                logger.debug(f"Would discover servers via WebSocket")
            except Exception as e:
    ...
    def check_server_health(self, server_id: str) -> Dict[str, Any]:
        ...
        if server_info.api_endpoint:
            try:
                ...
                is_healthy = time_since_last_seen < 300  # Consider healthy if seen in last 5 minutes
        ...
        if (not health_checked and server_info.libp2p_peer_id and 
            getattr(self, 'has_libp2p', False) and self.libp2p_model):
            try:
        ...
        if not health_checked:
            with self.server_lock:
                if server_id in self.known_servers:
                    result["healthy"] = self.known_servers[server_id].health_status.get("healthy", False)
                    result["health_source"] = "last_known"
                    result["last_checked"] = self.known_servers[server_id].health_status.get("last_checked", 0)
                    health_checked = True
