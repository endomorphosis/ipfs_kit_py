...
from ipfs_kit_py.mcp.models.ipfs_model_anyio import normalize_response
...
class IPFSModel:
    ...
    async def shutdown_async(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if not webrtc_result.get("success", False):
                result["errors"].append(f"WebRTC connections cleanup failed: {webrtc_result.get('error', 'unknown error')}")
        except Exception as e:
    ...
    def shutdown(self) -> Dict[str, Any]:
        ...
        try:
            ...
            # Run async shutdown
            if loop.is_running():
                logger.warning("Event loop is running, can't use run_until_complete - doing limited synchronous cleanup")
