...
class RoutingPolicy(BaseModel):
    ...
    weight_factors: Dict[str, float] = Field(default_factory=dict, description="Weighting factors for criteria")
    content_patterns: Optional[Dict[str, Any]] = Field(None, description="Content patterns for routing")
...
class RoutingResult(BaseModel):
    ...
    alternatives: List[Dict[str, Any]] = Field(default_factory=list, description="Alternative backends")
...
class RouteStatistics(BaseModel):
    ...
    backend_usage: Dict[str, int] = Field(default_factory=dict, description="Usage count per backend")
    ...
    content_size_histogram: Dict[str, int] = Field(default_factory=dict, description="Content size histogram")
...
class DataRoutingService:
    ...
    async def route_content(self, request: ContentRequest, policy_id: Optional[str] = None) -> RoutingResult:
        """
        ...
        try:
            ...
            if policy_id:
                ...
                if result:
                    ...
                    self.stats.backend_usage[result.backend_id] = self.stats.backend_usage.get(result.backend_id, 0) + 1
                    self.stats.policy_usage[policy.id] = self.stats.policy_usage.get(policy.id, 0) + 1
                    
                    # Update content size histogram
                    size_bin = self._get_size_bin(request.content_size)
                    self.stats.content_size_histogram[size_bin] = self.stats.content_size_histogram.get(size_bin, 0) + 1
        ...
        if self.metrics_service:
            try:
                labels = {
                    ...
                    "policy": result.policy_id or "fallback",
                ...
                self.metrics_service.record_api_operation("route_content", "routing", 
                                                        time.time() - duration, "success")
            except Exception as e:
                logger.error(f"Error updating metrics: {e}")
    
    async def _apply_policy(self, policy: RoutingPolicy, request: ContentRequest, 
                          available_backends: List[str]) -> Optional[RoutingResult]:
        """
        ...
        for backend_id in filtered_backends:
            ...
            if policy.criteria.get("min_reliability") and metadata.reliability < policy.criteria["min_reliability"]:
                continue
            
            if request.content_size > 0 and metadata.max_file_size and request.content_size > metadata.max_file_size:
                continue
            
            if policy.criteria.get("preferred_tier") and metadata.tier != policy.criteria["preferred_tier"]:
                continue
    ...
    async def _calculate_backend_score(self, metadata: BackendMetadata, 
                                     request: ContentRequest, 
                                     policy: RoutingPolicy) -> float:
        """
    ...
    async def _calculate_cost_factor(self, metadata: BackendMetadata, request: ContentRequest) -> float:
        """
    ...
    async def _calculate_performance_factor(self, metadata: BackendMetadata, request: ContentRequest) -> float:
        """
    ...
    async def _calculate_geographic_factor(self, metadata: BackendMetadata, request: ContentRequest) -> float:
        """
            ...
            if client_location:
                ...
                if backend_location:
                    ...
                    distance = self._calculate_geo_distance(
                        ...
                        backend_location.latitude, backend_location.longitude
                    )
        ...
        if backend_region_parts and request_region_parts:
            if backend_region_parts[0] == request_region_parts[0]:
                # Same main region
                return 0.9 if len(backend_region_parts) > 1 and len(request_region_parts) > 1 and backend_region_parts[1] != request_region_parts[1] else 1.0
            else:
    ...
    def _get_selection_reasons(self, metadata: BackendMetadata, request: ContentRequest, 
                             policy: RoutingPolicy) -> List[str]:
        """
        ...
        if metadata.capacity > 0:
            ...
            reasons.append(f"Capacity: {used_percent:.1f}% used of {self._format_size(metadata.capacity)}")
    ...
    def _estimate_latency(self, metadata: BackendMetadata, request: ContentRequest) -> Optional[float]:
        """
