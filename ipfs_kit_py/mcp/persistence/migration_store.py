...
class MigrationStore:
    ...
    async def get_summary(self) -> Dict[str, Any]:
        ...
        most_used_source = max(source_backends.items(), key=lambda x: x[1])[0] if source_backends else None
        most_used_target = max(target_backends.items(), key=lambda x: x[1])[0] if target_backends else None
