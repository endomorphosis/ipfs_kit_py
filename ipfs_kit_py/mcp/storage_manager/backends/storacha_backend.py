...
class StorachaConnectionManager:
    ...
    def __init__(
        ...
        self.endpoint_performance = {endpoint: {'success': 0, 'error': 0, 'latency': []} for endpoint in self.api_endpoints}
        ...
        self.session.headers.update({
            'User-Agent': 'IPFS-Kit-Python/1.0',
            ...
            try:
                ...
                with self.lock:
                    ...
                    if len(self.endpoint_performance[endpoint]['latency']) > 10:
                        self.endpoint_performance[endpoint]['latency'] = self.endpoint_performance[endpoint]['latency'][-10:]
    ...
    def _handle_request_with_retry(self, method, path, **kwargs):
        ...
        raise requests.RequestException(f"Failed to connect to any Storacha endpoints after {self.max_retries} retries")
    ...
    def _mock_response(self, method, path, **kwargs):
        ...
        if method.lower() == 'get' and path.startswith('status'):
            mock_content = {"ok": True, "message": "Mock mode active", "version": "mock-1.0.0"}
            ...
            if isinstance(content, (str, bytes)):
                mock_cid = base64.b32encode(hashlib.sha256(str(content).encode()).digest()[:10]).decode().lower()
...
class StorachaBackend(BackendStorage):
    ...
    def __init__(self, resources: Dict[str, Any], metadata: Dict[str, Any]):
        ...
        self.connection_timeout = int(resources.get('connection_timeout', DEFAULT_CONNECTION_TIMEOUT))
    ...
    def _init_local_cache(self):
        ...
        if not cache_dir:
            ...
            cache_dir = os.path.join(temp_dir, f'mcp_storacha_cache_{base64.b32encode(os.urandom(5)).decode().lower()}')
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                if cid:
                    ...
                    if options.get('cache', True) and isinstance(data, bytes) and len(data) < self.cache_size_limit // 10:
    ...
    def migrate_to(
        ...
        storage_options = {
            'metadata': {**metadata, **migration_metadata}
        }
        ...
        result = {
            ...
            "content_size": len(data) if isinstance(data, bytes) else retrieve_result.get('details', {}).get('content_length'),
        ...
        if verification_result:
            ...
            if not verification_result.get('success', False):
                ...
                result["error"] = f"Migration verification failed: {verification_result.get('error', 'Unknown error')}"
    ...
    def get_status(self) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                "success": True,
                "backend": self.get_name(),
                "available": any(endpoint["healthy"] for endpoint in connection_status["endpoints"].values()),
                "status": {
