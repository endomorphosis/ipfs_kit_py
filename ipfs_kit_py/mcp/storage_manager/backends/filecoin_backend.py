...
class FilecoinBackend(BackendStorage):
    ...
    def __init__(self, resources: Dict[str, Any], metadata: Dict[str, Any]):
        ...
        try:
            ...
            try:
                ...
                logger.error("Failed to initialize Filecoin backend: lotus_kit and filecoin_storage not available")
    ...
    def retrieve(
        ...
        try:
            ...
            if self.mode == "lotus":
                # Use Lotus client
                result = self.lotus.lotus_client_retrieve(
                    cid=identifier,
                    output_path=temp_file
                )
            else:
                # Use Filecoin Storage client
                result = self.filecoin.filecoin_retrieve_file(
                    cid=identifier,
                    output_path=temp_file
                )
    ...
    def delete(
        ...
        try:
            ...
            if not result.get("success", False) and "sealed deals cannot be cancelled" in result.get("error", "").lower():
                # Content is in sealed deals which cannot be deleted
                logger.warning(f"Content {identifier} has sealed deals which cannot be cancelled in Filecoin")
                return {
