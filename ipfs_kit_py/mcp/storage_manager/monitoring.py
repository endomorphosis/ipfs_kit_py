...
class MonitoringSystem:
    ...
    def _initialize_metrics(self):
        ...
        for backend_type, backend in self.storage_manager.backends.items():
            ...
            self.error_counts[backend_type.value] = {
                op_type.value: 0
                for op_type in OperationType
            }
            ...
            self.success_rates[backend_type.value] = {
                op_type.value: 1.0  # Start optimistic
                for op_type in OperationType
            }
    ...
    def record_operation(
        ...
        self.success_rates[backend_type][operation] = (total_ops - errors) / total_ops if total_ops > 0 else 1.0
    ...
    def _prune_history(self, backend_type: str, operation: str):
        ...
        self.performance_history[backend_type][operation] = [
            entry for entry in self.performance_history[backend_type][operation]
    ...
    def check_all_backends_health(self) -> Dict[str, Any]:
        ...
        for backend_type in self.storage_manager.backends:
            ...
            elif result["status"] == BackendStatus.DEGRADED.value and overall_status != BackendStatus.UNHEALTHY.value:
    ...
    def update_capacity_metrics(self) -> Dict[str, Any]:
        ...
        for backend_type, backend in self.storage_manager.backends.items():
            ...
            if hasattr(backend, 'get_capacity_metrics') and callable(getattr(backend, 'get_capacity_metrics', None)):
                try:
    ...
    def get_performance_metrics(
        ...
        for b_type in backend_types:
            ...
            for op_type in op_types:
                ...
                if b_type in self.performance_history and op_type in self.performance_history[b_type]:
                    ...
                    if history:
                        ...
                        recent_entries = [entry for entry in history if entry["timestamp"] > day_ago]
    ...
    def get_backend_status(
        ...
        for b_type, status in self.backend_status.items():
            ...
            if status == BackendStatus.UNHEALTHY.value:
                overall_status = BackendStatus.UNHEALTHY.value
            elif status == BackendStatus.DEGRADED.value and overall_status != BackendStatus.UNHEALTHY.value:
                overall_status = BackendStatus.DEGRADED.value
            elif status == BackendStatus.UNKNOWN.value and overall_status == BackendStatus.HEALTHY.value:
    ...
    def generate_report(self) -> Dict[str, Any]:
        ...
        for backend, status in metrics["backend_status"]["backends"].items():
            if status["status"] == BackendStatus.UNHEALTHY.value:
                ...
                recommendations.append(f"Check connectivity to {backend} backend and verify credentials")
        ...
        for backend, ops in metrics["performance_metrics"]["performance_metrics"].items():
            for op_type, metrics in ops.items():
                ...
                if metrics["success_rate"] < 0.95:
                    warnings.append(f"Backend {backend} has low success rate ({metrics['success_rate']:.2%}) for {op_type} operations")
                    recommendations.append(f"Investigate failures for {op_type} operations on {backend} backend")
                    
                # Check for unusually high latency
                avg_time = metrics["avg_time"]
                if avg_time > 5.0:  # More than 5 seconds
                    warnings.append(f"Backend {backend} has high latency ({avg_time:.2f}s) for {op_type} operations")
                    recommendations.append(f"Consider optimizing {op_type} operations on {backend} backend")
        ...
        for backend, capacity in metrics["capacity_metrics"]["backends"].items():
            if "usage_percent" in capacity and capacity["usage_percent"] > 80:
                warnings.append(f"Backend {backend} is nearing capacity ({capacity['usage_percent']}% used)")
                recommendations.append(f"Consider adding more storage to {backend} backend or migrating content")
        ...
        if assessment != "Healthy":
            recommendations.append(f"Perform a full system check to identify issues causing {assessment.lower()} status")
