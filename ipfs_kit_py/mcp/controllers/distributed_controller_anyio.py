...
class PeerDiscoveryRequest(BaseModel):
    ...
    max_peers: int = Field(
        default=10, 
        description="Maximum number of peers to discover"
    )
    ...
    discovery_namespace: Optional[str] = Field(
        None, 
        description="Namespace for discovery (e.g., 'ipfs-kit-cluster')"
    )
...
class PeerDiscoveryResponse(DistributedResponse):
    ...
    discovery_methods_used: List[str] = Field(
        default=[],
        description="Discovery methods that were successful"
    )
    total_peers_found: int = Field(
        0,
        description="Total number of peers found"
    )
...
class ClusterCacheRequest(BaseModel):
    ...
    value: Optional[Any] = Field(
        None,
        description="Value to cache (for 'put' operations)"
    )
    ...
    propagate: bool = Field(
        True,
        description="Whether to propagate the operation to other cluster nodes"
    )
...
class ClusterCacheResponse(DistributedResponse):
    ...
    value: Optional[Any] = Field(
        None,
        description="Retrieved cache value (for 'get' operations)"
    )
    nodes_affected: int = Field(
        0,
        description="Number of cluster nodes affected by the operation"
    )
    propagation_status: Dict[str, Any] = Field(
        default={},
        description="Status of operation propagation to other nodes"
    )
...
class ClusterStateRequest(BaseModel):
    ...
    operation: str = Field(
        ...,
        description="State operation to perform (query, update, subscribe)"
    )
    path: Optional[str] = Field(
        None,
        description="State path to operate on (e.g., 'nodes.worker1.status')"
    )
    value: Optional[Any] = Field(
        None,
        description="Value to set (for 'update' operations)"
    )
    ...
    subscription_id: Optional[str] = Field(
        None,
        description="Subscription ID for 'subscribe' operations"
    )
...
class StateSyncRequest(BaseModel):
    ...
    force_full_sync: bool = Field(
        False, 
        description="Whether to force a full state synchronization"
    )
    target_nodes: Optional[List[str]] = Field(
        None,
        description="Specific nodes to synchronize with"
    )
...
class ClusterStateResponse(DistributedResponse):
    ...
    operation: str = Field(
        ...,
        description="State operation performed"
    )
    path: Optional[str] = Field(
        None,
        description="State path operated on"
    )
    value: Optional[Any] = Field(
        None,
        description="Retrieved or updated state value"
    )
    subscription_id: Optional[str] = Field(
        None,
        description="Subscription ID for 'subscribe' operations"
    ...
    update_count: Optional[int] = Field(
        None,
        description="Number of state entries updated"
    )
...
class NodeRegistrationRequest(BaseModel):
    ...
    node_id: Optional[str] = Field(
        None,
        description="Node identifier (generated if not provided)"
    )
    role: str = Field(
        ...,
        description="Node role (master, worker, leecher)"
    )
...
class NodeRegistrationResponse(DistributedResponse):
    ...
    role: str = Field(
        ...,
        description="Confirmed node role"
    )
    status: str = Field(
        ...,
        description="Node status after registration"
    )
    ...
    master_address: Optional[str] = Field(
        None,
        description="Master node address (for worker/leecher nodes)"
    )
    peers: List[Dict[str, Any]] = Field(
        default=[],
        description="List of immediate peers to connect to"
    )
...
class DistributedTaskRequest(BaseModel):
    ...
    task_type: str = Field(
        ...,
        description="Type of task to perform or submit"
    )
    ...
    priority: int = Field(
        default=5,
        description="Task priority (1-10, with 10 being highest)"
    )
    target_role: Optional[str] = Field(
        None,
        description="Target role for the task (e.g., 'worker')"
    )
    target_node: Optional[str] = Field(
        None,
        description="Specific node to assign the task to"
    )
...
class DistributedTaskResponse(DistributedResponse):
    ...
    assigned_to: Optional[str] = Field(
        None,
        description="Node the task is assigned to"
    )
    result_cid: Optional[str] = Field(
        None,
        description="CID of task result (if available)"
    )
...
class DistributedControllerAnyIO:
    """
    Controller for distributed operations using AnyIO.
    ...
    def __init__(self, ipfs_model):
        ...
        logger.info("Distributed Controller (AnyIO) initialized")
    ...
    def register_routes(self, router: APIRouter):
        ...
        router.add_api_websocket_route(
            "/distributed/events",
            self.cluster_events_websocket,
            name="cluster_events"
        )
        
        logger.info("Distributed Controller (AnyIO) routes registered")
    ...
    async def discover_peers(self, request: PeerDiscoveryRequest) -> Dict[str, Any]:
        ...
        try:
            # Call the model method for peer discovery, using anyio for potential timeouts
            async with anyio.fail_after(request.timeout_seconds):
                # Check if execute_command is already async
                if hasattr(self.ipfs_model.execute_command, "__await__"):
                    # Method is already async
                    result = await self.ipfs_model.execute_command(
                    ...
                    result = await anyio.to_thread.run_sync(
                        lambda: self.ipfs_model.execute_command(
                            command="discover_peers",
                            args=[],
                            params={
                                "discovery_methods": request.discovery_methods,
            ...
            raise HTTPException(status_code=504, detail="Peer discovery timed out")
    ...
    async def list_known_peers(self, 
                         ...
                         filter_role: Optional[str] = None) -> Dict[str, Any]:
        """
    ...
    async def get_cache_status(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if hasattr(self.ipfs_model.execute_command, "__await__"):
                # Method is already async
                result = await self.ipfs_model.execute_command(
                    command="get_cluster_cache_status",
                    args=[],
                    params={}
                )
            else:
    ...
    async def synchronize_state(self, 
                         sync_data: StateSyncRequest) -> Dict[str, Any]:
        """
    ...
    async def submit_task(self, request: DistributedTaskRequest) -> Dict[str, Any]:
        ...
        try:
            ...
            timeout = request.timeout_seconds if request.timeout_seconds else 60  # Default timeout
            ...
            # Check if we timed out (result will be None)
            if result is None:
                raise TimeoutError(f"Task submission timed out after {timeout} seconds")
    ...
    async def list_tasks(self, 
                   ...
                   filter_node: Optional[str] = None) -> Dict[str, Any]:
        """
    ...
    async def cluster_events_websocket(self, websocket):
        ...
        try:
            ...
            if hasattr(self.ipfs_model.execute_command, "__await__"):
                # Method is already async
                await self.ipfs_model.execute_command(
            ...
            # Check if add_event_listener is already async
            if hasattr(self.ipfs_model.add_event_listener, "__await__"):
                # Method is already async
                await self.ipfs_model.add_event_listener(subscription_id, event_handler)
            else:
                # Method is sync, run in a thread
                await anyio.to_thread.run_sync(
                    lambda: self.ipfs_model.add_event_listener(subscription_id, event_handler)
                )
            ...
            if 'subscription_id' in locals():
                ...
                # Check if remove_event_listener is already async
                if hasattr(self.ipfs_model.remove_event_listener, "__await__"):
                    # Method is already async
                    await self.ipfs_model.remove_event_listener(subscription_id)
                else:
                    # Method is sync, run in a thread
                    await anyio.to_thread.run_sync(
                        lambda: self.ipfs_model.remove_event_listener(subscription_id)
                    )
