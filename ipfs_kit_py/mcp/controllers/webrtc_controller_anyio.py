...
from ipfs_kit_py.mcp.controllers.webrtc_controller import (
    WebRTCResponse, ResourceStatsResponse, StreamRequest, StreamResponse,
    ConnectionResponse, ConnectionsListResponse, ConnectionStatsResponse,
    DependencyResponse, BenchmarkRequest, BenchmarkResponse, QualityRequest
)
...
class WebRTCControllerAnyIO:
                ...
                while not self.is_shutting_down:
                    try:
                        ...
                        for server_id, server_info in list(self.active_streaming_servers.items()):
                            ...
                            if is_benchmark and (current_time - started_at) > (duration + 60):  # Add 60s buffer
                                stale_servers.append(server_id)
                                logger.info(f"Detected stale benchmark server: {server_id}, age: {current_time - started_at:.0f}s")
                        ...
                        try:
                            ...
                            if result.get("success", False):
                                ...
                                for conn_id in stale_connections:
                                    logger.info(f"Removing stale connection from tracking: {conn_id}")
                        ...
                        try:
                            ...
                            if "system" in stats and "health_score" in stats["system"]:
                                ...
                                if health_score < stats["resource_management"]["auto_cleanup_threshold"]:
                                    logger.warning(
                                    ...
                                    logger.info(f"Servers before cleanup: {len(self.active_streaming_servers)}")
                                    logger.info(f"Connections before cleanup: {len(self.active_connections)}")
                                    ...
                                    for server_info in stats["servers"]["servers"]:
                                        ...
                                        if server_info["connection_count"] == 0:
                                            logger.info(f"Marking idle server for cleanup: {server_id}")
                                            high_impact_servers.append(server_id)
                                            continue
                                            
                                        # Check if server has high impact score
                                        if server_info["impact_score"] > 70:
                                            logger.info(f"Marking high-impact server for cleanup: {server_id}")
                                            high_impact_servers.append(server_id)
                                    
                                    # Clean up high-impact servers, oldest first
                                    high_impact_servers.sort(
                                        key=lambda sid: self.active_streaming_servers.get(sid, {}).get("started_at", 0)
                                    )
                                    ...
                                    for server_id in servers_to_cleanup:
                                        try:
                                            ...
                                            logger.error(f"Error auto-cleaning server {server_id}: {e}")
                                    
                                    # Log the result of cleanup
                                    logger.info(f"Auto-cleanup complete. Stopped {len(servers_to_cleanup)} servers")
                                    logger.info(f"Servers after cleanup: {len(self.active_streaming_servers)}")
                                    logger.info(f"Connections after cleanup: {len(self.active_connections)}")
                        except Exception as e:
        ...
        def handle_anyio_cancel():
            ...
            try:
                ...
                if isinstance(self.cleanup_task, dict):
                    ...
                    logger.info(f"Task is being tracked as: {self.cleanup_task.get('type', 'unknown')}")
                ...
                # For standard AnyIO task
                    ...
                    # Unknown task type
                    logger.warning(f"Unknown task type: {type(self.cleanup_task).__name__}, falling back to flag-based cancellation")
    ...
    async def close_all_streaming_servers(self):
        ...
        for server_id, server_info in list(self.active_streaming_servers.items()):
            try:
                ...
                if isinstance(result, dict) and not result.get("success", False):
                    logger.error(f"Error stopping streaming server {server_id}: {result.get('error', 'Unknown error')}")
                else:
    ...
    def get_resource_stats(self):
        ...
        try:
            ...
            system_resources["status"] = "critical" if health_score < 30 else "warning" if health_score < 70 else "healthy"
        ...
        return {
            ...
            "resource_management": {
                ...
                "last_auto_cleanup": self.last_auto_cleanup if hasattr(self, "last_auto_cleanup") else None
            }
        }
    ...
    async def stream_content(self, request: StreamRequest) -> Dict[str, Any]:
        ...
        if "system" in stats and "health_score" in stats["system"]:
            ...
            if health_score < 50 and request.quality == "high":
                logger.warning(f"Downgrading quality from high to medium due to health score {health_score}")
        ...
        try:
            # Run the streaming operation in a background thread using anyio
            result = await anyio.to_thread.run_sync(
    ...
    async def stop_streaming(self, server_id: str) -> Dict[str, Any]:
        ...
        try:
            # Run the stop streaming operation in a background thread using anyio
            result = await anyio.to_thread.run_sync(
                self.ipfs_model.stop_webrtc_streaming,
                server_id=server_id
            )
    ...
    async def list_connections(self) -> Dict[str, Any]:
        ...
        try:
            # Run the list connections operation in a background thread using anyio
            result = await anyio.to_thread.run_sync(self.ipfs_model.list_webrtc_connections)
    ...
    async def get_connection_stats(self, connection_id: str) -> Dict[str, Any]:
        ...
        try:
            # Run the get connection stats operation in a background thread using anyio
            result = await anyio.to_thread.run_sync(
                self.ipfs_model.get_webrtc_connection_stats,
                connection_id=connection_id
            )
            ...
            if stats and connection_id in self.active_connections:
                ...
                if "quality_metrics" in stats:
                    self.active_connections[connection_id]["quality_metrics"] = stats["quality_metrics"]
                if "latency" in stats:
    ...
    async def close_connection(self, connection_id: str) -> Dict[str, Any]:
        ...
        try:
            # Run the close connection operation in a background thread using anyio
            result = await anyio.to_thread.run_sync(
                self.ipfs_model.close_webrtc_connection,
                connection_id=connection_id
            )
    ...
    async def close_all_connections(self) -> Dict[str, Any]:
        ...
        try:
            # Run the close all connections operation in a background thread using anyio
            result = await anyio.to_thread.run_sync(self.ipfs_model.close_all_webrtc_connections)
    ...
    async def set_quality(self, request: QualityRequest) -> Dict[str, Any]:
        ...
        Returns:
            Dictionary with operation results
        """
        logger.debug(f"Setting quality for connection {request.connection_id} to: {request.quality}")
        
        try:
            # Run the set quality operation in a background thread using anyio
            result = await anyio.to_thread.run_sync(
                self.ipfs_model.set_webrtc_quality,
    ...
    async def run_benchmark(self, request: BenchmarkRequest) -> Dict[str, Any]:
        ...
        try:
            # Run the benchmark operation in a background thread using anyio
            result = await anyio.to_thread.run_sync(
                self.ipfs_model.run_webrtc_benchmark,
                cid=request.cid,
                    ...
                    if benchmark_id in self.active_streaming_servers:
                        ...
                        try:
                            ...
                            if cleanup_result and not cleanup_result.get("success", False):
                                logger.warning(f"Cleanup after benchmark failed: {cleanup_result.get('error', 'Unknown error')}")
                        except Exception as e:
