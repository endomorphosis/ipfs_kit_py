...
from ..auth.middleware import get_current_user, require_permission, require_role
from ..auth.models import (
    User, Role, Permission, ApiKey, Session, 
    ...
    async def list_api_keys(user_id: str = Depends(get_current_user)):
        ...
        try:
            ...
            for key_id, key_data in api_keys.items():
                sanitized_key = {k: v for k, v in key_data.items() if k not in ["hashed_key", "key"]}
    ...
    async def list_sessions(user_id: str = Depends(get_current_user)):
        ...
        try:
            ...
            for session_id, session_data in sessions.items():
                if session_data.get("active", False) and session_data.get("expires_at", 0) > datetime.now().timestamp():
        ...
        try:
            ...
            if session.get("user_id") != user_id:
                return JSONResponse(
                    ...
                    content={"success": False, "message": "Session does not belong to this user"}
                )
    ...
    async def list_users(
        ...
        try:
            ...
            for user_id, user_data in users.items():
                sanitized_user = {k: v for k, v in user_data.items() if k not in ["hashed_password"]}
