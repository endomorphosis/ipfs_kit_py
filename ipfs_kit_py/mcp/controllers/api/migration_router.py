...
from fastapi import APIRouter, HTTPException, Body, Query, BackgroundTasks, Depends, Request
...
def create_migration_router(migration_controller):
    ...
    async def cancel_migration(migration_id: str):
        ...
        try:
            ...
            if not migration.get("success", False):
                return JSONResponse(
                    status_code=404,
                    content={"success": False, "error": f"Migration {migration_id} not found"}
                )
    ...
    @router.post("/cleanup")
    async def cleanup_old_migrations(days: int = Body(30, description="Number of days to keep records for")):
