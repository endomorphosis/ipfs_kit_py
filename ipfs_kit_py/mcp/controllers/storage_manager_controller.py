...
class ReplicationPolicyResponse(OperationResponse):
    ...
    successful_backends: List[str] = Field([], description="List of backends where replication succeeded")
...
class BackendStatusResponse(OperationResponse):
    ...
    capabilities: List[str] = Field([], description="List of capabilities supported by this backend")
...
class AllBackendsStatusResponse(OperationResponse):
    ...
    backends: Dict[str, BackendStatusResponse] = Field({}, description="Status of each storage backend")
...
class ContentMigrationRequest(BaseModel):
    ...
    options: Optional[Dict[str, Any]] = Field(None, description="Backend-specific options")
    delete_source: bool = Field(False, description="Whether to delete content from source after migration")
    verify_integrity: bool = Field(True, description="Whether to verify content integrity after migration")
...
class StorageManagerController:
    ...
    async def handle_verify_request(self, content_id: str = Body(..., embed=True), backends: List[str] = Body(None, embed=True)):
        ...
        verified_backends = [name for name, result in verification_results.items() if result.get("success", False)]
    ...
    def _transfer_content(self, source_backend: str, target_backend: str, content_id: str, options: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
        """
    ...
    async def handle_replication_policy_request(self, request: ReplicationPolicyRequest):
        ...
        if not hasattr(self.storage_manager, "storage_bridge"):
            raise HTTPException(
                status_code=400,
                detail={
                    "error": "Storage bridge not available for policy application",
                    "error_type": "StorageBridgeNotAvailable"
                }
            )
