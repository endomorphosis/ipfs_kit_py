...
class CliControllerAnyIO:
    ...
    async def add_content(self, content: str = Body(..., description="Content to add"), 
        ...
        try:
            ...
            result = await anyio.to_thread.run_sync(
                lambda: self.api.add(content, **params)
            )
    ...
    async def pin_content(self, cid: str = Path(..., description="Content identifier"),
        ...
        try:
            ...
            result = await anyio.to_thread.run_sync(
                lambda: self.api.pin(cid, recursive=recursive)
            )
    ...
    async def wal_command(self, command: str = Path(..., description="WAL command to execute"),
                        body: Dict[str, Any] = Body({}, description="Command parameters")) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            result = await anyio.to_thread.run_sync(
                lambda: handle_wal_command(command, **body)
            )
    ...
    async def wal_status(self) -> Dict[str, Any]:
        ...
        try:
            ...
            result = await anyio.to_thread.run_sync(
                lambda: handle_wal_command("status")
            )
    ...
    async def wal_logs(self, tail: int = Query(10, description="Number of log entries to return")) -> StreamingResponse:
        """
        Stream WAL logs.
