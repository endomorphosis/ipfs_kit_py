...
class URIListModel(BaseModel):
    ...
    options: Optional[Dict[str, Any]] = Field(None, description="Optional advanced options for the download")
...
class DaemonOptionsModel(BaseModel):
    ...
    options: Optional[Dict[str, Any]] = Field(None, description="Optional daemon configuration options")
...
class Aria2Controller:
    ...
    async def add_torrent(self, torrent_file: UploadFile = File(...), options: str = Form(None)):
        ...
        try:
            ...
            # Add torrent
            result = self.aria2_model.add_torrent(
    ...
    async def create_metalink(self, files_data: MetalinkFileModel):
        ...
        try:
            ...
            result = self.aria2_model.aria2_kit.create_metalink(
    ...
    async def remove_download(self, download: DownloadIDModel):
        ...
        """
        result = self.aria2_model.remove_download(
            gid=download.gid,
    ...
    async def pause_download(self, download: DownloadIDModel):
        ...
        """
        result = self.aria2_model.pause_download(
            gid=download.gid,
            force=download.force
        )
        if not result.get("success", False):
