...
class CredentialBaseRequest(BaseModel):
    ...
    name: str = Field(..., description="Name for this credential set (e.g., 'default', 'production')")
...
class S3CredentialRequest(CredentialBaseRequest):
    ...
    endpoint_url: Optional[str] = Field(None, description="Optional endpoint URL for custom S3 services")
...
class FilecoinCredentialRequest(CredentialBaseRequest):
    ...
    provider: Optional[str] = Field(None, description="Optional provider name (e.g., 'estuary', 'lotus', 'glif')")
...
class CredentialInfoResponse(BaseModel):
    ...
    success: bool = Field(..., description="Whether the operation was successful")
    credentials: List[Dict[str, Any]] = Field(..., description="List of credential information without secrets")
...
class CredentialController:
    ...
    async def add_generic_credential(self, credential_request: GenericCredentialRequest):
        ...
        """
        logger.debug(f"Adding generic credentials for service: {credential_request.service}, name: {credential_request.name}")
        
        try:
            # Validate service
            valid_services = ["ipfs", "s3", "storacha", "filecoin", "ipfs_cluster"]
            if credential_request.service not in valid_services:
                raise HTTPException(status_code=400, detail=f"Invalid service: {credential_request.service}")
    ...
    async def add_s3_credentials(self, credential_request: S3CredentialRequest):
        ...
        try:
            ...
            if not success:
                raise HTTPException(status_code=500, detail="Failed to add S3 credentials")
            ...
            raise HTTPException(status_code=500, detail=f"Failed to add S3 credentials: {str(e)}")
    ...
    async def add_storacha_credentials(self, credential_request: StorachaCredentialRequest):
        ...
        try:
            ...
            if not success:
                raise HTTPException(status_code=500, detail="Failed to add Storacha credentials")
            ...
            raise HTTPException(status_code=500, detail=f"Failed to add Storacha credentials: {str(e)}")
    ...
    async def add_filecoin_credentials(self, credential_request: FilecoinCredentialRequest):
        ...
        try:
            ...
            if not success:
                raise HTTPException(status_code=500, detail="Failed to add Filecoin credentials")
            ...
            raise HTTPException(status_code=500, detail=f"Failed to add Filecoin credentials: {str(e)}")
    ...
    async def add_ipfs_credentials(self, credential_request: IPFSCredentialRequest):
        ...
        try:
            ...
            if credential_request.cluster_secret:
                ...
                self.credential_manager.add_credential("ipfs_cluster", credential_request.name, {
                    "type": "cluster_secret",
            ...
            success = self.credential_manager.add_credential("ipfs", credential_request.name, credentials)
            
            if not success:
                raise HTTPException(status_code=500, detail="Failed to add IPFS credentials")
            ...
            raise HTTPException(status_code=500, detail=f"Failed to add IPFS credentials: {str(e)}")
    ...
    async def remove_credential(self, service: str, name: str):
        ...
        try:
            ...
            if not success:
                raise HTTPException(status_code=404, detail=f"Credentials not found for {service}/{name}")
