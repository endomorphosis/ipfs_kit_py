...
try:
    from ipfs_kit_py.mcp.models.mcp_discovery_model import (
        MCPDiscoveryModel, MCPServerInfo, MCPFeatureSet,
        MCPServerRole, MCPServerCapabilities, MCPMessageType
    )
except ImportError:
    ...
    class MCPFeatureSet:
        def __init__(self, features): pass
    
    class MCPServerInfo:
        def __init__(self, server_id, role, feature_set): pass
...
class AnnounceRequest(BaseModel):
    ...
    additional_metadata: Optional[Dict[str, Any]] = Field(None, description="Additional metadata to include")
...
class DispatchTaskRequest(BaseModel):
    ...
    required_features: Optional[List[str]] = Field(None, description="Required features for the task")
...
class MCPDiscoveryControllerAnyIO:
    ...
    def get_known_servers(self, filter_role: Optional[str] = None, filter_features: Optional[str] = None):
        """Synchronous version that warns if called from async context."""
    ...
    async def get_local_server_info_async(self) -> Dict[str, Any]:
        ...
        server_info = await anyio.to_thread.run_sync(
            self.discovery_model.get_server_info,
            self.discovery_model.server_id
        )
    ...
    async def announce_server_async(self, request: Optional[AnnounceRequest] = None) -> Dict[str, Any]:
        """
        ...
        announcement_result = await anyio.to_thread.run_sync(
            self.discovery_model.announce_server
        )
    ...
    async def get_known_servers_async(self, 
                               filter_role: Optional[str] = None,
                               filter_features: Optional[str] = None) -> Dict[str, Any]:
        """
    ...
    async def get_compatible_servers_async(self, 
                                    feature_requirements: Optional[str] = None) -> Dict[str, Any]:
        """
    ...
    async def register_server_async(self, request: RegisterServerRequest) -> Dict[str, Any]:
        ...
        register_result = await anyio.to_thread.run_sync(
            self.discovery_model.register_server,
            request.server_info
        )
    ...
    async def remove_server_async(self, server_id: str) -> Dict[str, Any]:
        ...
        remove_result = await anyio.to_thread.run_sync(
            self.discovery_model.remove_server,
            server_id
        )
        
        # If server not found, raise 404
        if not remove_result["success"] and remove_result.get("error") == f"Server not found: {server_id}":
            raise HTTPException(status_code=404, detail=f"Server not found: {server_id}")
    ...
    async def clean_stale_servers_async(self, max_age_seconds: int = 3600) -> Dict[str, Any]:
        ...
        clean_result = await anyio.to_thread.run_sync(
            self.discovery_model.clean_stale_servers,
    ...
    async def check_server_health_async(self, server_id: str) -> Dict[str, Any]:
        ...
        health_result = await anyio.to_thread.run_sync(
            self.discovery_model.check_server_health,
            server_id
        )
        
        # If server not found, raise 404
        if not health_result["success"] and health_result.get("error", "").startswith("Server not found"):
            raise HTTPException(status_code=404, detail=f"Server not found: {server_id}")
    ...
    async def get_stats_async(self) -> Dict[str, Any]:
        ...
        stats_result = await anyio.to_thread.run_sync(
            self.discovery_model.get_stats
        )
    ...
    async def reset_async(self) -> Dict[str, Any]:
        ...
        reset_result = await anyio.to_thread.run_sync(
