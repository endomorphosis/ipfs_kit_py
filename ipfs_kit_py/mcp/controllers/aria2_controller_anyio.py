...
try:
    ...
    from ipfs_kit_py.mcp.controllers.aria2_controller import (
        Aria2Controller, URIListModel, DownloadIDModel, 
        DaemonOptionsModel, MetalinkFileModel
    )
except ImportError:
    ...
    class Aria2Controller:
        pass
    class URIListModel(BaseModel): pass
    class DownloadIDModel(BaseModel): pass
    class DaemonOptionsModel(BaseModel): pass
    class MetalinkFileModel(BaseModel): pass
...
class Aria2ControllerAnyIO(Aria2Controller):
    ...
    async def health_check_async(self):
        ...
        try:
            ...
            health_status = {
                ...
                "timestamp": time.time(),
                "features": {"bittorrent": False, "metalink": False, "websocket": False}
            }
    ...
    async def get_version_async(self):
        ...
        if not result.get("success", False):
            raise HTTPException(
                status_code=503,
                detail=f"Aria2 not available: {result.get('error', 'Unknown error')}"
            )
        return result
    ...
    async def add_uri_async(self, uri_data: URIListModel):
        ...
        if not result.get("success", False):
            raise HTTPException(
                status_code=400,
                detail=f"Failed to add download: {result.get('error', 'Unknown error')}"
            )
        return result
    
    async def add_torrent_async(self, torrent_file: UploadFile = File(...), options: str = Form(None)):
        """
        ...
        try:
            ...
            if not result.get("success", False):
                raise HTTPException(
                    status_code=400,
                    detail=f"Failed to add torrent: {result.get('error', 'Unknown error')}"
                )
            ...
            raise HTTPException(
                status_code=400,
                detail="Invalid options format. Must be valid JSON."
            )
        except Exception as e:
            logger.error(f"Error in add_torrent: {e}")
            raise HTTPException(
                status_code=500,
                detail=f"Error processing torrent: {str(e)}"
            )
    
    async def add_metalink_async(self, metalink_file: UploadFile = File(...), options: str = Form(None)):
        """
        ...
        try:
            ...
            if not result.get("success", False):
                raise HTTPException(
                    status_code=400,
                    detail=f"Failed to add metalink: {result.get('error', 'Unknown error')}"
                )
            ...
            raise HTTPException(
                status_code=500,
                detail=f"Error processing metalink: {str(e)}"
            )
    ...
    async def create_metalink_async(self, files_data: MetalinkFileModel):
        ...
        try:
            ...
            result = await anyio.to_thread.run_sync(
                self.aria2_model.aria2_kit.create_metalink,
            ...
            if not result.get("success", False):
                raise HTTPException(
                    status_code=400,
                    detail=f"Failed to create metalink: {result.get('error', 'Unknown error')}"
                )
            ...
            raise HTTPException(
                status_code=500,
                detail=f"Error creating metalink: {str(e)}"
            )
    ...
    async def remove_download_async(self, download: DownloadIDModel):
        ...
        """
        result = await anyio.to_thread.run_sync(
            self.aria2_model.remove_download,
            gid=download.gid,
        ...
        if not result.get("success", False):
            raise HTTPException(
                status_code=400,
                detail=f"Failed to remove download: {result.get('error', 'Unknown error')}"
            )
        return result
    ...
    async def pause_download_async(self, download: DownloadIDModel):
        ...
        """
        result = await anyio.to_thread.run_sync(
            self.aria2_model.pause_download,
            gid=download.gid,
            force=download.force
        )
        if not result.get("success", False):
            raise HTTPException(
                status_code=400,
                detail=f"Failed to pause download: {result.get('error', 'Unknown error')}"
            )
        return result
    ...
    async def resume_download_async(self, download: DownloadIDModel):
        ...
        """
        result = await anyio.to_thread.run_sync(
            self.aria2_model.resume_download,
            gid=download.gid
        )
        if not result.get("success", False):
            raise HTTPException(
                status_code=400,
                detail=f"Failed to resume download: {result.get('error', 'Unknown error')}"
            )
        return result
    ...
    async def get_status_async(self, gid: str):
        ...
        """
        result = await anyio.to_thread.run_sync(
            self.aria2_model.get_status,
            gid=gid
        )
        if not result.get("success", False):
            raise HTTPException(
                status_code=404,
                detail=f"Download not found: {result.get('error', 'Unknown error')}"
            )
        return result
    ...
    async def list_downloads_async(self):
        ...
        """
        result = await anyio.to_thread.run_sync(
            self.aria2_model.list_downloads
        )
        if not result.get("success", False):
            raise HTTPException(
                status_code=503,
                detail=f"Failed to list downloads: {result.get('error', 'Unknown error')}"
            )
        return result
    ...
    async def purge_downloads_async(self):
        ...
        """
        result = await anyio.to_thread.run_sync(
            self.aria2_model.purge_downloads
        )
        if not result.get("success", False):
            raise HTTPException(
                status_code=400,
                detail=f"Failed to purge downloads: {result.get('error', 'Unknown error')}"
            )
        return result
    ...
    async def get_global_status_async(self):
        ...
        """
        result = await anyio.to_thread.run_sync(
            self.aria2_model.get_global_status
        )
        if not result.get("success", False):
            raise HTTPException(
                status_code=503,
                detail=f"Failed to get global status: {result.get('error', 'Unknown error')}"
            )
        return result
    ...
    async def start_daemon_async(self, options: DaemonOptionsModel = Body(None)):
        ...
        result = await anyio.to_thread.run_sync(
            self.aria2_model.start_daemon,
            options=daemon_options
        )
        if not result.get("success", False):
            raise HTTPException(
                status_code=500,
                detail=f"Failed to start daemon: {result.get('error', 'Unknown error')}"
            )
        return result
    ...
    async def stop_daemon_async(self):
        ...
        """
        result = await anyio.to_thread.run_sync(
            self.aria2_model.stop_daemon
        )
        if not result.get("success", False):
            raise HTTPException(
                status_code=500,
                detail=f"Failed to stop daemon: {result.get('error', 'Unknown error')}"
            )
        return result
    ...
    def register_routes(self, router: APIRouter):
        ...
        logger.info(f"Registered {len(self.initialized_endpoints)} Aria2 endpoints with AnyIO support")