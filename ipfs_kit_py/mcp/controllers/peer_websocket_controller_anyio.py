...
# Import peer WebSocket components (prioritize anyio version)
try:
    from ipfs_kit_py.peer_websocket_anyio import (
        PeerInfo, PeerWebSocketServer, PeerWebSocketClient, 
        register_peer_websocket, create_peer_info_from_ipfs_kit,
        PeerRole, MessageType, WEBSOCKET_AVAILABLE
    )
...
class StartServerRequest(BaseModel):
    ...
    capabilities: Optional[List[str]] = Field(None, description="Override default peer capabilities")
...
class ConnectToServerRequest(BaseModel):
    ...
    auto_connect: bool = Field(True, description="Whether to automatically connect to discovered peers")
...
class PeerWebSocketControllerAnyIO:
    ...
    def __init__(self, ipfs_model):
        ...
        if not HAS_PEER_WEBSOCKET:
            logger.warning("WebSocket dependency not available. Peer WebSocket discovery will be disabled.")
    ...
    async def start_server(self, request: StartServerRequest) -> Dict[str, Any]:
        ...
        try:
            ...
            await self.peer_websocket_server.start(
                host=request.host,
                port=request.port
            )
            ...
            if hasattr(self, "peer_websocket_server") and self.peer_websocket_server is not None:
                try:
                    ...
                    logger.error(f"Error cleaning up WebSocket server after failed start: {cleanup_error}")
    ...
    async def stop_server(self) -> Dict[str, Any]:
        ...
        try:
            ...
            current_server = self.peer_websocket_server
            self.peer_websocket_server = None
    ...
    async def connect_to_server(self, request: ConnectToServerRequest) -> Dict[str, Any]:
        ...
        try:
            ...
            # Connect to server
            connection_result = await self.peer_websocket_client.connect_to_discovery_server(request.server_url)
            ...
            if hasattr(self, "peer_websocket_client") and self.peer_websocket_client is not None:
                try:
                    ...
                    logger.error(f"Error cleaning up WebSocket client after failed connect: {cleanup_error}")
    ...
    async def disconnect_from_server(self) -> Dict[str, Any]:
        ...
        try:
            ...
            current_client = self.peer_websocket_client
            self.peer_websocket_client = None
    ...
    async def get_discovered_peers(self, 
                              filter_role: Optional[str] = None,
                              filter_capabilities: Optional[str] = None) -> Dict[str, Any]:
        """
