...
class LibP2PController:
    ...
    def register_routes(self, router: APIRouter):
        ...
        logger.info(
            f"Registered libp2p controller routes: {len(self.initialized_endpoints)} endpoints"
        )
    ...
    async def health_check(self):
        ...
        try:
            ...
            complete_result = {
                ...
                "libp2p_available": result.get("dependency_status", {}).get(
                    "libp2p_available", False
                ),
    ...
    async def discover_peers(self, request: PeerDiscoveryRequest):
        ...
        result = await self.libp2p_model.discover_peers(
            discovery_method=request.discovery_method, limit=request.limit
        )
    ...
    async def get_peers(
        ...
        # Call model method
        result = await self.libp2p_model.discover_peers(discovery_method=method, limit=limit)
    ...
    async def dht_find_providers(self, request: DHTFindProvidersRequest):
        ...
        result = await self.libp2p_model.dht_find_providers(
            request.cid, timeout=request.timeout, limit=request.limit
        )
    ...
    async def publish_message(self, request: PubSubPublishRequest):
        ...
        result = await self.libp2p_model.publish_message(
            topic=request.topic, message=request.message
        )
    ...
    async def pubsub_subscribe(self, request: PubSubSubscribeRequest):
        ...
        # Call model method
        result = await self.libp2p_model.pubsub_subscribe(
            request.topic, handler_id=request.handler_id
        )
    ...
    async def pubsub_unsubscribe(self, request: PubSubUnsubscribeRequest):
        ...
        # Call model method
        result = await self.libp2p_model.pubsub_unsubscribe(
            request.topic, handler_id=request.handler_id
        )
    ...
    async def unregister_message_handler(self, request: MessageHandlerRequest):
        ...
        result = await self.libp2p_model.unregister_message_handler(
            handler_id=request.handler_id, protocol_id=request.protocol_id
        )
