...
class ServiceInfo(BaseModel):
    ...
    status: str = Field("starting", description="Service status: starting, active, degraded, failed")
...
class ClusterConfig(BaseModel):
    ...
    primary_selection: str = Field("auto", description="Primary selection mode: auto, manual, failover")
...
class ClusterState(BaseModel):
    ...
    failover_history: List[FailoverEvent] = Field(default_factory=list, description="Failover history")
...
class HighAvailabilityService:
    ...
    def register_with_app(self, app: FastAPI):
        ...
        async def heartbeat(node_id: str, status: Optional[str] = None, load: Optional[Dict[str, float]] = None):
        ...
        async def sync_state(state: ClusterState, background_tasks: BackgroundTasks):
            ...
            # Apply the new state
            async with self.state_lock:
                old_role = self.node_info.role if self.node_id in self.cluster_state.nodes else "unknown"
            ...
            if old_primary != self.cluster_state.primary_node_id:
                logger.info(f"Primary node changed from {old_primary} to {self.cluster_state.primary_node_id}")
        ...
        async def trigger_failover(forced: bool = False, target_node: Optional[str] = None, reason: str = "Manual failover"):
            if not self.cluster_state:
                return {"status": "error", "message": "Cluster not initialized"}
            
            # Only the primary can trigger an orderly failover
            if not forced and self.node_info.role != "primary":
                return {"status": "error", "message": "Only the primary node can trigger a normal failover"}
    ...
    async def stop(self):
        ...
        # Cancel tasks
        for task in [self.heartbeat_task, self.health_check_task, 
                     self.state_sync_task, self.failover_monitor_task]:
            if task:
                task.cancel()
                try:
                    await task
    ...
    async def _join_through_primary(self, primary_address: str) -> bool:
        ...
        try:
            ...
            async with self.http_session.post(
                f"http://{primary_address}/api/v0/ha/join",
                json=self.node_info.dict()
            ) as response:
    ...
    async def _health_check_loop(self):
        ...
        while True:
            ...
            interval = (self.cluster_state.config.heartbeat_interval if self.cluster_state else 10) / 2
            await asyncio.sleep(interval)
            ...
            for node_id, node in list(self.cluster_state.nodes.items()):
                ...
                if time_since_heartbeat > failover_timeout:
                    if node.status != "failed":
                        logger.warning(f"Node {node_id} marked as failed (no heartbeat for {time_since_heartbeat:.1f}s)")
                        node.status = "failed"
                        state_updated = True
                    ...
                    if node.status == "active":
                        logger.warning(f"Node {node_id} marked as degraded (slow heartbeat: {time_since_heartbeat:.1f}s)")
                ...
                if node_id == self.node_id and service_id in self.local_services:
                    ...
                    if service.health_check and node.status == "active":
                        try:
                            ...
                            service_url = f"http://{node.ip_address}:{node.port}{service.health_check}"
    ...
    async def _propagate_state_update(self):
        ...
        for node_id, node in state.nodes.items():
            ...
            try:
                ...
                async with self.http_session.post(
                    f"http://{node_address}/api/v0/ha/sync",
                    json=state.dict()
                ) as response:
                    if response.status != 200:
    ...
    async def _sync_from_primary(self):
        ...
        try:
            ...
            primary_address = f"{primary_node.ip_address}:{primary_node.port}"
            async with self.http_session.get(f"http://{primary_address}/api/v0/ha/cluster") as response:
                if response.status != 200:
                ...
                if self.cluster_state and self.node_info.role != "primary":
                    ...
                    if primary_node_id and primary_node_id in self.cluster_state.nodes:
                        ...
                        if primary_node.status in ["failed", "shutdown"]:
                            logger.warning(f"Primary node {primary_node_id} is {primary_node.status}, initiating failover")
                            
                            # Attempt to elect a new primary
                            await self._elect_new_primary(f"Primary node {primary_node_id} is {primary_node.status}")
            except asyncio.CancelledError:
                break
            except Exception as e:
    ...
    async def _elect_new_primary(self, reason: str):
        ...
        async with self.election_lock:
            ...
            if len(active_nodes) < self.cluster_state.config.quorum_size:
                logger.warning(f"Not enough active nodes for election: {len(active_nodes)} < {self.cluster_state.config.quorum_size}")
                return
            ...
            if new_primary_id:
                ...
                if new_primary_id == self.node_id:
                    ...
                    logger.info(f"Node {new_primary_id} elected as new primary after failover: {event_id}")
    ...
    async def _perform_failover(self, forced: bool, target_node: Optional[str], reason: str) -> Dict[str, Any]:
        """
        ...
        if target_node and target_node not in self.cluster_state.nodes:
            return {"status": "error", "message": f"Target node {target_node} not found in cluster"}
        ...
        if not new_primary:
            # Find active nodes
            active_nodes = [
                node_id for node_id, node in self.cluster_state.nodes.items()
            ...
            if active_nodes:
                ...
                return {"status": "error", "message": "No active nodes available for failover"}
