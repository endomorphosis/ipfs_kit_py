...
def simulate_add(content: Union[str, bytes]) -> Dict[str, Any]:
    ...
    logger.info(f"Using simulated add")
...
def simulate_list_pins() -> Dict[str, Any]:
    ...
    logger.info(f"Using simulated list_pins")
...
def simulate_id() -> Dict[str, Any]:
    """Simulated id method."""
    logger.info(f"Using simulated id")
    return {
        "success": True,
        "operation": "id",
...
def simulate_files_write(path: str, data: Union[str, bytes], create: bool = True, truncate: bool = True) -> Dict[str, Any]:
...
def simulate_block_put(data: Union[str, bytes]) -> Dict[str, Any]:
    ...
    logger.info(f"Using simulated block_put")
    if isinstance(data, str):
        data_bytes = data.encode('utf-8')
    else:
        data_bytes = data
...
def simulate_dht_findprovs(cid: str, num_providers: int = 5) -> Dict[str, Any]:
    ...
    for i in range(num_providers):
        ...
        providers.append({
            "ID": peer_id,
            "Addrs": [f"/ip4/192.168.1.{i}/tcp/4001/p2p/{peer_id}"]
        })
...
def normalize_instance(instance: Any, logger=None) -> Any:
    ...
    instance_methods = [name for name, attr in inspect.getmembers(instance) 
    ...
    instance_methods_after = [name for name, attr in inspect.getmembers(instance) 
                            if callable(attr) and not name.startswith('_')]
    
    logger.debug(f"Instance methods after normalization: {instance_methods_after}")
    logger.info(f"Normalized IPFS instance: added {len(instance_methods_after) - len(instance_methods)} methods")
        ...
        if instance is None:
            ...
            try:
                ...
                try:
                    ...
                    self.logger.error("Could not create IPFS instance, only simulation will be available")
                    instance = None
    ...
    def __getattr__(self, name):
        ...
        def tracked_method(*args, **kwargs):
            ...
            try:
                ...
                if name in ["add", "add_content", "add_file"] and isinstance(result, dict) and "Size" in result:
