...
class MCPFileHandler(FileSystemEventHandler):
    ...
    def _handle_file_change(self, file_path: str):
        ...
        with self.restart_lock:
            ...
            if affected_modules:
                logger.info(f"Change to {file_path} affects {len(affected_modules)} module(s): {', '.join(affected_modules)}")
            ...
            threading.Thread(target=self.restart_callback, args=(file_path, affected_modules)).start()
    ...
    def _format_import_error(self, file_path: str, error_text: str) -> str:
        ...
        if "ModuleNotFoundError: No module named" in error_text:
            ...
            if match:
                ...
                if '.' not in module_name:
                    ...
                    if len(parts) > 1:
                        ...
                        lines.append(f"Check if the module structure matches your import statement.")
            ...
            if match:
                ...
                lines.append(f"Check for typos or incorrect module references.")
    ...
    def _suggest_syntax_fix(self, error: SyntaxError) -> str:
        ...
        if "indentation" in msg:
            if "expected an indented block" in msg:
                ...
                return "Make sure your indentation is consistent (use either spaces or tabs, not both)"
    ...
    def _check_dependencies(self, file_path: str) -> List[str]:
        ...
        try:
            ...
            import_prefixes = [
                ...
                *[f"from {module_path.rsplit('.', i)[0]} import" for i in range(1, module_path.count('.') + 1)]
            ]
                ...
                try:
                    ...
                    for prefix in import_prefixes:
                        if prefix in content:
                            ...
                            import_module = os.path.splitext(rel_import_path)[0].replace(os.path.sep, '.')
    ...
    def _can_import(self, file_path: str) -> tuple:
        ...
        try:
            ...
            import_cmd = f"import sys; sys.path.insert(0, '{file_dir}'); import {module_name}"
...
class MCPFileWatcher:
    ...
    def _print_startup_message(self):
        ...
        divider = "=" * 80
        print(f"\n{divider}")
        print(f"MCP FILE WATCHER STARTED")
        ...
        print(f"{divider}")
        print(f"Auto-restarting server when Python files change.")
        print(f"Syntax errors will be shown in the console.")
        print(f"{divider}\n")
            ...
            if self.server_class:
                ...
                if affected_modules:
                    for module_path in affected_modules:
                        try:
                            if module_path != module_name:  # Skip the main module we already reloaded
                ...
                if hasattr(self.server_instance, '__dict__'):
                    ...
                    common_args = [
                        'debug_mode', 'log_level', 'persistence_path', 
                        'isolation_mode', 'config', 'skip_daemon'
                    ]
