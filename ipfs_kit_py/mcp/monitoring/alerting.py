"""
Alerting and notification system for MCP server.

This module implements the alerting and notification functionality
...
class AlertRule(BaseModel):
    ...
    duration: int = Field(0, description="Duration in seconds the condition must be true before alerting")
    ...
    annotations: Dict[str, str] = Field(default_factory=dict, description="Annotations for the alert")
    enabled: bool = Field(True, description="Whether the rule is enabled")
    silenced_until: Optional[float] = Field(None, description="Timestamp until which the alert is silenced")
    notify_channels: List[str] = Field(default_factory=list, description="Notification channels to use")
...
class AlertInstance(BaseModel):
    ...
    repeat_interval: int = Field(3600, description="Interval in seconds for repeating notifications")
...
class NotificationChannel(BaseModel):
    ...
    filter_severity: List[str] = Field(default_factory=lambda: ["critical", "warning", "info"], 
                                    description="Severities to include")
    filter_labels: Dict[str, str] = Field(default_factory=dict, 
                                        description="Labels that must match for notifications")
...
class AlertingService:
    ...
    async def _create_or_update_alert(self, rule: AlertRule, value: float):
        ...
        if existing_alert:
            ...
            existing_alert.last_notification = None if existing_alert.repeat_notification else existing_alert.last_notification
        else:
            # Create new alert
            import uuid
    ...
    async def _send_resolution_notifications(self, alert: AlertInstance):
        ...
        if rule.notify_channels:
            ...
            # Use all enabled channels
            for channel in self.channels.values():
                if channel.enabled and channel.send_resolved and alert.severity in channel.filter_severity:
                    channels_to_notify.append(channel)
    ...
    async def _send_resolution_notification(self, channel: NotificationChannel, alert: AlertInstance):
        """
    ...
    async def _send_email_notification(self, channel: NotificationChannel, alert: AlertInstance):
        ...
        if not all(k in config for k in ["smtp_server", "smtp_port", "from_address", "to_addresses"]):
            ...
            "attachments": [
                {
                    ...
                    "fields": [
                        ...
                        {
                            "title": "Time",
                            "value": datetime.fromtimestamp(alert.start_time).strftime('%Y-%m-%d %H:%M:%S'),
                            "short": True
                        },
                        {
                            "title": "Value",
            ...
            "attachments": [
                {
                    ...
                    "fields": [
                        ...
                        {
                            "title": "Resolved",
                            "value": datetime.fromtimestamp(alert.end_time).strftime('%Y-%m-%d %H:%M:%S') if alert.end_time else "Unknown",
                            "short": True
                        },
                        {
                            "title": "Duration",
