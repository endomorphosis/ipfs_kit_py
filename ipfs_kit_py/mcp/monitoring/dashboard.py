...
class DashboardService:
    ...
    async def get_dashboard_data(self, dashboard_id: str, time_range: Optional[TimeRange] = None) -> Dict[str, Any]:
        """
        ...
        if time_range:
            ...
            now = time.time()
            if isinstance(default_range, dict) and "from" in default_range and "to" in default_range:
    ...
    async def _get_panel_data(self, panel: Dict[str, Any], time_range: Dict[str, Any]) -> Dict[str, Any]:
        """
    ...
    async def _query_prometheus_range(self, query: str, start: float, end: float, step: str) -> List[Dict[str, Any]]:
        """
    ...
    async def get_label_values(self, label_name: str, metric_name: Optional[str] = None) -> List[str]:
        """
...
def create_dashboard_router(dashboard_service: DashboardService) -> APIRouter:
    ...
    async def get_dashboard(dashboard_id: str):
        ...
        try:
            ...
            if not dashboard:
                return JSONResponse(
                    status_code=404,
                    content={"success": False, "error": f"Dashboard '{dashboard_id}' not found"}
                )
    ...
    async def get_metric_labels(metric: str):
        """Get all label names for a metric."""
        try:
