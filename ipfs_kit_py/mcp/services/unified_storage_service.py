...
class UnifiedStorageService:
            ...
            try:
                ...
                for backend in backends:
                    try:
                        ...
                        logger.warning(f"Error updating content location map for backend {backend}: {e}")
    ...
    async def get_content_from_any(self, cid: str) -> Tuple[Optional[bytes], Optional[str]]:
        ...
        for backend in backends:
            ...
            if content:
                ...
                self.content_location[cid].append({
                    "backend": backend,
                    "info": {"cid": cid}
                })
                return content, backend
    ...
    async def store_content(self, backend: str, content: Union[bytes, BinaryIO, str], 
                           cid: Optional[str] = None) -> Optional[Dict[str, Any]]:
        """
    ...
    async def set_metadata(self, backend: str, cid: str, 
        ...
        try:
            ...
            if hasattr(backend_module, "remove_content"):
                ...
                if cid in self.content_location:
                    ...
                    self.content_location[cid] = [
                        loc for loc in self.content_location[cid]
                        if loc.get("backend") != backend
                    ]
    ...
    async def list_content(self, backend: str, prefix: Optional[str] = None, 
    ...
    async def replicate_content(self, cid: str, source_backend: str, 
        ...
        try:
            ...
            logger.error(f"Error replicating content {cid} from {source_backend} to {target_backend}: {e}")
            return False
    
    async def sync_metadata(self, cid: str, source_backend: str, 
                           target_backends: List[str]) -> Dict[str, bool]:
        """
        ...
        try:
            ...
            for backend in target_backends:
                try:
                    ...
                    if not info:
                        logger.warning(f"Content {cid} not found in {backend}, cannot sync metadata")
