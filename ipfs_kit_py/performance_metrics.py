...
class PerformanceMetrics:
    ...
    def __init__(self, max_history: int = 1000, metrics_dir: Optional[str] = None, 
                 ...
                 track_system_resources: bool = True, retention_days: int = 7):
        """
    ...
    def track_operation(self, operation_name: str, correlation_id: Optional[str] = None):
        ...
        try:
            ...
            self.record_operation_time(operation_name, duration, correlation_id=current_correlation_id)
    ...
    def _calculate_throughput(self):
        ...
        self.throughput['operations_per_second'].append({
            'timestamp': now,
    ...
    def _create_metrics_snapshot(self, timestamp=None):
        ...
        snapshot = {
            ...
            "throughput": {
                "operations_per_second": list(self.throughput["operations_per_second"])[-1]["value"] 
                    if self.throughput["operations_per_second"] else 0,
                "bytes_per_second": list(self.throughput["bytes_per_second"])[-1]["total"] 
                    if self.throughput["bytes_per_second"] else 0
            }
        }
        ...
        if self.track_system_resources and self.system_metrics["cpu"]:
            snapshot["system"] = {
                "cpu": {
                    ...
                    "average": statistics.mean([item["percent"] for item in self.system_metrics["cpu"]])
                },
                "memory": {
                    "current": list(self.system_metrics["memory"])[-1]["percent"],
                    "available_mb": list(self.system_metrics["memory"])[-1]["available"] / (1024 * 1024)
                },
                "disk": {
    ...
    def record_bandwidth_usage(self, direction: str, size_bytes: int, 
                              source: str = None, correlation_id: Optional[str] = None):
        """
    ...
    def record_error(self, operation: str, error: Exception, details: Dict = None,
                    correlation_id: Optional[str] = None):
        """
        ...
        if output_format == "json":
            ...
            for key, value in analysis.get("summary", {}).items():
                if isinstance(value, dict):
                    report.append(f"- **{key.replace('_', ' ').title()}**: {', '.join([f'{k}: {v}' for k, v in value.items()])}")
                else:
            ...
            for op, stats in self.get_operation_stats().get("operations", {}).items():
                if stats.get("count", 0) > 0:
                    report.append(f"| {op} | {stats.get('count')} | {stats.get('avg', 0):.4f} | {stats.get('min', 0):.4f} | {stats.get('max', 0):.4f} | {stats.get('p95', 0):.4f} |")
            ...
            for op, stats in self.get_operation_stats().get("operations", {}).items():
                if stats.get("count", 0) > 0:
                    report.append(f"{op:<20} {stats.get('count'):<8} {stats.get('avg', 0):<10.4f} {stats.get('min', 0):<10.4f} {stats.get('max', 0):<10.4f} {stats.get('p95', 0):<10.4f}")
...
def profile(metrics: PerformanceMetrics, name: Optional[str] = None, 
           correlation_id: Optional[str] = None):
    """
