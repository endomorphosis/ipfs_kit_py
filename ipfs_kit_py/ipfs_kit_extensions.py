...
try:
    from .error import IPFSError, IPFSValidationError, handle_error, create_result_dict
except ImportError:
...
def propose_config_change(self, key, value, **kwargs):
    ...
    try:
        ...
        if not key or not isinstance(key, str):
            return handle_error(result, IPFSValidationError("Configuration key must be a non-empty string"))
            
        # Submit proposal
        proposal_result = self.cluster_manager.propose_configuration_change(
            key=key,
            value=value
        )
