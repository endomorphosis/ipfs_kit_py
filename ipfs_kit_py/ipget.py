...
class ipget:
    def __init__(self, resources=None, metadata=None):
        ...
        try:
            ...
            logger.debug(f"Initialized IPFS ipget with role={self.role}, "
    ...
    def ipget_download_object(self, **kwargs):
        ...
        try:
            ...
            if re.search(r'[;&|"`\'$<>]', cid):
                return handle_error(result, IPFSValidationError(f"CID contains invalid characters: {cid}"))
            
            # Validate path and create parent directory if needed
            if not isinstance(path, str):
                return handle_error(result, IPFSValidationError(f"Path must be a string, got {type(path).__name__}"))
            ...
            try:
                ...
                return handle_error(result, IOError(f"Failed to create directory for download: {str(e)}"))
