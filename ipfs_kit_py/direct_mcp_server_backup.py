    ...
    if DASHBOARD_AVAILABLE:
        ...
        try:
            ...
            async def comprehensive_dashboard():
                """Comprehensive MCP Dashboard with all features."""
                return create_comprehensive_dashboard_html()

            @app.get("/dashboard/{page}", response_class=HTMLResponse) 
            async def dashboard_page(page: str):
                """Dashboard sub-pages.""" 
                return create_comprehensive_dashboard_html(page)

            @app.post("/dashboard/jsonrpc")
            async def dashboard_jsonrpc(request: dict):
                """Enhanced JSON-RPC endpoint for dashboard operations."""
                return await handle_dashboard_jsonrpc(request)

            @app.get("/dashboard/events")
            async def dashboard_events():
                """Server-Sent Events endpoint for real-time updates."""
                return await create_sse_endpoint()

            logger.info("Comprehensive dashboard endpoints added")
