...
needed to decide when a blob is safe to release in a transactional DuckDB
index.  A
...
class ReferenceTracker:
    """Transactional DuckDB index of blobs, revisions, and active leases.

    The connection is deliberately exposed as ``_db`` for the collector's
    tightly-coupled journal operations, but all public mutations use
    :meth:`_transaction`.  A re-entrant lock and transaction-depth counter make
    compound operations such as repair atomic without relying on savepoints,
    which DuckDB does not implement.
    """
        ...
        if self.database != ":memory:":
            ...
            if not parent_existed:
                try:
                    ...
                    pass
        self._db = duckdb.connect(self.database)
        if self.database != ":memory:":
            try:
    ...
    def _create_schema(self) -> None:
        self._db.execute(
            ...
            CREATE TABLE IF NOT EXISTS blobs (
                blob_hash VARCHAR PRIMARY KEY,
                size UBIGINT NOT NULL,
                first_seen DOUBLE NOT NULL,
                unreferenced_at DOUBLE,
                deleted_at DOUBLE
            );
            ...
            CREATE TABLE IF NOT EXISTS namespace_quotas (
                ...
                quota_bytes UBIGINT NOT NULL
            );
            CREATE TABLE IF NOT EXISTS gc_runs (
                run_id VARCHAR PRIMARY KEY,
                marked_at DOUBLE NOT NULL,
                retention_seconds DOUBLE NOT NULL,
                dry_run BOOLEAN NOT NULL,
                policy_json JSON NOT NULL,
                status VARCHAR NOT NULL CHECK(status IN
                    ('marked', 'sweeping', 'dry-run', 'interrupted', 'failed', 'completed')),
                completed_at DOUBLE,
                receipt_json JSON
            );
        ...
        *,
        ...
        with self._transaction():
            ...
            for digest, size in references.items():
                ...
                self._db.execute(
                    "INSERT INTO revision_refs(namespace_id,revision,blob_hash) VALUES(?,?,?) ON CONFLICT DO NOTHING",
        ...
        *,
        ...
        with self._transaction():
            exists = self._db.execute(
                "SELECT 1 FROM revisions WHERE namespace_id=? AND revision=?",
                (namespace_id, revision),
            ).fetchone()
            if exists is None:
                raise KeyError((namespace_id, revision))
            self._db.execute(
                """UPDATE revisions SET active=FALSE, retained_until=?
    ...
    def forget_revision(self, namespace_id: str, revision: int) -> None:
        ...
        with self._transaction():
            self._db.execute(
                "DELETE FROM revision_refs WHERE namespace_id=? AND revision=?",
                (namespace_id, revision),
            )
            self._db.execute(
    ...
    def set_quota(self, namespace_id: str, quota_bytes: int | None) -> None:
        ...
        with self._transaction():
            if quota_bytes is None:
                self._db.execute(
                    "DELETE FROM namespace_quotas WHERE namespace_id=?", (namespace_id,)
                )
            else:
                self._db.execute(
    ...
    def repair(
        ...
        *,
        ...
        with self._transaction():
            ...
            self._db.execute("DELETE FROM revision_refs")
    ...
    def _journal(
        ...
        *,
        ...
        """Append a durable, immutable recovery event for one sweep action."""

        self._db.execute(
            """INSERT INTO gc_recovery_journal
               (journal_id,run_id,blob_hash,operation_id,event_at,state,error_code)
               VALUES(?,?,?,?,?,?,?)""",
            (
                str(uuid.uuid4()),
                run_id,
...
class _Transaction:
    def __init__(self, index: ReferenceTracker) -> None:
        self.index = index
        self.outermost = False

    def __enter__(self) -> None:
        self.index._transaction_lock.acquire()
        self.outermost = self.index._transaction_depth == 0
        if self.outermost:
            self.index._db.execute("BEGIN TRANSACTION")
            self.index._transaction_failed = False
        self.index._transaction_depth += 1

    def __exit__(self, exc_type: Any, *_: Any) -> None:
        try:
            if exc_type is not None:
                self.index._transaction_failed = True
            self.index._transaction_depth -= 1
            if self.outermost:
                command = "ROLLBACK" if self.index._transaction_failed else "COMMIT"
                self.index._db.execute(command)
        finally:
            self.index._transaction_lock.release()
...
class IrohGarbageCollector:
        ...
        *,
        ...
        with self.index._transaction():
            ...
            db.execute(
                ...
                (run_id, now, float(policy.retention_seconds), dry_run, policy_json),
            )
            candidates: list[GCCandidate] = []
            for row in rows:
                operation_id = str(uuid.uuid5(uuid.NAMESPACE_URL, f"iroh-gc:{run_id}:{row[0]}"))
                db.execute(
                    "INSERT INTO gc_actions VALUES(?,?,?,?,?,'marked',NULL)",
    ...
    @staticmethod
    def _apply_limits(
        rows: Sequence[tuple[Any, ...]], policy: GCPolicy, usage: int
    ) -> list[tuple[Any, ...]]:
    ...
    async def _sweep_locked(self, run_id: str, *, dry_run: bool | None) -> GCReceipt:
        db = self.index._db
        run = db.execute(
            """SELECT run_id,marked_at,retention_seconds,dry_run,policy_json,
                      status,completed_at,CAST(receipt_json AS VARCHAR)
               FROM gc_runs WHERE run_id=?""",
            (run_id,),
        ).fetchone()
        if run is None:
        ...
        reclaimed = 0
        with self.index._transaction():
            db.execute("UPDATE gc_runs SET status='sweeping' WHERE run_id=?", (run_id,))
        for row in rows:
            if row[5] == "deleted":
            ...
            with self.index._transaction():
                ...
                protected = (
                    ...
                    or self.index._is_protected(row[1], now)
                )
                ...
                if protected or changed:
                    ...
                    continue
                db.execute(
                    "UPDATE gc_actions SET state='deleting' WHERE run_id=? AND blob_hash=?",
                    (run_id, row[1]),
                )
                self.index._journal(run_id, row[1], row[4], "prepared")
            try:
                ...
                if not _release_confirmed(result, row[1]):
                    ...
                    continue
                with self.index._transaction():
                    self.index._journal(run_id, row[1], row[4], "released")
            except asyncio.CancelledError:
                with self.index._transaction():
                    db.execute("UPDATE gc_runs SET status='interrupted' WHERE run_id=?", (run_id,))
                    self.index._journal(run_id, row[1], row[4], "interrupted")
                raise
            except Exception as exc:  # one failed blob must not hide other safe candidates
                code = _failure_code(exc)
                with self.index._transaction():
                    db.execute(
                        "UPDATE gc_actions SET state='failed',error_code=? WHERE run_id=? AND blob_hash=?",
                        (code, run_id, row[1]),
                    )
                    self.index._journal(run_id, row[1], row[4], "failed", error_code=code)
                failures.append(GCFailure(row[1], code))
                continue
            with self.index._transaction():
                ...
                now = self.clock()
                if self.index._is_protected(row[1], now):
                    db.execute(
                        "UPDATE gc_actions SET state='skipped' WHERE run_id=? AND blob_hash=?",
                        (run_id, row[1]),
                    )
                    ...
                    self.index._journal(run_id, row[1], row[4], "committed")
                    deleted.append(row[1])
                    reclaimed += row[2]
