...
from ipfs_kit_py.error import (
    ...
    IPFSValidationError, IPFSConfigurationError
)
...
class IPFSLibp2pPeer:
    ...
    def __init__(self, 
        ...
        self.enable_relay_server = (role in ["master", "worker"]) and enable_relay and HAS_NAT_TRAVERSAL
    ...
    async def _setup_pubsub_async(self):
        ...
        if self.role == "master":
            ...
            await self.pubsub.subscribe(
                topic_id="ipfs-kit/workers",
                handler=self._handle_worker_updates
            )
            
            await self.pubsub.subscribe(
                topic_id="ipfs-kit/content-announce",
                handler=self._handle_content_announcements
            )
            
            # Master also subscribes to relay announcements
            await self.pubsub.subscribe(
                topic_id="ipfs-kit/relay-announce",
                handler=self._handle_relay_announcements
            )
            
        elif self.role == "worker":
            # Worker subscribes to task assignments and content requests
            await self.pubsub.subscribe(
                topic_id="ipfs-kit/tasks",
                handler=self._handle_task_assignments
            )
            
            await self.pubsub.subscribe(
                topic_id="ipfs-kit/content-requests",
                handler=self._handle_content_requests
            )
    ...
    async def _handle_bitswap_want(self, stream, request):
        ...
        if "cid" not in request:
            await stream.write(json.dumps({
                "type": "error",
                "error": "Missing CID in want request"
            }).encode())
            return
        ...
        if stored_content:
            ...
            self.logger.debug(f"Sent content for CID {cid} from tiered storage ({len(stored_content)} bytes)")
        ...
        if self.role in ["master", "worker"]:
            ...
            if providers:
                ...
                self.logger.debug(f"Sent provider list for CID {cid} with {len(providers)} providers")
    ...
    async def _handle_bitswap_have(self, stream, request):
        ...
        if "cid" not in request:
            await stream.write(json.dumps({
                "type": "error",
                "error": "Missing CID in have request"
            }).encode())
            return
    ...
    async def _handle_bitswap_cancel(self, stream, request):
        ...
        if "cid" not in request:
            await stream.write(json.dumps({
                "type": "error",
                "error": "Missing CID in cancel request"
            }).encode())
            return
    ...
    def _setup_pubsub_discovery(self, rendezvous_string: str) -> None:
        ...
        self.pubsub.subscribe(
            ...
            handler=self._handle_discovery_message
        )
    ...
    def _announce_to_discovery_topic(self, topic: str) -> None:
        ...
        threading.Timer(
            300,  # 5 minutes
            lambda: self._announce_to_discovery_topic(topic)
        ).start()
    ...
    def _setup_random_walk_discovery(self) -> None:
        ...
        self._random_walk_thread = threading.Thread(
    ...
    def enable_relay(self) -> bool:
        ...
        try:
            ...
            if self.role in ["master", "worker"]:
                ...
                self.host.set_stream_handler("/libp2p/circuit/relay/hop/1.0.0", self._handle_relay_hop)
                self.host.peerstore.add_supported_protocol(self.host.get_id(), "/libp2p/circuit/relay/hop/1.0.0")
    ...
    async def _handle_relay(self, stream) -> None:
        ...
        try:
            ...
            if self.enable_relay_server:
                ...
                await stream.write(json.dumps({
                    "status": "error",
                    "error": "This node is not a relay server"
                }).encode())
    ...
    async def _handle_relay_hop(self, stream) -> None:
        ...
        try:
            ...
            if self.role in ["master", "worker"] and self.enable_relay_server:
                ...
                await stream.write(json.dumps({
                    "status": "error",
                    "error": "This node cannot serve as a relay hop"
                }).encode())
    ...
    def store_bytes(self, cid: str, data: bytes) -> bool:
        ...
        try:
            with self._lock:
                ...
                self.content_metadata[cid].update({
                    "size": len(data),
                    "stored_at": time.time()
                })
    ...
    def find_providers(self, cid: str, count: int = 20, timeout: int = 60) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            if len(providers) < count and self.dht:
                ...
                asyncio.set_event_loop(loop)
                try:
                    dht_result = loop.run_until_complete(asyncio.wait_for(find_in_dht(), timeout=timeout))
    ...
    def request_content(self, cid: str, timeout: int = 30) -> Optional[bytes]:
        ...
        for provider in providers:
            try:
                    ...
                    try:
                        ...
                        try:
                            ...
                            if isinstance(error_check, dict) and "error" in error_check:
                                self.logger.warning(f"Provider returned error: {error_check['error']}")
                                return None
                        except json.JSONDecodeError:
                ...
                asyncio.set_event_loop(loop)
                try:
                    content = loop.run_until_complete(asyncio.wait_for(request_from_peer(), timeout=timeout))
        ...
        try:
            ...
            async def receive_stream():
                ...
                stream = await self.host.new_stream(
                    ...
                    protocol_id=PROTOCOLS["FILE_EXCHANGE"]
                )
    ...
    def _send_content_response(self, requester: str, cid: str, request_id: Optional[str] = None) -> bool:
        ...
        try:
            ...
            async def send_response():
                # Create stream
                stream = await self.host.new_stream(
                    peer_id=PeerID.from_base58(requester),
                    protocol_id=PROTOCOLS["BITSWAP"]
                )
    ...
    async def _find_providers_async(self, cid: str, count: int = 5, timeout: int = 5) -> List[Dict[str, Any]]:
                ...
                try:
                    ...
                    try:
                        ...
                        if header.get("type") == "block":
                            ...
                            if content:
                                self.store_bytes(cid, content)
                                self.logger.info(f"Successfully fetched content {cid} from {provider_id}")
                                await stream.close()
                                return True
                    except json.JSONDecodeError:
