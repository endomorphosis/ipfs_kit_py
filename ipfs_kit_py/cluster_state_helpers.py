...
def find_nodes_with_gpu(state_path: str) -> List[Dict[str, Any]]:
    ...
    return [
        node for node in nodes 
        if (node.get("resources", {}).get("gpu_count", 0) > 0 and 
            node.get("resources", {}).get("gpu_available", False))
    ]
...
def find_content_by_provider(state_path: str, provider_id: str) -> List[Dict[str, Any]]:
    ...
    return [
        ...
        if "providers" in item and provider_id in item["providers"]
    ]
    ...
    try:
        ...
        for node in nodes:
            ...
            if "resources" in node:
                ...
                available_memory_gb += node["resources"].get("memory_available", 0) / (1024 * 1024 * 1024)
        ...
        return {
            ...
            "resources": {
                ...
                "gpu_cores": {
                    "total": total_gpu_cores,
                    "available": available_gpu_cores
                },
                "memory_gb": {
            ...
            "content": {
                "total": len(content),
                "total_size_gb": sum(item.get("size", 0) for item in content) / (1024 * 1024 * 1024)
            }
        }
...
def find_tasks_by_resource_requirements(state_path: str, 
                                       cpu_cores: Optional[int] = None,
                                       gpu_cores: Optional[int] = None,
                                       memory_mb: Optional[int] = None) -> List[Dict[str, Any]]:
    """
...
def find_available_node_for_task(state_path: str, task_id: str) -> Optional[Dict[str, Any]]:
    ...
    for node in online_workers:
        ...
        if required_gpu > 0 and (
            node_resources.get("gpu_count", 0) < required_gpu or 
            not node_resources.get("gpu_available", False)
        ):
            continue
            
        if node_resources.get("memory_available", 0) < required_memory * 1024 * 1024:  # Convert to bytes
            continue
        ...
        memory_util = node_resources.get("memory_available", 0) / max(node_resources.get("memory_total", 1), 1)
...
def get_node_resource_utilization(state_path: str, node_id: str) -> Optional[Dict[str, float]]:
    """
    Calculate resource utilization metrics for a specific node.
    ...
    Returns:
        Dictionary with resource utilization metrics if found, None if node not found
    """
    # Get the node
    node = get_node_by_id(state_path, node_id)
    if not node:
        return None
    
    # Get all tasks assigned to this node
    node_tasks = find_tasks_by_node(state_path, node_id)
    ...
    cpu_load = resources.get("cpu_load", 0)  # As percentage
    cpu_utilization = cpu_load / 100.0  # Convert to 0-1 scale
    
    # Calculate memory utilization
    memory_total = resources.get("memory_total", 1)
    memory_available = resources.get("memory_available", 0)
    memory_used = memory_total - memory_available
    memory_utilization = memory_used / memory_total if memory_total > 0 else 1.0
    
    # Calculate GPU utilization if applicable
    gpu_count = resources.get("gpu_count", 0)
    gpu_memory_total = resources.get("gpu_memory_total", 0)
    gpu_memory_used = resources.get("gpu_memory_used", 0)
    gpu_utilization = gpu_memory_used / gpu_memory_total if gpu_memory_total > 0 else 0.0
    
    # Calculate disk utilization
    disk_total = resources.get("disk_total", 1)
    disk_available = resources.get("disk_available", 0)
    disk_used = disk_total - disk_available
    disk_utilization = disk_used / disk_total if disk_total > 0 else 1.0
    
    # Count active vs completed tasks
    active_tasks = len([t for t in node_tasks if t.get("status") in ["assigned", "running"]])
    completed_tasks = len([t for t in node_tasks if t.get("status") == "completed"])
    failed_tasks = len([t for t in node_tasks if t.get("status") == "failed"])
    
    # Calculate success rate
    total_completed = completed_tasks + failed_tasks
    success_rate = completed_tasks / total_completed if total_completed > 0 else 1.0
    
    return {
        "cpu_utilization": cpu_utilization,
        "memory_utilization": memory_utilization,
        "disk_utilization": disk_utilization,
        "gpu_utilization": gpu_utilization if gpu_count > 0 else None,
        "active_tasks": active_tasks,
        "completed_tasks": completed_tasks,
        "failed_tasks": failed_tasks,
        "success_rate": success_rate,
        "load_index": (cpu_utilization * 0.4 + memory_utilization * 0.4 + disk_utilization * 0.2)
    }
...
def estimate_time_to_completion(state_path: str, task_id: str) -> Optional[float]:
    ...
    similar_tasks = [
        ...
        if t.get("type") == task.get("type") and 
    ...
    for node in nodes:
        ...
        topology_nodes.append({
            ...
            "resources": {
                ...
                "memory_gb": node.get("resources", {}).get("memory_total", 0) / (1024 * 1024 * 1024),
...
def get_content_availability_map(state_path: str) -> Dict[str, List[str]]:
    """
    Create a mapping of content CIDs to the nodes that have the content available.
    
    Args:
        state_path: Path to the cluster state directory
        
    Returns:
        Dictionary mapping content CIDs to lists of node IDs
    """
    # Get content and nodes
    content_items = get_all_content(state_path)
    nodes = get_all_nodes(state_path)
    
    if not content_items or not nodes:
        return {}
    ...
    for item in content_items:
        ...
        # Add to mapping
        availability_map[cid] = providers
