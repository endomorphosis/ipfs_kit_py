...
class IPFSKitBenchmark:
    ...
    def __init__(self, metrics_dir=None, role="leecher", daemon_port=None,
                parallelism=1, iterations=5, warmup=1):
        """
        ...
        try:
            ...
            for name, (path, content) in files.items():
                ...
                for i in range(self.iterations):
                    with ProfilingContext(self.metrics, f"add_{name}", self.correlation_id) as profile:
            ...
            for name, info in add_results.items():
                ...
                # Timed iterations
                for i in range(self.iterations):
                    with ProfilingContext(self.metrics, f"cat_{name}", self.correlation_id) as profile:
            ...
            for name, info in add_results.items():
                ...
                for i in range(self.iterations):
                    ...
                    with ProfilingContext(self.metrics, f"pin_{name}", self.correlation_id) as profile:
            ...
            for name, info in add_results.items():
                ...
                for i in range(self.iterations):
                    ...
                    with ProfilingContext(self.metrics, f"unpin_{name}", self.correlation_id) as profile:
            ...
            for name, batch in batch_files.items():
                ...
                # Timed iterations
                for i in range(self.iterations):
                    with ProfilingContext(self.metrics, f"batch_add_{name}", self.correlation_id) as profile:
    ...
    def benchmark_caching(self):
        ...
        try:
            ...
            sequential_results = self._run_cache_access_pattern(
                cids, "sequential", iterations=5
            )
            results["sequential"] = sequential_results
            
            # Test 2: Random access pattern
            random_results = self._run_cache_access_pattern(
                cids, "random", iterations=5
            )
            results["random"] = random_results
            
            # Test 3: Repeated access to a small set (hot spot)
            repeated_results = self._run_cache_access_pattern(
                cids, "repeated", iterations=5
            )
            ...
            for name, info in cids.items():
                ...
                with ProfilingContext(self.metrics, f"promotion_first_{name}", self.correlation_id) as profile:
                    content = self.kit.ipfs_cat(cid)
    ...
    def _run_cache_access_pattern(self, cids, pattern, iterations=5):
        ...
        for i, name in enumerate(access_sequence):
            ...
            # Access the content
            with ProfilingContext(self.metrics, f"cache_{pattern}_{name}", self.correlation_id) as profile:
                content = fs.cat(cid)
    ...
    def benchmark_fsspec_operations(self):
        ...
        try:
            ...
            for name, info in cids.items():
                ...
                open_timings = []
                for i in range(self.iterations):
                    with ProfilingContext(self.metrics, f"fsspec_open_{name}", self.correlation_id) as profile:
                ...
                with self.fs.open(path, "rb") as f:
                    for i in range(self.iterations):
                        ...
                        with ProfilingContext(self.metrics, f"fsspec_read_small_{name}", self.correlation_id) as profile:
                ...
                for i in range(self.iterations):
                    with self.fs.open(path, "rb") as f:
                        with ProfilingContext(self.metrics, f"fsspec_read_full_{name}", self.correlation_id) as profile:
                            content = f.read()
                ...
                with self.fs.open(path, "rb") as f:
                    for i in range(self.iterations):
                        ...
                        for pos in positions:
                            with ProfilingContext(self.metrics, f"fsspec_seek_{name}", self.correlation_id) as profile:
                                f.seek(pos)
            ...
            with tempfile.TemporaryDirectory() as tmpdir:
                ...
                if result and "Hash" in result:
                    ...
                    # Test ls operation
                    with ProfilingContext(self.metrics, "fsspec_ls_root", self.correlation_id) as profile:
                        root_listing = self.fs.ls(path)
                    
                    # Test ls on subdirectory
                    with ProfilingContext(self.metrics, "fsspec_ls_subdir", self.correlation_id) as profile:
                        subdir_listing = self.fs.ls(f"{path}/dir1")
                    
                    # Test glob operation
                    with ProfilingContext(self.metrics, "fsspec_glob", self.correlation_id) as profile:
                        glob_result = self.fs.glob(f"{path}/**/*.txt")
                    
                    # Test find operation
                    with ProfilingContext(self.metrics, "fsspec_find", self.correlation_id) as profile:
    ...
    def benchmark_parallel_operations(self):
        ...
        try:
            ...
            for parallelism in [1, 2, 4, 8]:
                ...
                with ProfilingContext(self.metrics, f"parallel_add_{parallelism}", self.correlation_id) as profile:
            ...
            for parallelism in [1, 2, 4, 8]:
                ...
                with ProfilingContext(self.metrics, f"parallel_cat_{parallelism}", self.correlation_id) as profile:
                    # Use ThreadPoolExecutor to run parallel operations
                    import concurrent.futures
                    with concurrent.futures.ThreadPoolExecutor(max_workers=parallelism) as executor:
...
def main():
    ...
    parser.add_argument("--role", default="leecher", choices=["master", "worker", "leecher"],
                       help="IPFS node role to use")
    parser.add_argument("--daemon-port", type=int, help="IPFS daemon API port")
    parser.add_argument("--parallelism", type=int, default=2,
                       help="Number of parallel operations to run")
    parser.add_argument("--iterations", type=int, default=5,
                       help="Number of iterations for each benchmark")
    parser.add_argument("--warmup", type=int, default=1,
                       help="Number of warmup iterations before timing")
    parser.add_argument("--benchmark", choices=["all", "core", "caching", "fsspec", "parallel"],
