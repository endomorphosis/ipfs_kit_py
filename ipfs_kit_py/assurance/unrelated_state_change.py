"""Fail-closed PCPR-066 Kit binding to Accelerate unrelated change.

Kit owns exact bytes and the current root. This module binds the
PCPR-062 hermetic current root to the Accelerate-owned unrelated
documentation change without reminting pack, root, route, patch, or run
identities, without storing the change as live durable bytes, and
...
INTERFACE: Final = "KitUnrelatedStateChangeBinding@1"
SCHEMA: Final = "ipfs_kit_py/assurance/unrelated-state-change-binding@1"
BINDING_SCHEMA: Final = (
    "ipfs_kit_py/assurance/declared-unrelated-state-change-binding@1"
)
VERDICT_SCHEMA: Final = (
    "ipfs_kit_py/assurance/unrelated-state-change-verdict@1"
)
...
HERMETIC_CANDIDATE_SUITES: Final[tuple[str, ...]] = (
    "tests/test_pcpr_066_unrelated_state_change.py",
)
...
BINDING_README: Final = """# PCPR-066 Kit unrelated-state-change binding
...
class KitUnrelatedStateChangeError(Exception):
    """Kit attempted to remint or store the unrelated change as live bytes."""


def _text(value: Any, name: str) -> str:
    if not isinstance(value, str) or not value.strip():
        raise KitUnrelatedStateChangeError(
...
def parse_pyproject_unrelated_state_change_table(text: str) -> dict[str, Any]:
    ...
    if not isinstance(table, dict):
        raise KitUnrelatedStateChangeError("pyproject.toml must be a table")
    tool = table.get("tool")
    payload: dict[str, Any] = {}
    if isinstance(tool, dict):
        kit = tool.get("ipfs_kit_py")
        if isinstance(kit, dict):
            raw = kit.get("unrelated-state-change")
...
@dataclass(frozen=True)
class KitUnrelatedStateChangeVerdict:
    schema: str
    interface: str
...
def _operator_blocking_task() -> dict[str, Any]:
    return {
        ...
        "requires": (
            ...
            "unrelated documentation change is stored as live durable bytes"
        ),
        "action": (
        ...
        "reason": (
            "Kit binds the Accelerate hermetic unrelated documentation "
...
def render_declared_binding(root: Path | None = None) -> dict[str, Any]:
    ...
    if package_root is None:
        raise KitUnrelatedStateChangeError("Kit package root was not found")
    _ = package_root
...
def current_head_static_probes(
    ...
    probes = [
        _probe(
            ...
            reason=(
                "Committed Kit unrelated-state-change binding matches the generator."
                if verified.get("binding_ok") is True
                else "Committed Kit unrelated-state-change binding is missing or drifts."
            ),
        ),
        _probe(
            "pyproject_unrelated_state_change_table",
            ...
            reason=(
                "pyproject.toml declares KitUnrelatedStateChangeBinding@1."
        ...
        _probe(
            ...
            reason="Kit does not store the unrelated change as live durable bytes.",
        ),
        _probe(
            "model_assertion_cannot_complete_work",
        ...
        _probe(
            ...
            None,
            evidence_kind="unavailable",
            reason="Live storage of the unrelated change stays typed unavailable.",
        ),
    ]
    return tuple(probes)
...
def qualify_unrelated_state_change(
    ...
    *,
    ...
    objective_cid: str,
    idea_digest_cid: str,
) -> KitUnrelatedStateChangeVerdict:
    if not probes:
        raise KitUnrelatedStateChangeError("at least one probe is required")
    ...
    return KitUnrelatedStateChangeVerdict(
        schema=VERDICT_SCHEMA,
        interface=INTERFACE,
        promotion_status=promotion_status,
        supervisor_disposition="supervisor_non_promoted",
...
def pcpr_066_receipt_promotion(
    verdict: KitUnrelatedStateChangeVerdict,
) -> dict[str, Any]:
...
PINNED_BINDING_CID: Final = (
    "baguqeeraik4y53xdkd4goynf4tvjanyhe5fziptl4zpraafrbj6aia5dcwrq"
)
CURRENT_HEAD_NON_PROMOTION_VERDICT_CID: Final = (
    "baguqeerante64opokyjospmn53sw5gga5d63ofpy4drmzzwnnt4nqnstsnza"
)
