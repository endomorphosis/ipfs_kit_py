"""Fail-closed PCPR-090 Kit binding to Accelerate threat model.

Kit owns exact bytes and the current root. This module binds the
PCPR-062 hermetic current root to the Accelerate-owned threat model
without reminting pack, root, chain, client, parity, or bypass
identities, without storing the model as live durable bytes, and without
deciding semantic soundness.

Durable reconstruction of the current root is from disk, not memory.
Threat-model replay is identity-preserving. This module does not import
...
INTERFACE: Final = "KitThreatModelBinding@1"
SCHEMA: Final = "ipfs_kit_py/assurance/threat-model-binding@1"
BINDING_SCHEMA: Final = "ipfs_kit_py/assurance/declared-threat-model-binding@1"
VERDICT_SCHEMA: Final = "ipfs_kit_py/assurance/threat-model-verdict@1"
...
HERMETIC_CANDIDATE_SUITES: Final[tuple[str, ...]] = (
    "tests/test_pcpr_090_threat_model.py",
)
...
REQUIRED_GOOD_PROBE_IDS: Final[frozenset[str]] = frozenset(
    {
        ...
        "threat_model_owned_by_accelerate",
        "current_root_survives_threat_model_from_disk",
        "threat_model_not_stored_as_live_bytes",
...
BINDING_README: Final = """# PCPR-090 Kit threat-model binding
...
class KitThreatModelError(Exception):
    """Kit attempted to remint or store the threat model as live bytes."""


def _text(value: Any, name: str) -> str:
    if not isinstance(value, str) or not value.strip():
        raise KitThreatModelError(f"{name} must be a non-empty string")
    return value.strip()
...
def _kind(value: Any, name: str) -> str:
    ...
    if kind not in EVIDENCE_KINDS:
        raise KitThreatModelError(f"{name} is not an admitted evidence kind")
    return kind


def _reject_closed_release_value(value: Any, name: str) -> None:
    if isinstance(value, str) and value in CLOSED_RELEASE_OUTCOMES:
        raise KitThreatModelError(
...
def parse_pyproject_threat_model_table(text: str) -> dict[str, Any]:
    ...
    if not isinstance(table, dict):
        raise KitThreatModelError("pyproject.toml must be a table")
    tool = table.get("tool")
    payload: dict[str, Any] = {}
    if isinstance(tool, dict):
        kit = tool.get("ipfs_kit_py")
        if isinstance(kit, dict):
            raw = kit.get("threat-model")
...
def refuse_live_storage(*, stored: bool) -> None:
    if stored:
        raise KitThreatModelError(
            "Kit must not store the hermetic threat model as live durable bytes"
        )
...
def _operator_blocking_task() -> dict[str, Any]:
    return {
        ...
        "action": (
            "Keep the Kit current root bound to the Accelerate threat model. "
            "Do not store the model as live durable bytes. Do not write "
            "DuckDB or Quack state."
        ),
        "reason": (
            "Kit binds the Accelerate hermetic threat model. That binding is "
...
def render_declared_binding(root: Path | None = None) -> dict[str, Any]:
    ...
    if package_root is None:
        raise KitThreatModelError("Kit package root was not found")
    _ = package_root
    ...
    document = {
        ...
        "live_application": typed_unavailable(
            reason=(
                "Kit binds the Accelerate threat model and does not emit a "
                "live Supervisor.run."
            )
        ),
        ...
        "live_storage": typed_unavailable(
            reason="Kit does not store the hermetic threat model as live durable bytes."
        ),
        "source": {
            "kind": "git",
...
def current_head_static_probes(start: Path | None = None) -> tuple[OutcomeProbe, ...]:
    ...
    probes = [
        _probe(
            ...
            reason=(
                "Committed Kit threat-model binding matches the generator."
                if verified.get("binding_ok") is True
                else "Committed Kit threat-model binding is missing or drifts."
            ),
        ),
        _probe(
            "pyproject_threat_model_table",
            ...
            reason=(
                "pyproject.toml declares KitThreatModelBinding@1."
                if table.get("interface") == INTERFACE
                else "pyproject.toml does not declare the PCPR-090 binding."
            ),
        ),
        _probe(
            "owner_pack_cid_matches_pin",
        ...
        _probe(
            "current_root_survives_threat_model_from_disk",
            binding["storage"]["survives_threat_model"] is True
            and binding["storage"]["reconstructed_from"] == "disk"
            and binding["storage"]["reconstructed_from_memory"] is False,
            reason="The current root survives the threat model and is reconstructed from disk.",
        ),
        _probe(
            "threat_model_not_stored_as_live_bytes",
...
def qualify_threat_model(
    ...
    *,
    ...
    objective_cid: str,
    idea_digest_cid: str,
) -> KitThreatModelVerdict:
    if not probes:
        raise KitThreatModelError("at least one probe is required")
...
PINNED_BINDING_CID: Final = (
    "baguqeeranyqsorlvpdhsnf6oidawm6x3uqg6334hku35ikazxzg4omksjesa"
)
CURRENT_HEAD_NON_PROMOTION_VERDICT_CID: Final = (
    "baguqeera67csowvo5k2id5e2zszi4gkaa34rsmdg5pz7etgln7ax7ye6ix5q"
)
