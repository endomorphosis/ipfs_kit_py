"""Fail-closed PCPR-091 Kit binding to Accelerate TCB inventory.

Kit owns exact bytes and the current root. This module binds the
PCPR-062 hermetic current root to the Accelerate-owned trusted
computing base without reminting pack, root, chain, client, parity,
bypass, or threat-model identities, without storing the inventory as
live durable bytes, and without deciding semantic soundness.

Durable reconstruction of the current root is from disk, not memory.
TCB-inventory replay is identity-preserving. This module does not import
...
INTERFACE: Final = "KitTrustedComputingBaseBinding@1"
SCHEMA: Final = "ipfs_kit_py/assurance/trusted-computing-base-binding@1"
BINDING_SCHEMA: Final = (
    "ipfs_kit_py/assurance/declared-trusted-computing-base-binding@1"
)
VERDICT_SCHEMA: Final = "ipfs_kit_py/assurance/trusted-computing-base-verdict@1"
...
HERMETIC_CANDIDATE_SUITES: Final[tuple[str, ...]] = (
    "tests/test_pcpr_091_trusted_computing_base.py",
)
...
REQUIRED_GOOD_PROBE_IDS: Final[frozenset[str]] = frozenset(
    {
        ...
        "tcb_inventory_owned_by_accelerate",
        "current_root_survives_tcb_inventory_from_disk",
        "tcb_inventory_not_stored_as_live_bytes",
...
BINDING_README: Final = """# PCPR-091 Kit trusted-computing-base binding
...
class KitTrustedComputingBaseError(Exception):
    """Kit attempted to remint or store the TCB inventory as live bytes."""


def _text(value: Any, name: str) -> str:
    if not isinstance(value, str) or not value.strip():
        raise KitTrustedComputingBaseError(f"{name} must be a non-empty string")
    return value.strip()
...
def _kind(value: Any, name: str) -> str:
    ...
    if kind not in EVIDENCE_KINDS:
        raise KitTrustedComputingBaseError(f"{name} is not an admitted evidence kind")
    return kind


def _reject_closed_release_value(value: Any, name: str) -> None:
    if isinstance(value, str) and value in CLOSED_RELEASE_OUTCOMES:
        raise KitTrustedComputingBaseError(
...
def parse_pyproject_tcb_inventory_table(text: str) -> dict[str, Any]:
    ...
    if not isinstance(table, dict):
        raise KitTrustedComputingBaseError("pyproject.toml must be a table")
    tool = table.get("tool")
    payload: dict[str, Any] = {}
    if isinstance(tool, dict):
        kit = tool.get("ipfs_kit_py")
        if isinstance(kit, dict):
            raw = kit.get("trusted-computing-base")
...
def refuse_live_storage(*, stored: bool) -> None:
    if stored:
        raise KitTrustedComputingBaseError(
            "Kit must not store the hermetic TCB inventory as live durable bytes"
        )
...
@dataclass(frozen=True)
class KitTrustedComputingBaseVerdict:
    schema: str
    interface: str
...
def _operator_blocking_task() -> dict[str, Any]:
    return {
        ...
        "action": (
            "Keep the Kit current root bound to the Accelerate TCB inventory. "
            "Do not store the inventory as live durable bytes. Do not write "
            "DuckDB or Quack state."
        ),
        "reason": (
            "Kit binds the Accelerate hermetic TCB inventory. That binding is "
...
def render_declared_binding(root: Path | None = None) -> dict[str, Any]:
    ...
    if package_root is None:
        raise KitTrustedComputingBaseError("Kit package root was not found")
    _ = package_root
    ...
    document = {
        ...
        "live_application": typed_unavailable(
            reason=(
                "Kit binds the Accelerate TCB inventory and does not emit a "
                "live Supervisor.run."
            )
        ),
        ...
        "live_storage": typed_unavailable(
            reason="Kit does not store the hermetic TCB inventory as live durable bytes."
        ),
        "source": {
            "kind": "git",
...
def current_head_static_probes(start: Path | None = None) -> tuple[OutcomeProbe, ...]:
    ...
    probes = [
        _probe(
            ...
            reason=(
                "Committed Kit TCB-inventory binding matches the generator."
                if verified.get("binding_ok") is True
                else "Committed Kit TCB-inventory binding is missing or drifts."
            ),
        ),
        _probe(
            "pyproject_tcb_inventory_table",
            ...
            reason=(
                "pyproject.toml declares KitTrustedComputingBaseBinding@1."
                if table.get("interface") == INTERFACE
                else "pyproject.toml does not declare the PCPR-091 binding."
            ),
        ),
        _probe(
            "owner_pack_cid_matches_pin",
        ...
        _probe(
            "current_root_survives_tcb_inventory_from_disk",
            binding["storage"]["survives_tcb_inventory"] is True
            and binding["storage"]["reconstructed_from"] == "disk"
            and binding["storage"]["reconstructed_from_memory"] is False,
            reason="The current root survives the TCB inventory and is reconstructed from disk.",
        ),
        _probe(
            "tcb_inventory_not_stored_as_live_bytes",
...
def qualify_tcb_inventory(
    ...
    *,
    ...
    objective_cid: str,
    idea_digest_cid: str,
) -> KitTrustedComputingBaseVerdict:
    if not probes:
        raise KitTrustedComputingBaseError("at least one probe is required")
    ...
    return KitTrustedComputingBaseVerdict(
        schema=VERDICT_SCHEMA,
        interface=INTERFACE,
        promotion_status=promotion_status,
        supervisor_disposition="supervisor_non_promoted",
...
def pcpr_091_receipt_promotion(
    verdict: KitTrustedComputingBaseVerdict,
) -> dict[str, Any]:
...
PINNED_BINDING_CID: Final = (
    "baguqeera3y65uiessnkkz7wk4bffzav6msjfzqac4nzvzsccvtexftcy5p7a"
)
CURRENT_HEAD_NON_PROMOTION_VERDICT_CID: Final = (
    "baguqeera2chgarl5gqzldjrtaf2m2347echoefcjyjlxynjrtntooljptgvq"
)
