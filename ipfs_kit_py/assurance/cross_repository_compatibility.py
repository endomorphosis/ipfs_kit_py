"""Fail-closed PCPR-043 Kit binding to cross-repository compatibility.

Kit owns DurableArtifactReceipt. This module binds the PCPR-043
compatibility document CID published by the Accelerate-owned checker and
refuses reminting. Kit verifies and stores exact bytes; it does not
...
is not a freeze, not the signed portfolio lock (PCPR-056), not a closed
...
INTERFACE: Final = "KitCrossRepositoryCompatibilityBinding@1"
SCHEMA: Final = (
...
class KitCrossRepositoryCompatibilityError(ValueError):
    """Kit attempted to remint a compatibility identity."""


def refuse_compatibility_remint(cid: str) -> str:
    if cid != PINNED_COMPATIBILITY_DOCUMENT_CID:
        raise KitCrossRepositoryCompatibilityError(
            f"compatibility document CID {cid} remints {PINNED_COMPATIBILITY_DOCUMENT_CID}"
        )
    return cid


def kit_compatibility_binding_mapping() -> dict[str, object]:
    return {
        "schema": SCHEMA,
        "interface": INTERFACE,
