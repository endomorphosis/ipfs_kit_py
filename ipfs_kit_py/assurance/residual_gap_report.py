"""Fail-closed PCPR-095 Kit binding to Accelerate residual-gap report receipt.
...
storing the residual-gap report as live durable bytes, and without deciding
semantic soundness.
...
INTERFACE: Final = "KitResidualGapReportBinding@1"
SCHEMA: Final = "ipfs_kit_py/assurance/residual-gap-report-binding@1"
BINDING_SCHEMA: Final = (
    "ipfs_kit_py/assurance/declared-residual-gap-report-binding@1"
)
VERDICT_SCHEMA: Final = "ipfs_kit_py/assurance/residual-gap-report-verdict@1"
...
HERMETIC_CANDIDATE_SUITES: Final[tuple[str, ...]] = (
    "tests/test_pcpr_095_residual_gap_report.py",
)
...
BINDING_README: Final = """# PCPR-095 Kit residual-gap-report binding
...
class KitResidualGapReportError(Exception):
    """Kit attempted to remint or store the residual-gap report as live bytes."""


def _text(value: Any, name: str) -> str:
    if not isinstance(value, str) or not value.strip():
        raise KitResidualGapReportError(f"{name} must be a non-empty string")
    return value.strip()
...
def _kind(value: Any, name: str) -> str:
    ...
    if kind not in EVIDENCE_KINDS:
        raise KitResidualGapReportError(f"{name} is not an admitted evidence kind")
    return kind


def _reject_closed_release_value(value: Any, name: str) -> None:
    if isinstance(value, str) and value in CLOSED_RELEASE_OUTCOMES:
        raise KitResidualGapReportError(
...
def parse_pyproject_residual_gap_report_table(text: str) -> dict[str, Any]:
    ...
    if not isinstance(table, dict):
        raise KitResidualGapReportError("pyproject.toml must be a table")
    tool = table.get("tool")
    payload: dict[str, Any] = {}
    if isinstance(tool, dict):
        kit = tool.get("ipfs_kit_py")
        if isinstance(kit, dict):
            raw = kit.get("residual-gap-report")
...
def refuse_live_storage(*, stored: bool) -> None:
    if stored:
        raise KitResidualGapReportError(
            "Kit must not store the hermetic residual-gap report receipt as live durable bytes"
        )
...
@dataclass(frozen=True)
class KitResidualGapReportVerdict:
    schema: str
    interface: str
...
def _operator_blocking_task() -> dict[str, Any]:
    return {
        ...
        "action": (
            "Keep the Kit current root bound to the Accelerate residual-gap report. "
            "Do not store the report as live durable bytes. Do not write "
            "DuckDB or Quack state. Do not emit a closed release. Do not "
            "create a successor campaign. PCPR-096 recommends the next bounded pilot."
        ),
        "reason": (
            "Kit binds the Accelerate hermetic residual-gap report receipt. That "
...
def render_declared_binding(root: Path | None = None) -> dict[str, Any]:
    ...
    if package_root is None:
        raise KitResidualGapReportError("Kit package root was not found")
    _ = package_root
    ...
    document = {
        ...
        "live_application": typed_unavailable(
            reason=(
                "Kit binds the Accelerate residual-gap report receipt and does "
        ...
        "live_storage": typed_unavailable(
            reason=(
                "Kit does not store the hermetic residual-gap report receipt as "
                "live durable bytes."
            )
        ),
        "source": {
...
def current_head_static_probes(start: Path | None = None) -> tuple[OutcomeProbe, ...]:
    ...
    probes = [
        _probe(
            ...
            reason=(
                "Committed Kit residual-gap-report binding matches the generator."
                if verified.get("binding_ok") is True
                else "Committed Kit residual-gap-report binding is missing or drifts."
            ),
        ),
        _probe(
            "pyproject_residual_gap_report_table",
            ...
            reason=(
                "pyproject.toml declares KitResidualGapReportBinding@1."
        ...
        _probe(
            "current_root_survives_report_from_disk",
            binding["storage"]["survives_residual_gap_report"] is True
            and binding["storage"]["reconstructed_from"] == "disk"
            and binding["storage"]["reconstructed_from_memory"] is False,
            reason="Current root survives the residual-gap report and is reconstructed from disk.",
        ),
        _probe(
            "report_not_stored_as_live_bytes",
            client["stored"] is False and client["live"] is False,
            reason="Kit does not store the hermetic residual-gap report as live durable bytes.",
        ),
        _probe(
            "idempotent_current_root",
        ...
        _probe(
            ...
            None,
            evidence_kind="unavailable",
            reason="Live durable storage of the residual-gap report stays typed unavailable.",
        ),
    ]
    return tuple(probes)
...
def qualify_residual_gap_report(
    ...
    *,
    ...
    objective_cid: str,
    idea_digest_cid: str,
) -> KitResidualGapReportVerdict:
    if not probes:
        raise KitResidualGapReportError("at least one probe is required")
    ...
    return KitResidualGapReportVerdict(
        schema=VERDICT_SCHEMA,
        interface=INTERFACE,
        promotion_status=promotion_status,
        supervisor_disposition="supervisor_non_promoted",
...
def pcpr_095_receipt_promotion(
    verdict: KitResidualGapReportVerdict,
) -> dict[str, Any]:
...
PINNED_BINDING_CID: Final = (
    "baguqeeraegcwr7pxn5al5k52zlqy64uzpr6phwiiaj3o6v52qz7jog2d2eca"
)
CURRENT_HEAD_NON_PROMOTION_VERDICT_CID: Final = (
    "baguqeeraekomugrj2zfz6ln6bp6jmivwbati7y2jqt4qqklvr6ntmlmvg6rq"
)
