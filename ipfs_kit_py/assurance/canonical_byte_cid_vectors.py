"""Fail-closed PCPR-041 Kit binding to canonical-byte and CID vectors.

Kit owns DurableArtifactReceipt. This module binds the PCPR-041 vector
CIDs published by the Accelerate-owned catalog encoder and refuses
reminting. Kit verifies and stores exact bytes; it does not decide proof
validity, task completion, reuse, or semantic meaning, and it does not
re-encode canonical bytes.

This module does not import sibling Accelerate or Datasets packages. It
is not a freeze, not negative or cross-language vectors, not a closed
PCPR release, and it never writes DuckDB or Quack state.
"""
...
INTERFACE: Final = "KitCanonicalByteCidVectorBinding@1"
SCHEMA: Final = "ipfs_kit_py/assurance/canonical-byte-cid-vector-binding@1"
...
KIT_OWNED_VECTORS: Final[tuple[str, ...]] = ("DurableArtifactReceipt",)
...
class KitCanonicalByteCidVectorError(ValueError):
    """Kit attempted to remint a canonical-byte or CID vector."""


def refuse_vector_remint(name: str, cid: str) -> str:
    expected = VECTOR_CIDS.get(name)
    if expected is None:
        raise KitCanonicalByteCidVectorError(
...
def kit_vector_binding_mapping() -> dict[str, object]:
    return {
        "schema": SCHEMA,
        "interface": INTERFACE,
        ...
        "owned_vectors": list(KIT_OWNED_VECTORS),
