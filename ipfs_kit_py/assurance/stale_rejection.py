"""Fail-closed PCPR-069 Kit binding to Accelerate stale rejection.

Kit owns exact bytes and the current root. This module binds the
PCPR-062 hermetic current root to the Accelerate-owned stale rejection
and PlanDelta without reminting pack, root, route, patch, run,
unrelated-change, reuse, or relevant-interface identities, without
storing the PlanDelta as live durable bytes, and without deciding
semantic soundness.
...
INTERFACE: Final = "KitStaleRejectionAndPlanDeltaBinding@1"
SCHEMA: Final = "ipfs_kit_py/assurance/stale-rejection-binding@1"
BINDING_SCHEMA: Final = (
    "ipfs_kit_py/assurance/declared-stale-rejection-binding@1"
)
VERDICT_SCHEMA: Final = "ipfs_kit_py/assurance/stale-rejection-verdict@1"
...
HERMETIC_CANDIDATE_SUITES: Final[tuple[str, ...]] = (
    "tests/test_pcpr_069_stale_rejection.py",
)
...
REQUIRED_GOOD_PROBE_IDS: Final[frozenset[str]] = frozenset(
    {
        ...
        "delta_not_stored_as_live_bytes",
...
BINDING_README: Final = """# PCPR-069 Kit stale-rejection binding
...
class KitStaleRejectionError(Exception):
    """Kit attempted to remint or store stale rejection as live bytes."""


def _text(value: Any, name: str) -> str:
    if not isinstance(value, str) or not value.strip():
        raise KitStaleRejectionError(f"{name} must be a non-empty string")
    return value.strip()
...
def _kind(value: Any, name: str) -> str:
    ...
    if kind not in EVIDENCE_KINDS:
        raise KitStaleRejectionError(f"{name} is not an admitted evidence kind")
    return kind


def _reject_closed_release_value(value: Any, name: str) -> None:
    if isinstance(value, str) and value in CLOSED_RELEASE_OUTCOMES:
        raise KitStaleRejectionError(
...
def parse_pyproject_stale_rejection_table(text: str) -> dict[str, Any]:
    ...
    if not isinstance(table, dict):
        raise KitStaleRejectionError("pyproject.toml must be a table")
    tool = table.get("tool")
    payload: dict[str, Any] = {}
    if isinstance(tool, dict):
        kit = tool.get("ipfs_kit_py")
        if isinstance(kit, dict):
            raw = kit.get("stale-rejection")
...
def refuse_live_storage(*, stored: bool) -> None:
    if stored:
        raise KitStaleRejectionError(
            "Kit must not store the hermetic PlanDelta as live durable bytes"
        )
...
@dataclass(frozen=True)
class KitStaleRejectionVerdict:
    schema: str
    interface: str
...
def _operator_blocking_task() -> dict[str, Any]:
    return {
        ...
        "requires": (
            ...
            "rejection and PlanDelta are stored as live durable bytes"
        ),
        "action": (
            "Keep the Kit current root bound to the Accelerate PlanDelta. "
            "Do not remint the root. Do not store the PlanDelta as live "
            "bytes. Do not write DuckDB or Quack state."
        ),
        "reason": (
            "Kit binds the Accelerate hermetic stale rejection and "
            "PlanDelta. That binding is not live storage of the delta."
        ),
    }
...
def render_declared_binding(root: Path | None = None) -> dict[str, Any]:
    ...
    if package_root is None:
        raise KitStaleRejectionError("Kit package root was not found")
    _ = package_root
    ...
    document = {
        ...
        "stale_rejection": {
            "task_id": PCPR_069_TASK_ID,
            "kind": "stale_rejection_and_plan_delta",
        ...
        "live_application": typed_unavailable(
            reason=(
                "Kit binds the Accelerate stale rejection and PlanDelta "
                "and does not store it as live bytes."
            )
        ),
        "source": {
            "kind": "git",
...
def current_head_static_probes(
    ...
    probes = [
        _probe(
            ...
            reason=(
                "Committed Kit stale-rejection binding matches the generator."
                if verified.get("binding_ok") is True
                else "Committed Kit stale-rejection binding is missing or drifts."
            ),
        ),
        _probe(
            "pyproject_stale_rejection_table",
            ...
            reason=(
                "pyproject.toml declares KitStaleRejectionAndPlanDeltaBinding@1."
        ...
        _probe(
            ...
            reason="Kit binds the Accelerate-owned PlanDelta and does not remint it.",
        ),
        _probe(
            "delta_not_stored_as_live_bytes",
            delta["stored"] is False
            and delta["live"] is False
            and binding["bounded_patch"]["stored"] is False
            and binding["selected_tests"]["stored"] is False,
            reason="Kit does not store the PlanDelta as live durable bytes.",
        ),
        _probe(
            "model_assertion_cannot_complete_work",
        ...
        _probe(
            ...
            None,
            evidence_kind="unavailable",
            reason="Live storage of the PlanDelta stays typed unavailable.",
        ),
    ]
    return tuple(probes)
...
def qualify_stale_rejection(
    ...
    *,
    ...
    objective_cid: str,
    idea_digest_cid: str,
) -> KitStaleRejectionVerdict:
    if not probes:
        raise KitStaleRejectionError("at least one probe is required")
    ...
    return KitStaleRejectionVerdict(
        schema=VERDICT_SCHEMA,
        interface=INTERFACE,
        promotion_status=promotion_status,
        supervisor_disposition="supervisor_non_promoted",
...
def pcpr_069_receipt_promotion(
    verdict: KitStaleRejectionVerdict,
) -> dict[str, Any]:
...
PINNED_BINDING_CID: Final = (
    "baguqeeraikjds5jmxcr7olt5ig4z63pe7fylesfigpeawhebn3ihzgylagwa"
)
CURRENT_HEAD_NON_PROMOTION_VERDICT_CID: Final = (
    "baguqeeraizs3x3pl6faq57bviuay6rrhubwdzxqjantddha4efostdcjk3qq"
)
