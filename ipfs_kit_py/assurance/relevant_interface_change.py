"""Fail-closed PCPR-068 Kit binding to Accelerate relevant interface change.

Kit owns exact bytes and the current root. This module binds the
PCPR-062 hermetic current root to the Accelerate-owned relevant
interface change without reminting pack, root, route, patch, run,
unrelated-change, or reuse identities, without storing the change as
...
INTERFACE: Final = "KitRelevantInterfaceChangeBinding@1"
SCHEMA: Final = "ipfs_kit_py/assurance/relevant-interface-change-binding@1"
BINDING_SCHEMA: Final = (
    "ipfs_kit_py/assurance/declared-relevant-interface-change-binding@1"
)
VERDICT_SCHEMA: Final = (
    "ipfs_kit_py/assurance/relevant-interface-change-verdict@1"
)
...
HERMETIC_CANDIDATE_SUITES: Final[tuple[str, ...]] = (
    "tests/test_pcpr_068_relevant_interface_change.py",
)
...
REQUIRED_GOOD_PROBE_IDS: Final[frozenset[str]] = frozenset(
    {
        ...
        "change_not_stored_as_live_bytes",
...
BINDING_README: Final = """# PCPR-068 Kit relevant-interface-change binding
...
class KitRelevantInterfaceChangeError(Exception):
    """Kit attempted to remint or store the relevant interface change as live bytes."""


def _text(value: Any, name: str) -> str:
    if not isinstance(value, str) or not value.strip():
        raise KitRelevantInterfaceChangeError(
...
def parse_pyproject_relevant_interface_change_table(text: str) -> dict[str, Any]:
    ...
    if not isinstance(table, dict):
        raise KitRelevantInterfaceChangeError("pyproject.toml must be a table")
    tool = table.get("tool")
    payload: dict[str, Any] = {}
    if isinstance(tool, dict):
        kit = tool.get("ipfs_kit_py")
        if isinstance(kit, dict):
            raw = kit.get("relevant-interface-change")
...
@dataclass(frozen=True)
class KitRelevantInterfaceChangeVerdict:
    schema: str
    interface: str
...
def _operator_blocking_task() -> dict[str, Any]:
    return {
        ...
        "requires": (
            ...
            "relevant interface change is stored as live durable bytes"
        ),
        "action": (
            "Keep the Kit current root bound to the Accelerate change. Do "
            "not remint the root. Do not store the change as live bytes. "
            "Do not write DuckDB or Quack state."
        ),
        "reason": (
            "Kit binds the Accelerate hermetic relevant interface change "
            "change. That binding is not live storage of the change."
        ),
    }
...
def render_declared_binding(root: Path | None = None) -> dict[str, Any]:
    ...
    if package_root is None:
        raise KitRelevantInterfaceChangeError("Kit package root was not found")
    _ = package_root
    ...
    document = {
        ...
        "live_application": typed_unavailable(
            reason=(
                "Kit binds the Accelerate unrelated state change and does "
...
def current_head_static_probes(
    ...
    probes = [
        _probe(
            ...
            reason=(
                "Committed Kit relevant-interface-change binding matches the generator."
                if verified.get("binding_ok") is True
                else "Committed Kit relevant-interface-change binding is missing or drifts."
            ),
        ),
        _probe(
            "pyproject_relevant_interface_change_table",
            ...
            reason=(
                "pyproject.toml declares KitRelevantInterfaceChangeBinding@1."
        ...
        _probe(
            ...
            reason="Kit binds the Accelerate-owned change and does not remint it.",
        ),
        _probe(
            "change_not_stored_as_live_bytes",
            change["stored"] is False
            and change["live"] is False
            and binding["bounded_patch"]["stored"] is False
            and binding["selected_tests"]["stored"] is False,
            reason="Kit does not store the relevant interface change as live durable bytes.",
        ),
        _probe(
            "model_assertion_cannot_complete_work",
        ...
        _probe(
            ...
            None,
            evidence_kind="unavailable",
            reason="Live storage of the relevant interface change stays typed unavailable.",
        ),
    ]
    return tuple(probes)
...
def qualify_relevant_interface_change(
    ...
    *,
    ...
    objective_cid: str,
    idea_digest_cid: str,
) -> KitRelevantInterfaceChangeVerdict:
    if not probes:
        raise KitRelevantInterfaceChangeError("at least one probe is required")
    ...
    return KitRelevantInterfaceChangeVerdict(
        schema=VERDICT_SCHEMA,
        interface=INTERFACE,
        promotion_status=promotion_status,
        supervisor_disposition="supervisor_non_promoted",
...
def pcpr_068_receipt_promotion(
    verdict: KitRelevantInterfaceChangeVerdict,
) -> dict[str, Any]:
...
PINNED_BINDING_CID: Final = (
    "baguqeeratzb7f3pygwnchlrpkfaaogwbllpcdqcnrofmqgnfiyh2x2kqr4ya"
)
CURRENT_HEAD_NON_PROMOTION_VERDICT_CID: Final = (
    "baguqeerayydztykxdu7a2vn66toelowhhrcpau3ubiyoevg3js24neq4nfca"
)
...
__all__ = [
    ...
    "ESCALATION_ORDER",
    "KitRelevantInterfaceChangeError",
