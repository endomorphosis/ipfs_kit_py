"""Fail-closed PCPR-081 Kit binding to Accelerate generic MCP client.
...
INTERFACE: Final = "KitGenericMcpClientBinding@1"
SCHEMA: Final = "ipfs_kit_py/assurance/generic-mcp-client-binding@1"
BINDING_SCHEMA: Final = (
    "ipfs_kit_py/assurance/declared-generic-mcp-client-binding@1"
)
VERDICT_SCHEMA: Final = "ipfs_kit_py/assurance/generic-mcp-client-verdict@1"
...
HERMETIC_CANDIDATE_SUITES: Final[tuple[str, ...]] = (
    "tests/test_pcpr_081_generic_mcp_client.py",
)
...
BINDING_README: Final = """# PCPR-081 Kit generic-MCP-client binding
...
class KitGenericMcpClientError(Exception):
    """Kit attempted to remint or store the client as live bytes."""


def _text(value: Any, name: str) -> str:
    if not isinstance(value, str) or not value.strip():
        raise KitGenericMcpClientError(f"{name} must be a non-empty string")
    return value.strip()
...
def _kind(value: Any, name: str) -> str:
    ...
    if kind not in EVIDENCE_KINDS:
        raise KitGenericMcpClientError(f"{name} is not an admitted evidence kind")
    return kind


def _reject_closed_release_value(value: Any, name: str) -> None:
    if isinstance(value, str) and value in CLOSED_RELEASE_OUTCOMES:
        raise KitGenericMcpClientError(
...
def parse_pyproject_generic_mcp_client_table(text: str) -> dict[str, Any]:
    ...
    if not isinstance(table, dict):
        raise KitGenericMcpClientError("pyproject.toml must be a table")
    tool = table.get("tool")
    payload: dict[str, Any] = {}
    if isinstance(tool, dict):
        kit = tool.get("ipfs_kit_py")
        if isinstance(kit, dict):
            raw = kit.get("generic-mcp-client")
...
def refuse_live_storage(*, stored: bool) -> None:
    if stored:
        raise KitGenericMcpClientError(
            "Kit must not store the hermetic generic MCP client as live durable bytes"
        )
...
@dataclass(frozen=True)
class KitGenericMcpClientVerdict:
    schema: str
    interface: str
...
def _operator_blocking_task() -> dict[str, Any]:
    return {
        ...
        "action": (
            ...
            "client. Do not store the client as live durable bytes. Do not "
            "write DuckDB or Quack state."
        ),
        "reason": (
            "Kit binds the Accelerate hermetic generic MCP client. That "
...
def render_declared_binding(root: Path | None = None) -> dict[str, Any]:
    ...
    if package_root is None:
        raise KitGenericMcpClientError("Kit package root was not found")
    _ = package_root
    ...
    document = {
        ...
        "generic_mcp_client": {
            "task_id": PCPR_081_TASK_ID,
            "kind": "generic_mcp_client_demonstration",
            "classified_by": EXECUTION_OWNER_REPOSITORY,
        ...
        "live_application": typed_unavailable(
            reason=(
                "Kit binds the Accelerate generic MCP client and does not emit "
        ...
        "live_storage": typed_unavailable(
            reason="Kit does not store the hermetic generic MCP client as live durable bytes."
        ),
        "source": {
            "kind": "git",
...
def current_head_static_probes(start: Path | None = None) -> tuple[OutcomeProbe, ...]:
    ...
    probes = [
        ...
        _probe(
            ...
            reason=(
                "pyproject.toml declares KitGenericMcpClientBinding@1."
...
def qualify_generic_mcp_client(
    ...
    *,
    ...
    objective_cid: str,
    idea_digest_cid: str,
) -> KitGenericMcpClientVerdict:
    if not probes:
        raise KitGenericMcpClientError("at least one probe is required")
    ...
    return KitGenericMcpClientVerdict(
        schema=VERDICT_SCHEMA,
        interface=INTERFACE,
        promotion_status=promotion_status,
        supervisor_disposition="supervisor_non_promoted",
...
def pcpr_081_receipt_promotion(
    verdict: KitGenericMcpClientVerdict,
) -> dict[str, Any]:
...
PINNED_BINDING_CID: Final = (
    "baguqeeralfiuiyf75n6xgehkk7crndlvpvvdfq6e7tx3x36eunel5ug4erzq"
)
CURRENT_HEAD_NON_PROMOTION_VERDICT_CID: Final = (
    "baguqeeraxtd7gfvmqcxh4emfgnonthmrdspnbcc2bcxvyphrrei65ogzbnna"
)
