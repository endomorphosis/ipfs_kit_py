"""Fail-closed PCPR-070 Kit binding to Accelerate state-owner restart.

Kit owns exact bytes and the current root. This module binds the
PCPR-062 hermetic current root to the Accelerate-owned authoritative
state-owner restart without reminting pack, root, route, patch, run,
unrelated-change, reuse, relevant-interface, or PlanDelta identities,
without storing the restart as live durable bytes, and without deciding
semantic soundness.
...
INTERFACE: Final = "KitAuthoritativeStateOwnerRestartBinding@1"
SCHEMA: Final = "ipfs_kit_py/assurance/state-owner-restart-binding@1"
BINDING_SCHEMA: Final = (
    "ipfs_kit_py/assurance/declared-state-owner-restart-binding@1"
)
VERDICT_SCHEMA: Final = "ipfs_kit_py/assurance/state-owner-restart-verdict@1"
...
HERMETIC_CANDIDATE_SUITES: Final[tuple[str, ...]] = (
    "tests/test_pcpr_070_state_owner_restart.py",
)
...
REQUIRED_GOOD_PROBE_IDS: Final[frozenset[str]] = frozenset(
    {
        ...
        "current_root_survives_restart_from_disk",
        "restart_not_stored_as_live_bytes",
...
BINDING_README: Final = """# PCPR-070 Kit state-owner-restart binding
...
class KitStateOwnerRestartError(Exception):
    """Kit attempted to remint or store restart as live bytes."""


def _text(value: Any, name: str) -> str:
    if not isinstance(value, str) or not value.strip():
        raise KitStateOwnerRestartError(f"{name} must be a non-empty string")
    return value.strip()
...
def _kind(value: Any, name: str) -> str:
    ...
    if kind not in EVIDENCE_KINDS:
        raise KitStateOwnerRestartError(f"{name} is not an admitted evidence kind")
    return kind


def _reject_closed_release_value(value: Any, name: str) -> None:
    if isinstance(value, str) and value in CLOSED_RELEASE_OUTCOMES:
        raise KitStateOwnerRestartError(
...
def parse_pyproject_state_owner_restart_table(text: str) -> dict[str, Any]:
    ...
    if not isinstance(table, dict):
        raise KitStateOwnerRestartError("pyproject.toml must be a table")
    tool = table.get("tool")
    payload: dict[str, Any] = {}
    if isinstance(tool, dict):
        kit = tool.get("ipfs_kit_py")
        if isinstance(kit, dict):
            raw = kit.get("state-owner-restart")
...
def refuse_live_storage(*, stored: bool) -> None:
    if stored:
        raise KitStateOwnerRestartError(
            "Kit must not store the hermetic restart as live durable bytes"
        )
...
@dataclass(frozen=True)
class KitStateOwnerRestartVerdict:
    schema: str
    interface: str
...
def _operator_blocking_task() -> dict[str, Any]:
    return {
        ...
        "requires": (
            ...
            "restart is stored as live durable bytes"
        ),
        "action": (
            "Keep the Kit current root bound to the Accelerate restart. "
            "Do not remint the root. Do not store the restart as live "
            "bytes. Do not write DuckDB or Quack state."
        ),
        "reason": (
            "Kit binds the Accelerate hermetic state-owner restart. That "
            "binding is not live storage of the restart."
        ),
    }
...
def render_declared_binding(root: Path | None = None) -> dict[str, Any]:
    ...
    if package_root is None:
        raise KitStateOwnerRestartError("Kit package root was not found")
    _ = package_root
    ...
    document = {
        ...
        "state_owner_restart": {
            "task_id": PCPR_070_TASK_ID,
            "kind": "authoritative_state_owner_restart",
        ...
        "live_application": typed_unavailable(
            reason=(
                "Kit binds the Accelerate state-owner restart and does "
...
def current_head_static_probes(
    ...
    probes = [
        _probe(
            ...
            reason=(
                "Committed Kit state-owner-restart binding matches the generator."
                if verified.get("binding_ok") is True
                else "Committed Kit state-owner-restart binding is missing or drifts."
            ),
        ),
        _probe(
            "pyproject_state_owner_restart_table",
            ...
            reason=(
                "pyproject.toml declares KitAuthoritativeStateOwnerRestartBinding@1."
        ...
        _probe(
            ...
            reason="Kit binds the Accelerate-owned restart and does not remint it.",
        ),
        _probe(
            "current_root_survives_restart_from_disk",
            binding["storage"]["survives_restart"] is True
        ...
        _probe(
            "restart_not_stored_as_live_bytes",
            restart["stored"] is False and restart["live"] is False,
            reason="Kit does not store the hermetic restart as live durable bytes.",
        ),
        _probe(
            "model_assertion_cannot_complete_work",
        ...
        _probe(
            ...
            None,
            evidence_kind="unavailable",
            reason="Live storage of the restart stays typed unavailable.",
        ),
        _probe(
            "live_restart",
            None,
...
def qualify_state_owner_restart(
    ...
    *,
    ...
    objective_cid: str,
    idea_digest_cid: str,
) -> KitStateOwnerRestartVerdict:
    if not probes:
        raise KitStateOwnerRestartError("at least one probe is required")
    ...
    return KitStateOwnerRestartVerdict(
        schema=VERDICT_SCHEMA,
        interface=INTERFACE,
        promotion_status=promotion_status,
        supervisor_disposition="supervisor_non_promoted",
...
def pcpr_070_receipt_promotion(
    verdict: KitStateOwnerRestartVerdict,
) -> dict[str, Any]:
    ...
    if verdict.live_application or verdict.live_restart or verdict.live_storage:
        raise KitStateOwnerRestartError(
...
PINNED_BINDING_CID: Final = (
    "baguqeerajfkmtxwpcss7j3eb6j6rc4rspuouyfcbkabmd25tf7uvljui2l7a"
)
CURRENT_HEAD_NON_PROMOTION_VERDICT_CID: Final = (
    "baguqeerayxj5nwq2v54uwmppaln2jh2e5wvkjn7zp4pwazp3loxpjqutwm7q"
)
