"""Fail-closed PCPR-057 Kit binding to branch and release gates.

Kit binds the Accelerate-owned declared branch-protection policy and
release-gate CIDs and refuses reminting. Kit verifies and stores exact
bytes; it does not decide proof validity, task completion, reuse, or
semantic meaning, and it does not apply GitHub settings.

This module does not import sibling Accelerate or Datasets packages. It
is a declared binding, not live GitHub branch protection, not live tag
protection, not a freeze, and not a closed PCPR release. Live GitHub
admin permission stays typed unavailable. Missing permission is an
...
INTERFACE: Final = "KitBranchAndReleaseGatesBinding@1"
SCHEMA: Final = "ipfs_kit_py/assurance/branch-and-release-gates-binding@1"
BINDING_SCHEMA: Final = (
    "ipfs_kit_py/assurance/declared-branch-and-release-gates-binding@1"
)
VERDICT_SCHEMA: Final = (
    "ipfs_kit_py/assurance/branch-and-release-gates-verdict@1"
)
...
GITHUB_REPOSITORY: Final = "endomorphosis/ipfs_kit_py"
...
HERMETIC_CANDIDATE_SUITES: Final[tuple[str, ...]] = (
    "tests/test_pcpr_057_branch_and_release_gates.py",
)
...
BINDING_README: Final = """# PCPR-057 Kit branch and release gates binding
...
class KitBranchAndReleaseGatesError(ValueError):
    """Kit attempted to remint a branch or release-gate identity."""


def _text(value: Any, name: str) -> str:
    if not isinstance(value, str) or not value.strip():
        raise KitBranchAndReleaseGatesError(
...
def parse_pyproject_gates_table(text: str) -> dict[str, Any]:
    ...
    if not isinstance(table, dict):
        raise KitBranchAndReleaseGatesError("pyproject.toml must be a table")
    tool = table.get("tool")
    payload: dict[str, Any] = {}
    if isinstance(tool, dict):
        kit = tool.get("ipfs_kit_py")
        if isinstance(kit, dict):
            raw = kit.get("branch-and-release-gates")
...
@dataclass(frozen=True)
class KitBranchAndReleaseGatesVerdict:
    schema: str
    interface: str
    ...
    document = {
        ...
        "operator_blocking_task": {
            ...
            "reason": (
                ...
                "endomorphosis/ipfs_kit_py. The declared binding is not live "
                "GitHub protection."
            ),
        },
        "remint": False,
...
def write_branch_and_release_gate_files(
    ...
    if root is None:
        raise KitBranchAndReleaseGatesError("Kit package root was not found")
...
def current_head_static_probes(
    ...
    probes = [
        _probe(
            ...
            reason=(
                "Committed Kit gate binding matches the generator."
                if verified.get("binding_ok") is True
                else "Committed Kit gate binding is missing or drifts."
            ),
            details=verified,
        ),
        _probe(
            ...
            reason=(
                "pyproject.toml declares KitBranchAndReleaseGatesBinding@1."
        ...
        _probe(
            "branch_policy_cid_matches_pin",
            binding["branch_policy_cid"] == PINNED_BRANCH_POLICY_CID,
            reason="Kit binds the Accelerate-owned branch-policy CID without remint.",
        ),
        _probe(
            "release_gate_cid_matches_pin",
            binding["release_gate_cid"] == PINNED_RELEASE_GATE_CID,
            reason="Kit binds the Accelerate-owned release-gate CID without remint.",
        ),
        _probe(
            "lock_cid_matches_pin",
        ...
        _probe(
            ...
            reason=(
                ...
                "Git pins; they are not this gate."
            ),
            details={
                "named_git_extras": list(NAMED_GIT_EXTRAS),
                "named_git_extras_role": NAMED_GIT_EXTRAS_ROLE,
...
def qualify_branch_and_release_gates(
    ...
    *,
    ...
    binding_cid: str,
    lock_cid: str,
) -> KitBranchAndReleaseGatesVerdict:
    if not probes:
        raise KitBranchAndReleaseGatesError("at least one probe is required")
    ...
    return KitBranchAndReleaseGatesVerdict(
        schema=VERDICT_SCHEMA,
        interface=INTERFACE,
        promotion_status=promotion_status,
        supervisor_disposition="supervisor_non_promoted",
...
def pcpr_057_receipt_promotion(
    verdict: KitBranchAndReleaseGatesVerdict,
) -> dict[str, Any]:
...
CURRENT_HEAD_NON_PROMOTION_VERDICT_CID: Final = (
    "baguqeeradpl7qvfevfldah5g2hj7vpevpqbka2iclzdvm4noaylz3qjdvilq"
)
