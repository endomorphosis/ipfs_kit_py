"""Fail-closed PCPR-094 Kit binding to Accelerate promotion or non-promotion receipt.
...
storing the promotion decision as live durable bytes, and without deciding
semantic soundness.
...
INTERFACE: Final = "KitPromotionOrNonPromotionBinding@1"
SCHEMA: Final = "ipfs_kit_py/assurance/promotion-or-non-promotion-binding@1"
BINDING_SCHEMA: Final = (
    "ipfs_kit_py/assurance/declared-promotion-or-non-promotion-binding@1"
)
VERDICT_SCHEMA: Final = "ipfs_kit_py/assurance/promotion-or-non-promotion-verdict@1"
...
HERMETIC_CANDIDATE_SUITES: Final[tuple[str, ...]] = (
    "tests/test_pcpr_094_promotion_or_non_promotion.py",
)
...
REQUIRED_GOOD_PROBE_IDS: Final[frozenset[str]] = frozenset(
    {
        ...
        "gate_owned_by_accelerate",
        "current_root_survives_gate_from_disk",
        "gate_not_stored_as_live_bytes",
...
BINDING_README: Final = """# PCPR-094 Kit promotion-or-non-promotion binding
...
class KitPromotionOrNonPromotionError(Exception):
    """Kit attempted to remint or store the promotion decision as live bytes."""


def _text(value: Any, name: str) -> str:
    if not isinstance(value, str) or not value.strip():
        raise KitPromotionOrNonPromotionError(f"{name} must be a non-empty string")
    return value.strip()
...
def _kind(value: Any, name: str) -> str:
    ...
    if kind not in EVIDENCE_KINDS:
        raise KitPromotionOrNonPromotionError(f"{name} is not an admitted evidence kind")
    return kind


def _reject_closed_release_value(value: Any, name: str) -> None:
    if isinstance(value, str) and value in CLOSED_RELEASE_OUTCOMES:
        raise KitPromotionOrNonPromotionError(
...
def parse_pyproject_promotion_or_non_promotion_table(text: str) -> dict[str, Any]:
    ...
    if not isinstance(table, dict):
        raise KitPromotionOrNonPromotionError("pyproject.toml must be a table")
    tool = table.get("tool")
    payload: dict[str, Any] = {}
    if isinstance(tool, dict):
        kit = tool.get("ipfs_kit_py")
        if isinstance(kit, dict):
            raw = kit.get("promotion-or-non-promotion")
...
def refuse_live_storage(*, stored: bool) -> None:
    if stored:
        raise KitPromotionOrNonPromotionError(
            "Kit must not store the hermetic promotion or non-promotion receipt as live durable bytes"
        )
...
@dataclass(frozen=True)
class KitPromotionOrNonPromotionVerdict:
    schema: str
    interface: str
...
def _operator_blocking_task() -> dict[str, Any]:
    return {
        ...
        "action": (
            "Keep the Kit current root bound to the Accelerate promotion decision. "
            "Do not store the gate as live durable bytes. Do not write "
            "DuckDB or Quack state. Do not emit a closed release. PCPR-095 "
            "publishes the residual-gap report."
        ),
        "reason": (
            "Kit binds the Accelerate hermetic promotion or non-promotion receipt. That "
...
def render_declared_binding(root: Path | None = None) -> dict[str, Any]:
    ...
    if package_root is None:
        raise KitPromotionOrNonPromotionError("Kit package root was not found")
    _ = package_root
    ...
    document = {
        ...
        "promotion_or_non_promotion": {
            "task_id": PCPR_094_TASK_ID,
            "kind": "promotion_or_non_promotion",
            "classified_by": EXECUTION_OWNER_REPOSITORY,
        ...
        "live_application": typed_unavailable(
            reason=(
                "Kit binds the Accelerate promotion or non-promotion receipt and does "
        ...
        "live_storage": typed_unavailable(
            reason=(
                "Kit does not store the hermetic promotion or non-promotion receipt as "
                "live durable bytes."
            )
        ),
        "source": {
...
def current_head_static_probes(start: Path | None = None) -> tuple[OutcomeProbe, ...]:
    ...
    probes = [
        _probe(
            ...
            reason=(
                "Committed Kit promotion-or-non-promotion binding matches the generator."
                if verified.get("binding_ok") is True
                else "Committed Kit promotion-or-non-promotion binding is missing or drifts."
            ),
        ),
        _probe(
            "pyproject_promotion_or_non_promotion_table",
            ...
            reason=(
                "pyproject.toml declares KitPromotionOrNonPromotionBinding@1."
        ...
        _probe(
            "current_root_survives_gate_from_disk",
            binding["storage"]["survives_promotion_or_non_promotion"] is True
            and binding["storage"]["reconstructed_from"] == "disk"
            and binding["storage"]["reconstructed_from_memory"] is False,
            reason="Current root survives the promotion decision and is reconstructed from disk.",
        ),
        _probe(
            "gate_not_stored_as_live_bytes",
            client["stored"] is False and client["live"] is False,
            reason="Kit does not store the hermetic promotion decision as live durable bytes.",
        ),
        _probe(
            "idempotent_current_root",
        ...
        _probe(
            ...
            None,
            evidence_kind="unavailable",
            reason="Live durable storage of the promotion decision stays typed unavailable.",
        ),
    ]
    return tuple(probes)
...
def qualify_promotion_or_non_promotion(
    ...
    *,
    ...
    objective_cid: str,
    idea_digest_cid: str,
) -> KitPromotionOrNonPromotionVerdict:
    if not probes:
        raise KitPromotionOrNonPromotionError("at least one probe is required")
    ...
    return KitPromotionOrNonPromotionVerdict(
        schema=VERDICT_SCHEMA,
        interface=INTERFACE,
        promotion_status=promotion_status,
        supervisor_disposition="supervisor_non_promoted",
...
def pcpr_094_receipt_promotion(
    verdict: KitPromotionOrNonPromotionVerdict,
) -> dict[str, Any]:
...
PINNED_BINDING_CID: Final = (
    "baguqeeradnt76amhno42mrihrprqbosz7fwclxi7y754ckdntd7zdwhanfea"
)
CURRENT_HEAD_NON_PROMOTION_VERDICT_CID: Final = (
    "baguqeeragfh2cxuemfivpfqe6iagzcug4qs2d7zhdo64nufmkrqspn6n5cza"
)
...
__all__ = [
    ...
    "INTERFACE",
    "KitPromotionOrNonPromotionError",
    "OBJECTIVE_KIND",
