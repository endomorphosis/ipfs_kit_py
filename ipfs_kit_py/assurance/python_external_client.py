"""Fail-closed PCPR-080 Kit binding to Accelerate Python client.

Kit owns exact bytes and the current root. This module binds the
PCPR-062 hermetic current root to the Accelerate-owned Python
external-client demonstration without reminting pack, root, or chain
...
INTERFACE: Final = "KitPythonExternalClientBinding@1"
SCHEMA: Final = "ipfs_kit_py/assurance/python-external-client-binding@1"
BINDING_SCHEMA: Final = (
    "ipfs_kit_py/assurance/declared-python-external-client-binding@1"
)
VERDICT_SCHEMA: Final = "ipfs_kit_py/assurance/python-external-client-verdict@1"
...
HERMETIC_CANDIDATE_SUITES: Final[tuple[str, ...]] = (
    "tests/test_pcpr_080_python_external_client.py",
)
...
REQUIRED_GOOD_PROBE_IDS: Final[frozenset[str]] = frozenset(
    {
        ...
        "current_root_survives_client_from_disk",
        "client_not_stored_as_live_bytes",
...
BINDING_README: Final = """# PCPR-080 Kit Python-external-client binding
...
class KitPythonExternalClientError(Exception):
    """Kit attempted to remint or store the client as live bytes."""


def _text(value: Any, name: str) -> str:
    if not isinstance(value, str) or not value.strip():
        raise KitPythonExternalClientError(f"{name} must be a non-empty string")
    return value.strip()
...
def _kind(value: Any, name: str) -> str:
    ...
    if kind not in EVIDENCE_KINDS:
        raise KitPythonExternalClientError(
...
def parse_pyproject_python_external_client_table(text: str) -> dict[str, Any]:
    ...
    if not isinstance(table, dict):
        raise KitPythonExternalClientError("pyproject.toml must be a table")
    tool = table.get("tool")
    payload: dict[str, Any] = {}
    if isinstance(tool, dict):
        kit = tool.get("ipfs_kit_py")
        if isinstance(kit, dict):
            raw = kit.get("python-external-client")
...
def refuse_live_storage(*, stored: bool) -> None:
    if stored:
        raise KitPythonExternalClientError(
            "Kit must not store the hermetic Python client as live durable bytes"
        )
...
@dataclass(frozen=True)
class KitPythonExternalClientVerdict:
    schema: str
    interface: str
...
def _operator_blocking_task() -> dict[str, Any]:
    return {
        ...
        "action": (
            "Keep the Kit current root bound to the Accelerate Python client. "
            "Do not store the client as live durable bytes. Do not write "
            "DuckDB or Quack state."
        ),
        "reason": (
            "Kit binds the Accelerate hermetic Python client. That binding "
            "is not live Supervisor.run or live durable storage."
        ),
    }
...
def render_declared_binding(root: Path | None = None) -> dict[str, Any]:
    ...
    if package_root is None:
        raise KitPythonExternalClientError("Kit package root was not found")
    _ = package_root
    ...
    document = {
        ...
        "python_external_client": {
            "task_id": PCPR_080_TASK_ID,
            "kind": "python_external_client_demonstration",
            "classified_by": EXECUTION_OWNER_REPOSITORY,
        ...
        "live_application": typed_unavailable(
            reason=(
                "Kit binds the Accelerate Python client and does not emit a "
                "live Supervisor.run."
            )
        ),
        ...
        "live_storage": typed_unavailable(
            reason="Kit does not store the hermetic Python client as live durable bytes."
        ),
        "source": {
            "kind": "git",
...
def current_head_static_probes(start: Path | None = None) -> tuple[OutcomeProbe, ...]:
    ...
    probes = [
        ...
        _probe(
            ...
            reason=(
                "pyproject.toml declares KitPythonExternalClientBinding@1."
        ...
        _probe(
            ...
            None,
            evidence_kind="unavailable",
            reason="Live durable client storage stays typed unavailable.",
        ),
    ]
    return tuple(probes)
...
def qualify_python_external_client(
    ...
    *,
    ...
    objective_cid: str,
    idea_digest_cid: str,
) -> KitPythonExternalClientVerdict:
    if not probes:
        raise KitPythonExternalClientError("at least one probe is required")
    ...
    return KitPythonExternalClientVerdict(
        schema=VERDICT_SCHEMA,
        interface=INTERFACE,
        promotion_status=promotion_status,
        supervisor_disposition="supervisor_non_promoted",
...
def pcpr_080_receipt_promotion(
    verdict: KitPythonExternalClientVerdict,
) -> dict[str, Any]:
    ...
    if verdict.live_client or verdict.live_application or verdict.live_storage:
        raise KitPythonExternalClientError(
...
PINNED_BINDING_CID: Final = (
    "baguqeeravilr2six2hpmqk5zu6zft6oasup7k4vqbwcv4fyy42bebmnglrra"
)
CURRENT_HEAD_NON_PROMOTION_VERDICT_CID: Final = (
    "baguqeerakgylvyn5lkyz2ehmb3vjdhaj3vcvd7fned3vasmsyht2235qh2va"
)
