"""Fail-closed PCPR-083 Kit binding to Accelerate authority bypass.

Kit owns exact bytes and the current root. This module binds the
PCPR-062 hermetic current root to the Accelerate-owned authority-bypass
proof without reminting pack, root, chain, client, or parity identities,
without storing the proof as live durable bytes, and without deciding
semantic soundness.

Durable reconstruction of the current root is from disk, not memory.
Bypass replay is identity-preserving. This module does not import
...
INTERFACE: Final = "KitAuthorityBypassBinding@1"
SCHEMA: Final = "ipfs_kit_py/assurance/authority-bypass-binding@1"
BINDING_SCHEMA: Final = (
    "ipfs_kit_py/assurance/declared-authority-bypass-binding@1"
)
VERDICT_SCHEMA: Final = "ipfs_kit_py/assurance/authority-bypass-verdict@1"
...
HERMETIC_CANDIDATE_SUITES: Final[tuple[str, ...]] = (
    "tests/test_pcpr_083_authority_bypass.py",
)
...
REQUIRED_GOOD_PROBE_IDS: Final[frozenset[str]] = frozenset(
    {
        ...
        "bypass_owned_by_accelerate",
        "current_root_survives_bypass_from_disk",
        "bypass_not_stored_as_live_bytes",
...
BINDING_README: Final = """# PCPR-083 Kit authority-bypass binding
...
class KitAuthorityBypassError(Exception):
    """Kit attempted to remint or store the bypass proof as live bytes."""


def _text(value: Any, name: str) -> str:
    if not isinstance(value, str) or not value.strip():
        raise KitAuthorityBypassError(f"{name} must be a non-empty string")
    return value.strip()
...
def _kind(value: Any, name: str) -> str:
    ...
    if kind not in EVIDENCE_KINDS:
        raise KitAuthorityBypassError(f"{name} is not an admitted evidence kind")
    return kind


def _reject_closed_release_value(value: Any, name: str) -> None:
    if isinstance(value, str) and value in CLOSED_RELEASE_OUTCOMES:
        raise KitAuthorityBypassError(
...
def parse_pyproject_authority_bypass_table(text: str) -> dict[str, Any]:
    ...
    if not isinstance(table, dict):
        raise KitAuthorityBypassError("pyproject.toml must be a table")
    tool = table.get("tool")
    payload: dict[str, Any] = {}
    if isinstance(tool, dict):
        kit = tool.get("ipfs_kit_py")
        if isinstance(kit, dict):
            raw = kit.get("authority-bypass")
...
def refuse_live_storage(*, stored: bool) -> None:
    if stored:
        raise KitAuthorityBypassError(
            "Kit must not store the hermetic authority-bypass proof as live durable bytes"
        )
...
@dataclass(frozen=True)
class KitAuthorityBypassVerdict:
    schema: str
    interface: str
...
def _operator_blocking_task() -> dict[str, Any]:
    return {
        ...
        "action": (
            "Keep the Kit current root bound to the Accelerate authority-bypass "
            "proof. Do not store the bypass comparison as live durable bytes. "
            "Do not write DuckDB or Quack state."
        ),
        "reason": (
            "Kit binds the Accelerate hermetic authority-bypass proof. That "
...
def render_declared_binding(root: Path | None = None) -> dict[str, Any]:
    ...
    if package_root is None:
        raise KitAuthorityBypassError("Kit package root was not found")
    _ = package_root
    ...
    document = {
        ...
        "live_application": typed_unavailable(
            reason=(
                "Kit binds the Accelerate authority-bypass proof and does not "
        ...
        "live_storage": typed_unavailable(
            reason="Kit does not store the hermetic authority-bypass proof as live durable bytes."
        ),
        "source": {
            "kind": "git",
...
def current_head_static_probes(start: Path | None = None) -> tuple[OutcomeProbe, ...]:
    ...
    probes = [
        _probe(
            ...
            reason=(
                "Committed Kit authority-bypass binding matches the generator."
                if verified.get("binding_ok") is True
                else "Committed Kit authority-bypass binding is missing or drifts."
            ),
        ),
        _probe(
            "pyproject_authority_bypass_table",
            ...
            reason=(
                "pyproject.toml declares KitAuthorityBypassBinding@1."
                if table.get("interface") == INTERFACE
                else "pyproject.toml does not declare the PCPR-083 binding."
            ),
        ),
        _probe(
            "owner_pack_cid_matches_pin",
        ...
        _probe(
            "current_root_survives_bypass_from_disk",
            binding["storage"]["survives_bypass"] is True
            and binding["storage"]["reconstructed_from"] == "disk"
            and binding["storage"]["reconstructed_from_memory"] is False,
            reason="The current root survives the bypass and is reconstructed from disk.",
        ),
        _probe(
            "bypass_not_stored_as_live_bytes",
...
def qualify_authority_bypass(
    ...
    *,
    ...
    objective_cid: str,
    idea_digest_cid: str,
) -> KitAuthorityBypassVerdict:
    if not probes:
        raise KitAuthorityBypassError("at least one probe is required")
    ...
    return KitAuthorityBypassVerdict(
        schema=VERDICT_SCHEMA,
        interface=INTERFACE,
        promotion_status=promotion_status,
        supervisor_disposition="supervisor_non_promoted",
...
def pcpr_083_receipt_promotion(
    verdict: KitAuthorityBypassVerdict,
) -> dict[str, Any]:
...
PINNED_BINDING_CID: Final = (
    "baguqeerablqglgunnjuo7nyn4rtrjg6tzxtogs3xwtvxfw2wckluu6s4bcya"
)
CURRENT_HEAD_NON_PROMOTION_VERDICT_CID: Final = (
    "baguqeeramw5f6cnuzkaveq45iidkheupiw7swacugjwbwvlspgchjrrrks6a"
)
