"""Fail-closed PCPR-093 Kit binding to Accelerate release-candidate gate.
...
storing the gate run as live durable bytes, and without deciding
semantic soundness.
...
INTERFACE: Final = "KitReleaseCandidateGateBinding@1"
SCHEMA: Final = "ipfs_kit_py/assurance/release-candidate-gate-binding@1"
BINDING_SCHEMA: Final = (
    "ipfs_kit_py/assurance/declared-release-candidate-gate-binding@1"
)
VERDICT_SCHEMA: Final = "ipfs_kit_py/assurance/release-candidate-gate-verdict@1"
...
HERMETIC_CANDIDATE_SUITES: Final[tuple[str, ...]] = (
    "tests/test_pcpr_093_release_candidate_gate.py",
)
...
BINDING_README: Final = """# PCPR-093 Kit release-candidate-gate binding
...
class KitReleaseCandidateGateError(Exception):
    """Kit attempted to remint or store the gate run as live bytes."""


def _text(value: Any, name: str) -> str:
    if not isinstance(value, str) or not value.strip():
        raise KitReleaseCandidateGateError(f"{name} must be a non-empty string")
    return value.strip()
...
def _kind(value: Any, name: str) -> str:
    ...
    if kind not in EVIDENCE_KINDS:
        raise KitReleaseCandidateGateError(f"{name} is not an admitted evidence kind")
    return kind


def _reject_closed_release_value(value: Any, name: str) -> None:
    if isinstance(value, str) and value in CLOSED_RELEASE_OUTCOMES:
        raise KitReleaseCandidateGateError(
...
def parse_pyproject_release_candidate_gate_table(text: str) -> dict[str, Any]:
    ...
    if not isinstance(table, dict):
        raise KitReleaseCandidateGateError("pyproject.toml must be a table")
    tool = table.get("tool")
    payload: dict[str, Any] = {}
    if isinstance(tool, dict):
        kit = tool.get("ipfs_kit_py")
        if isinstance(kit, dict):
            raw = kit.get("release-candidate-gate")
...
def refuse_live_storage(*, stored: bool) -> None:
    if stored:
        raise KitReleaseCandidateGateError(
            "Kit must not store the hermetic release-candidate gate as live durable bytes"
        )
...
@dataclass(frozen=True)
class KitReleaseCandidateGateVerdict:
    schema: str
    interface: str
...
def _operator_blocking_task() -> dict[str, Any]:
    return {
        ...
        "action": (
            "Keep the Kit current root bound to the Accelerate gate run. "
            "Do not store the gate as live durable bytes. Do not write "
            "DuckDB or Quack state. Do not emit a closed release. PCPR-094 "
            "produces the promotion or honest non-promotion receipt."
        ),
        "reason": (
            "Kit binds the Accelerate hermetic release-candidate gate. That "
...
def render_declared_binding(root: Path | None = None) -> dict[str, Any]:
    ...
    if package_root is None:
        raise KitReleaseCandidateGateError("Kit package root was not found")
    _ = package_root
    ...
    document = {
        ...
        "live_application": typed_unavailable(
            reason=(
                "Kit binds the Accelerate release-candidate gate and does "
        ...
        "live_storage": typed_unavailable(
            reason=(
                "Kit does not store the hermetic release-candidate gate as "
                "live durable bytes."
            )
        ),
        "source": {
...
def current_head_static_probes(start: Path | None = None) -> tuple[OutcomeProbe, ...]:
    ...
    probes = [
        _probe(
            ...
            reason=(
                "Committed Kit release-candidate-gate binding matches the generator."
                if verified.get("binding_ok") is True
                else "Committed Kit release-candidate-gate binding is missing or drifts."
            ),
        ),
        _probe(
            "pyproject_release_candidate_gate_table",
            ...
            reason=(
                "pyproject.toml declares KitReleaseCandidateGateBinding@1."
        ...
        _probe(
            "current_root_survives_gate_from_disk",
            binding["storage"]["survives_release_candidate_gate"] is True
            and binding["storage"]["reconstructed_from"] == "disk"
            and binding["storage"]["reconstructed_from_memory"] is False,
            reason="Current root survives the gate run and is reconstructed from disk.",
        ),
        _probe(
            "gate_not_stored_as_live_bytes",
            client["stored"] is False and client["live"] is False,
            reason="Kit does not store the hermetic gate run as live durable bytes.",
        ),
        _probe(
            "idempotent_current_root",
        ...
        _probe(
            ...
            None,
            evidence_kind="unavailable",
            reason="Live durable storage of the gate run stays typed unavailable.",
        ),
    ]
    return tuple(probes)
...
def qualify_release_candidate_gate(
    ...
    *,
    ...
    objective_cid: str,
    idea_digest_cid: str,
) -> KitReleaseCandidateGateVerdict:
    if not probes:
        raise KitReleaseCandidateGateError("at least one probe is required")
    ...
    return KitReleaseCandidateGateVerdict(
        schema=VERDICT_SCHEMA,
        interface=INTERFACE,
        promotion_status=promotion_status,
        supervisor_disposition="supervisor_non_promoted",
...
def pcpr_093_receipt_promotion(
    verdict: KitReleaseCandidateGateVerdict,
) -> dict[str, Any]:
...
PINNED_BINDING_CID: Final = (
    "baguqeeraqluhsxwbrwmtzrs475zdlj4aamyhgrihftrcuwi4xsyp7afg5p6a"
)
CURRENT_HEAD_NON_PROMOTION_VERDICT_CID: Final = (
    "baguqeera5y7uurvm5hs4udzhcws3davfsddr6groggw3kiebxdnlr6prpakq"
)
