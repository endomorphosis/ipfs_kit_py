"""Fail-closed PCPR-072 Kit binding to Accelerate final receipt chain.

Kit owns exact bytes and the current root. This module binds the
PCPR-062 hermetic current root to the Accelerate-owned final
proof-carrying receipt chain without reminting pack, root, route,
patch, run, unrelated-change, reuse, relevant-interface, PlanDelta,
restart, or recovery identities, without storing the chain as live
durable bytes, and without deciding semantic soundness.

Durable reconstruction of the current root is from disk, not memory.
Chain replay is identity-preserving. This module does not import
...
INTERFACE: Final = "KitFinalReceiptChainBinding@1"
SCHEMA: Final = "ipfs_kit_py/assurance/final-receipt-chain-binding@1"
BINDING_SCHEMA: Final = (
    "ipfs_kit_py/assurance/declared-final-receipt-chain-binding@1"
)
VERDICT_SCHEMA: Final = "ipfs_kit_py/assurance/final-receipt-chain-verdict@1"
...
HERMETIC_CANDIDATE_SUITES: Final[tuple[str, ...]] = (
    "tests/test_pcpr_072_final_receipt_chain.py",
)
...
REQUIRED_GOOD_PROBE_IDS: Final[frozenset[str]] = frozenset(
    {
        ...
        "current_root_survives_chain_from_disk",
        "chain_not_stored_as_live_bytes",
...
BINDING_README: Final = """# PCPR-072 Kit final-receipt-chain binding
...
class KitFinalReceiptChainError(Exception):
    """Kit attempted to remint or store the chain as live bytes."""


def _text(value: Any, name: str) -> str:
    if not isinstance(value, str) or not value.strip():
        raise KitFinalReceiptChainError(f"{name} must be a non-empty string")
    return value.strip()
...
def _kind(value: Any, name: str) -> str:
    ...
    if kind not in EVIDENCE_KINDS:
        raise KitFinalReceiptChainError(f"{name} is not an admitted evidence kind")
    return kind


def _reject_closed_release_value(value: Any, name: str) -> None:
    if isinstance(value, str) and value in CLOSED_RELEASE_OUTCOMES:
        raise KitFinalReceiptChainError(
...
def parse_pyproject_final_receipt_chain_table(text: str) -> dict[str, Any]:
    ...
    if not isinstance(table, dict):
        raise KitFinalReceiptChainError("pyproject.toml must be a table")
    tool = table.get("tool")
    payload: dict[str, Any] = {}
    if isinstance(tool, dict):
        kit = tool.get("ipfs_kit_py")
        if isinstance(kit, dict):
            raw = kit.get("final-receipt-chain")
...
def refuse_run_cid_remint(cid: str) -> str:
    if cid != PINNED_RUN_CID:
        raise KitFinalReceiptChainError(f"run CID {cid} remints {PINNED_RUN_CID}")
    return cid
...
def refuse_live_storage(*, stored: bool) -> None:
    if stored:
        raise KitFinalReceiptChainError(
            "Kit must not store the hermetic chain as live durable bytes"
        )
...
@dataclass(frozen=True)
class KitFinalReceiptChainVerdict:
    schema: str
    interface: str
...
def _operator_blocking_task() -> dict[str, Any]:
    return {
        ...
        "applied": False,
        "requires": (
            "An admitted Quack-fenced state-owner session before the chain "
            "is stored as live durable bytes"
        ),
        "action": (
            "Keep the Kit current root bound to the Accelerate chain. "
            "Do not remint the root. Do not store the chain as live bytes. "
            "Do not write DuckDB or Quack state."
        ),
        "reason": (
            "Kit binds the Accelerate hermetic final receipt chain. That "
            "binding is not live storage of the chain."
        ),
    }
...
def render_declared_binding(root: Path | None = None) -> dict[str, Any]:
    ...
    if package_root is None:
        raise KitFinalReceiptChainError("Kit package root was not found")
    _ = package_root
    ...
    document = {
        ...
        "live_application": typed_unavailable(
            reason=(
                "Kit binds the Accelerate chain and does not store it as live bytes."
            )
        ),
        "live_storage": typed_unavailable(
            reason="Live IPFS publication of the chain remains typed unavailable."
        ),
...
def current_head_static_probes(start: Path | None = None) -> tuple[OutcomeProbe, ...]:
    ...
    probes = [
        _probe(
            ...
            reason=(
                "Committed Kit chain binding matches the generator."
                if verified.get("binding_ok") is True
                else "Committed Kit chain binding is missing or drifts."
            ),
        ),
        _probe(
            "pyproject_final_receipt_chain_table",
            ...
            reason=(
                "pyproject.toml declares KitFinalReceiptChainBinding@1."
        ...
        _probe(
            ...
            reason="Kit binds the Accelerate-owned chain and does not remint it.",
        ),
        _probe(
            "current_root_survives_chain_from_disk",
            binding["storage"]["survives_chain"] is True
            and binding["storage"]["reconstructed_from"] == "disk"
            and binding["storage"]["reconstructed_from_memory"] is False
            and chain["reconstructed_from_memory"] is False,
            reason="Current root is reconstructed from disk, not memory.",
        ),
        _probe(
            "chain_not_stored_as_live_bytes",
            chain["stored"] is False and chain["live"] is False,
            reason="Kit does not store the hermetic chain as live durable bytes.",
        ),
        _probe(
            "idempotent_current_root",
            binding["storage"]["idempotent"] is True
            and chain["idempotent"] is True
            and chain["history_mutated"] is False,
            reason="Current root identity is preserved across chain replay.",
        ),
        _probe(
            "model_assertion_cannot_complete_work",
        ...
        _probe(
            ...
            reason="Kit must not remint the PCPR-071 recovery CID.",
        ),
        _probe(
            "simulated_results_represented_as_live",
            False,
        ...
        _probe(
            ...
            None,
            evidence_kind="unavailable",
            reason="Live IPFS storage stays typed unavailable.",
        ),
        _probe(
            "live_chain",
            None,
...
def qualify_final_receipt_chain(
    ...
    *,
    ...
    objective_cid: str,
    idea_digest_cid: str,
) -> KitFinalReceiptChainVerdict:
    if not probes:
        raise KitFinalReceiptChainError("at least one probe is required")
    ...
    return KitFinalReceiptChainVerdict(
        schema=VERDICT_SCHEMA,
        interface=INTERFACE,
        promotion_status=promotion_status,
        supervisor_disposition="supervisor_non_promoted",
...
def pcpr_072_receipt_promotion(
    verdict: KitFinalReceiptChainVerdict,
) -> dict[str, Any]:
    ...
    if verdict.live_application or verdict.live_chain or verdict.live_storage:
        raise KitFinalReceiptChainError(
...
PINNED_BINDING_CID: Final = (
    "baguqeerajl3u2qihbiixj3l575olykypnukl4aebbkyjesjrpdo4jms7kuza"
)
CURRENT_HEAD_NON_PROMOTION_VERDICT_CID: Final = (
    "baguqeeraneqymhvsbswxy55rwfaa4xwcvjrl7fec2zprb4kstbbfsi6aagzq"
)
