"""Fail-closed PCPR-096 Kit binding to Accelerate next-bounded-pilot recommendation receipt.

Kit owns exact bytes and the current root. This module binds the
PCPR-062 hermetic current root to the Accelerate-owned release-candidate
gate without reminting pack, root, chain, client, parity, bypass,
threat-model, TCB-inventory, or audit-package identities, without
storing the next-bounded-pilot recommendation as live durable bytes, and without deciding
semantic soundness.

Durable reconstruction of the current root is from disk, not memory.
Gate-run replay is identity-preserving. This module does not import
...
INTERFACE: Final = "KitNextBoundedPilotBinding@1"
SCHEMA: Final = "ipfs_kit_py/assurance/next-bounded-pilot-binding@1"
BINDING_SCHEMA: Final = (
    "ipfs_kit_py/assurance/declared-next-bounded-pilot-binding@1"
)
VERDICT_SCHEMA: Final = "ipfs_kit_py/assurance/next-bounded-pilot-verdict@1"
...
HERMETIC_CANDIDATE_SUITES: Final[tuple[str, ...]] = (
    "tests/test_pcpr_096_next_bounded_pilot.py",
)
...
REQUIRED_GOOD_PROBE_IDS: Final[frozenset[str]] = frozenset(
    {
        ...
        "report_owned_by_accelerate",
        "current_root_survives_report_from_disk",
        "report_not_stored_as_live_bytes",
...
BINDING_README: Final = """# PCPR-096 Kit next-bounded-pilot binding
...
class KitNextBoundedPilotError(Exception):
    """Kit attempted to remint or store the next-bounded-pilot recommendation as live bytes."""


def _text(value: Any, name: str) -> str:
    if not isinstance(value, str) or not value.strip():
        raise KitNextBoundedPilotError(f"{name} must be a non-empty string")
    return value.strip()
...
def _kind(value: Any, name: str) -> str:
    ...
    if kind not in EVIDENCE_KINDS:
        raise KitNextBoundedPilotError(f"{name} is not an admitted evidence kind")
    return kind


def _reject_closed_release_value(value: Any, name: str) -> None:
    if isinstance(value, str) and value in CLOSED_RELEASE_OUTCOMES:
        raise KitNextBoundedPilotError(
...
def parse_pyproject_next_bounded_pilot_table(text: str) -> dict[str, Any]:
    ...
    if not isinstance(table, dict):
        raise KitNextBoundedPilotError("pyproject.toml must be a table")
    tool = table.get("tool")
    payload: dict[str, Any] = {}
    if isinstance(tool, dict):
        kit = tool.get("ipfs_kit_py")
        if isinstance(kit, dict):
            raw = kit.get("next-bounded-pilot")
...
def refuse_live_storage(*, stored: bool) -> None:
    if stored:
        raise KitNextBoundedPilotError(
            "Kit must not store the hermetic next-bounded-pilot recommendation receipt as live durable bytes"
        )
...
@dataclass(frozen=True)
class KitNextBoundedPilotVerdict:
    schema: str
    interface: str
...
def _operator_blocking_task() -> dict[str, Any]:
    return {
        ...
        "action": (
            "Keep the Kit current root bound to the Accelerate next-bounded-pilot recommendation. "
            "Do not store the report as live durable bytes. Do not write "
            "DuckDB or Quack state. Do not emit a closed release. Do not "
            "create a successor campaign. This task recommends the next bounded synthetic pilot and does not create it."
        ),
        "reason": (
            "Kit binds the Accelerate hermetic next-bounded-pilot recommendation receipt. That "
            "binding is not live Supervisor.run or live durable storage."
        ),
    }
...
def render_declared_binding(root: Path | None = None) -> dict[str, Any]:
    ...
    if package_root is None:
        raise KitNextBoundedPilotError("Kit package root was not found")
    _ = package_root
    ...
    document = {
        ...
        "live_application": typed_unavailable(
            reason=(
                "Kit binds the Accelerate next-bounded-pilot recommendation receipt and does "
        ...
        "live_storage": typed_unavailable(
            reason=(
                "Kit does not store the hermetic next-bounded-pilot recommendation receipt as "
                "live durable bytes."
            )
        ),
        "source": {
...
def current_head_static_probes(start: Path | None = None) -> tuple[OutcomeProbe, ...]:
    ...
    probes = [
        _probe(
            ...
            reason=(
                "Committed Kit next-bounded-pilot binding matches the generator."
                if verified.get("binding_ok") is True
                else "Committed Kit next-bounded-pilot binding is missing or drifts."
            ),
        ),
        _probe(
            "pyproject_next_bounded_pilot_table",
            ...
            reason=(
                "pyproject.toml declares KitNextBoundedPilotBinding@1."
        ...
        _probe(
            ...
            reason="Kit binds the pack CID without remint.",
        ),
        _probe(
            "kit_current_root_bound_not_minted",
        ...
        _probe(
            "report_owned_by_accelerate",
            client["classified_by"] == EXECUTION_OWNER_REPOSITORY
            and client["client_owned_by"] == EXECUTION_OWNER_REPOSITORY,
            reason="The next-bounded-pilot recommendation receipt remains owned by Accelerate.",
        ),
        _probe(
            "current_root_survives_report_from_disk",
            binding["storage"]["survives_next_bounded_pilot"] is True
            and binding["storage"]["reconstructed_from"] == "disk"
            and binding["storage"]["reconstructed_from_memory"] is False,
            reason="Current root survives the next-bounded-pilot recommendation and is reconstructed from disk.",
        ),
        _probe(
            "report_not_stored_as_live_bytes",
            client["stored"] is False and client["live"] is False,
            reason="Kit does not store the hermetic next-bounded-pilot recommendation as live durable bytes.",
        ),
        _probe(
            "idempotent_current_root",
        ...
        _probe(
            "memory_reconstruction_accepted",
            False,
            reason="Current root is not reconstructed from memory.",
        ),
        _probe(
            "database_edited",
            False,
        ...
        _probe(
            ...
            None,
            evidence_kind="unavailable",
            reason="Live durable storage of the next-bounded-pilot recommendation stays typed unavailable.",
        ),
    ]
    return tuple(probes)
...
def qualify_next_bounded_pilot(
    ...
    *,
    ...
    objective_cid: str,
    idea_digest_cid: str,
) -> KitNextBoundedPilotVerdict:
    if not probes:
        raise KitNextBoundedPilotError("at least one probe is required")
    ...
    return KitNextBoundedPilotVerdict(
        schema=VERDICT_SCHEMA,
        interface=INTERFACE,
        promotion_status=promotion_status,
        supervisor_disposition="supervisor_non_promoted",
...
def pcpr_095_receipt_promotion(
    verdict: KitNextBoundedPilotVerdict,
) -> dict[str, Any]:
...
PINNED_BINDING_CID: Final = (
    "baguqeeraw5tuxnxicn256fa4kinyo4e3kdpcts677godvee47h2csbqgylua"
)
CURRENT_HEAD_NON_PROMOTION_VERDICT_CID: Final = (
    "baguqeeraspirmtkop2zmfqoacpkecczrgm2avaeidaypsb3cmtw7vbddqngq"
)
