"""Fail-closed PCPR-065 Kit binding to Accelerate selected tests.

Kit owns exact bytes and the current root. This module binds the
PCPR-062 hermetic current root to the Accelerate-owned selected-tests-
and-proofs run without reminting pack, root, route, or patch identities,
without storing the run as live durable bytes, and without deciding
semantic soundness.
...
INTERFACE: Final = "KitSelectedTestsAndProofsBinding@1"
SCHEMA: Final = "ipfs_kit_py/assurance/selected-tests-and-proofs-binding@1"
BINDING_SCHEMA: Final = (
    "ipfs_kit_py/assurance/declared-selected-tests-and-proofs-binding@1"
)
VERDICT_SCHEMA: Final = (
    "ipfs_kit_py/assurance/selected-tests-and-proofs-verdict@1"
)
...
HERMETIC_CANDIDATE_SUITES: Final[tuple[str, ...]] = (
    "tests/test_pcpr_065_selected_tests_and_proofs.py",
)
...
BINDING_README: Final = """# PCPR-065 Kit selected-tests-and-proofs binding
...
class KitSelectedTestsAndProofsError(Exception):
    """Kit attempted to remint or store selected tests as live bytes."""


def _text(value: Any, name: str) -> str:
    if not isinstance(value, str) or not value.strip():
        raise KitSelectedTestsAndProofsError(
...
def parse_pyproject_selected_tests_and_proofs_table(text: str) -> dict[str, Any]:
    ...
    if not isinstance(table, dict):
        raise KitSelectedTestsAndProofsError("pyproject.toml must be a table")
    tool = table.get("tool")
    payload: dict[str, Any] = {}
    if isinstance(tool, dict):
        kit = tool.get("ipfs_kit_py")
        if isinstance(kit, dict):
            raw = kit.get("selected-tests-and-proofs")
...
@dataclass(frozen=True)
class KitSelectedTestsAndProofsVerdict:
    schema: str
    interface: str
...
def _operator_blocking_task() -> dict[str, Any]:
    return {
        ...
        "requires": (
            ...
            "tests and proofs are stored as live durable bytes"
        ),
        "action": (
            "Keep the Kit current root bound to the Accelerate selected-"
            "tests-and-proofs run. Do not write DuckDB or Quack state. "
            "Do not claim live IPFS."
        ),
        "reason": (
            "Kit binds the Accelerate hermetic selected-tests-and-proofs "
            "run. That binding is not live durable publication."
        ),
    }
...
def render_declared_binding(root: Path | None = None) -> dict[str, Any]:
    ...
    if package_root is None:
        raise KitSelectedTestsAndProofsError("Kit package root was not found")
    _ = package_root
    document = {
        ...
        "live_application": typed_unavailable(
            reason=(
                "Kit binds the Accelerate selected-tests-and-proofs run "
...
def current_head_static_probes(
    ...
    probes = [
        _probe(
            ...
            reason=(
                "Committed Kit selected-tests-and-proofs binding matches the generator."
                if verified.get("binding_ok") is True
                else "Committed Kit selected-tests-and-proofs binding is missing or drifts."
            ),
        ),
        _probe(
            "pyproject_selected_tests_and_proofs_table",
            ...
            reason=(
                "pyproject.toml declares KitSelectedTestsAndProofsBinding@1."
        ...
        _probe(
            ...
            reason="Kit binds the Accelerate-owned run and does not remint it.",
        ),
        _probe(
            "model_assertion_cannot_complete_work",
        ...
        _probe(
            ...
            None,
            evidence_kind="unavailable",
            reason="Live durable storage of the run stays typed unavailable.",
        ),
    ]
    return tuple(probes)
...
def qualify_selected_tests_and_proofs(
    ...
    *,
    ...
    objective_cid: str,
    idea_digest_cid: str,
) -> KitSelectedTestsAndProofsVerdict:
    if not probes:
        raise KitSelectedTestsAndProofsError("at least one probe is required")
    ...
    return KitSelectedTestsAndProofsVerdict(
        schema=VERDICT_SCHEMA,
        interface=INTERFACE,
        promotion_status=promotion_status,
        supervisor_disposition="supervisor_non_promoted",
...
def pcpr_065_receipt_promotion(
    verdict: KitSelectedTestsAndProofsVerdict,
) -> dict[str, Any]:
...
PINNED_BINDING_CID: Final = (
    "baguqeeralylqr74mbpnmwmrqwdruxsfr4knabxoi7dpznqmsahe6grn6sfnq"
)
CURRENT_HEAD_NON_PROMOTION_VERDICT_CID: Final = (
    "baguqeera7k44lkjjtu77gk5bgyrmigcildm3vcq5xxelsfyfzw7azzyafjaa"
)
...
__all__ = [
    ...
    "ESCALATION_ORDER",
    "KitSelectedTestsAndProofsError",
