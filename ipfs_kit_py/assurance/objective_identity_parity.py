"""Fail-closed PCPR-082 Kit binding to Accelerate objective identity parity.

Kit owns exact bytes and the current root. This module binds the
PCPR-062 hermetic current root to the Accelerate-owned generic
MCP-client demonstration without reminting pack, root, or chain
identities, without storing the client as live durable bytes, and
without deciding semantic soundness.

Durable reconstruction of the current root is from disk, not memory.
Client replay is identity-preserving. This module does not import
...
INTERFACE: Final = "KitObjectiveIdentityParityBinding@1"
SCHEMA: Final = "ipfs_kit_py/assurance/objective-identity-parity-binding@1"
BINDING_SCHEMA: Final = (
    "ipfs_kit_py/assurance/declared-objective-identity-parity-binding@1"
)
VERDICT_SCHEMA: Final = "ipfs_kit_py/assurance/objective-identity-parity-verdict@1"
...
HERMETIC_CANDIDATE_SUITES: Final[tuple[str, ...]] = (
    "tests/test_pcpr_082_objective_identity_parity.py",
)
...
REQUIRED_GOOD_PROBE_IDS: Final[frozenset[str]] = frozenset(
    {
        ...
        "parity_owned_by_accelerate",
        "current_root_survives_parity_from_disk",
        "parity_not_stored_as_live_bytes",
...
BINDING_README: Final = """# PCPR-082 Kit objective-identity-parity binding
...
class KitObjectiveIdentityParityError(Exception):
    """Kit attempted to remint or store the client as live bytes."""


def _text(value: Any, name: str) -> str:
    if not isinstance(value, str) or not value.strip():
        raise KitObjectiveIdentityParityError(f"{name} must be a non-empty string")
    return value.strip()
...
def _kind(value: Any, name: str) -> str:
    ...
    if kind not in EVIDENCE_KINDS:
        raise KitObjectiveIdentityParityError(f"{name} is not an admitted evidence kind")
    return kind


def _reject_closed_release_value(value: Any, name: str) -> None:
    if isinstance(value, str) and value in CLOSED_RELEASE_OUTCOMES:
        raise KitObjectiveIdentityParityError(
...
def parse_pyproject_objective_identity_parity_table(text: str) -> dict[str, Any]:
    ...
    if not isinstance(table, dict):
        raise KitObjectiveIdentityParityError("pyproject.toml must be a table")
    tool = table.get("tool")
    payload: dict[str, Any] = {}
    if isinstance(tool, dict):
        kit = tool.get("ipfs_kit_py")
        if isinstance(kit, dict):
            raw = kit.get("objective-identity-parity")
...
def refuse_live_storage(*, stored: bool) -> None:
    if stored:
        raise KitObjectiveIdentityParityError(
            "Kit must not store the hermetic objective identity parity as live durable bytes"
        )
...
@dataclass(frozen=True)
class KitObjectiveIdentityParityVerdict:
    schema: str
    interface: str
...
def _operator_blocking_task() -> dict[str, Any]:
    return {
        ...
        "action": (
            "Keep the Kit current root bound to the Accelerate generic MCP "
            "client. Do not store the parity comparison as live durable bytes. Do not "
            "write DuckDB or Quack state."
        ),
        "reason": (
            "Kit binds the Accelerate hermetic objective identity parity. That "
...
def render_declared_binding(root: Path | None = None) -> dict[str, Any]:
    ...
    if package_root is None:
        raise KitObjectiveIdentityParityError("Kit package root was not found")
    _ = package_root
    ...
    document = {
        ...
        "objective_identity_parity": {
            "task_id": PCPR_082_TASK_ID,
            "kind": "objective_identity_parity",
            "classified_by": EXECUTION_OWNER_REPOSITORY,
        ...
        "live_application": typed_unavailable(
            reason=(
                "Kit binds the Accelerate objective identity parity and does not emit "
                "a live Supervisor.run."
            )
        ),
        ...
        "live_storage": typed_unavailable(
            reason="Kit does not store the hermetic objective identity parity as live durable bytes."
        ),
        "source": {
            "kind": "git",
...
def current_head_static_probes(start: Path | None = None) -> tuple[OutcomeProbe, ...]:
    ...
    probes = [
        _probe(
            ...
            reason=(
                "Committed Kit client binding matches the generator."
                if verified.get("binding_ok") is True
                else "Committed Kit client binding is missing or drifts."
            ),
        ),
        _probe(
            "pyproject_objective_identity_parity_table",
            ...
            reason=(
                "pyproject.toml declares KitObjectiveIdentityParityBinding@1."
                if table.get("interface") == INTERFACE
                else "pyproject.toml does not declare the PCPR-082 binding."
            ),
        ),
        _probe(
            "owner_pack_cid_matches_pin",
        ...
        _probe(
            "current_root_survives_parity_from_disk",
            binding["storage"]["survives_client"] is True
            and binding["storage"]["reconstructed_from"] == "disk"
            and binding["storage"]["reconstructed_from_memory"] is False,
            reason="The current root survives the client and is reconstructed from disk.",
        ),
        _probe(
            "parity_not_stored_as_live_bytes",
...
def qualify_objective_identity_parity(
    ...
    *,
    ...
    objective_cid: str,
    idea_digest_cid: str,
) -> KitObjectiveIdentityParityVerdict:
    if not probes:
        raise KitObjectiveIdentityParityError("at least one probe is required")
    ...
    return KitObjectiveIdentityParityVerdict(
        schema=VERDICT_SCHEMA,
        interface=INTERFACE,
        promotion_status=promotion_status,
        supervisor_disposition="supervisor_non_promoted",
...
def pcpr_082_receipt_promotion(
    verdict: KitObjectiveIdentityParityVerdict,
) -> dict[str, Any]:
...
PINNED_BINDING_CID: Final = (
    "baguqeeraixdsgk2p2kpzf6hvshzgnmzyrf276kyvcvswinh5eucypzp63aiq"
)
CURRENT_HEAD_NON_PROMOTION_VERDICT_CID: Final = (
    "baguqeera36d372bsyzcuu6wunwd52zedsbn6d4fsn56oac44scw2iakorlha"
)
...
__all__ = [
    ...
    "INTERFACE",
    "KitObjectiveIdentityParityError",
    "OBJECTIVE_KIND",
