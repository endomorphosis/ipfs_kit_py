"""Fail-closed PCPR-054 Kit SBOMs and build provenance.

Produce a declared-release-profile SPDX-2.3 SBOM and a declared build
provenance binding for ipfs_kit_py. Direct packages come from the
...
INTERFACE: Final = "KitSbomAndProvenance@1"
SCHEMA: Final = "ipfs_kit_py/assurance/sbom-and-provenance@1"
SBOM_SCHEMA: Final = "ipfs_kit_py/assurance/declared-sbom@1"
PROVENANCE_SCHEMA: Final = "ipfs_kit_py/assurance/build-provenance@1"
VERDICT_SCHEMA: Final = "ipfs_kit_py/assurance/sbom-and-provenance-verdict@1"
PCPR_054_TASK_ID: Final = "PCPR-054"
PCPR_054_GOAL_ID: Final = "PCPR-G600"
PCPR_PROGRAM_ID: Final = "proof-carrying-platform-qualification-and-release-v1"
EVIDENCE_ID: Final = "pcpr/kit-sbom-and-provenance@1"

CANONICAL_PYTHON_REQUIRES: Final = ">=3.12"
RUNTIME_REQUIRES_AUTHORITY: Final = "pyproject.toml:project.dependencies"
SOURCE_REPOSITORY: Final = "endomorphosis/ipfs_kit_py"
SOURCE_ORIGIN_URL: Final = "https://github.com/endomorphosis/ipfs_kit_py"
...
HERMETIC_CANDIDATE_SUITES: Final[tuple[str, ...]] = (
    "tests/test_pcpr_054_sbom_and_provenance.py",
)
...
artifacts, and containers for the Kit *release* profile. They are not
a signed SLSA/in-toto attestation and not a closed PCPR release.
...
- Host libmagic, FUSE, and the IPFS daemon stay typed unavailable
  when absent from the sealed PATH.
...
def parse_pyproject_sbom_table(text: str) -> dict[str, Any]:
    ...
    if isinstance(tool, dict):
        ...
        if isinstance(kit, dict):
            raw = kit.get("sbom-and-provenance")
...
def render_declared_sbom(root: Path | None = None) -> dict[str, Any]:
    ...
    if package_root is None:
        raise SbomProvenanceError("Kit package root was not found")
    lock = render_release_lock(package_root)
    packages = [
        parse_declared_spec(str(spec)) for spec in lock.get("direct_requirements") or []
    ]
...
def render_build_provenance(
    ...
    *,
    ...
    document = {
        ...
        "commands": {
            "materialize": (
                'python -c "from ipfs_kit_py.assurance.'
...
def current_head_static_probes(
    ...
    probes = [
        ...
        _probe(
            ...
            reason=(
                "pyproject.toml declares KitSbomAndProvenance@1."
                if table.get("interface") == INTERFACE
                else "pyproject.toml does not declare KitSbomAndProvenance@1."
            ),
            details={"table": table},
        ),
        _probe(
        ...
        _probe(
            ...
            reason=(
                ...
                "Git pins; they are not this SBOM."
            ),
            details={
                "named_git_extras": list(NAMED_GIT_EXTRAS),
                "named_git_extras_role": NAMED_GIT_EXTRAS_ROLE,
...
CURRENT_HEAD_NON_PROMOTION_VERDICT_CID: Final = (
    "baguqeeraznhqo24rkzrxo5um5yaoomx36gziuam5qbdxa2hl4lzexh3h32cq"
)
