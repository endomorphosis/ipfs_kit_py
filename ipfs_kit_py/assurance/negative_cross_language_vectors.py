"""Fail-closed PCPR-042 Kit binding to negative/cross-language vectors.

Kit owns DurableArtifactReceipt. This module binds the PCPR-042 negative
and cross-language document CID published by the Accelerate-owned encoder
...
is not a freeze, not cross-repository compatibility, not a closed PCPR
release, and it never writes DuckDB or Quack state.
"""
...
INTERFACE: Final = "KitNegativeCrossLanguageVectorBinding@1"
SCHEMA: Final = "ipfs_kit_py/assurance/negative-cross-language-vector-binding@1"
...
class KitNegativeCrossLanguageVectorError(ValueError):
    """Kit attempted to remint a negative/cross-language vector."""


def refuse_negative_remint(cid: str) -> str:
    if cid != PINNED_VECTOR_DOCUMENT_CID:
        raise KitNegativeCrossLanguageVectorError(
            f"negative/cross-language document CID {cid} remints {PINNED_VECTOR_DOCUMENT_CID}"
        )
    return cid


def kit_negative_vector_binding_mapping() -> dict[str, object]:
    return {
        "schema": SCHEMA,
        "interface": INTERFACE,
