            ...
            if hasattr(mcp_server, 'controllers'):
                for name, controller in mcp_server.controllers.items():
                    if hasattr(controller, 'ipfs_model'):
                        try:
                            # Check if this is the WebRTC controller
                            if 'webrtc' in name.lower() or hasattr(controller, 'check_dependencies'):
...
if __name__ == "__main__":
    ...
    if success:
        ...
        print("❌ Failed to apply WebRTC AnyIO fixes")
        sys.exit(1)
