...
# First, let's find the start and end line numbers of the method
...
if start_line is None:
    print("Could not find the ipfs_name_resolve method.")
    sys.exit(1)

# Find the end by looking for the next method or the end of the file
end_line = None
...
# Let's create a new implementation with proper indentation
new_method = """    def ipfs_name_resolve(self, name: str, recursive: bool = True, nocache: bool = False, timeout: int = None) -> Dict[str, Any]:
        """
...
# Now create a new file with the method replaced
with open('ipfs_kit_py/mcp/models/ipfs_model.py.new', 'w') as f:
    # Write the lines before the method
    f.writelines(lines[:start_line])

    # Write the new implementation
    f.write(new_method)

    # Write the lines after the method
    f.writelines(lines[end_line:])

# Now replace the original with the new file
import os
os.rename('ipfs_kit_py/mcp/models/ipfs_model.py.new', 'ipfs_kit_py/mcp/models/ipfs_model.py')

print(f"Replaced ipfs_name_resolve method lines {start_line+1} to {end_line}")
