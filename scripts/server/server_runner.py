#!/usr/bin/env python3
"""
Unified Server Runner for IPFS Kit MCP
...
It provides a comprehensive solution for running the MCP server with all
...
def get_server_class(server_type: str):
    ...
    Args:
        server_type: Type of server ('sync', 'anyio', 'real', etc.)
...
class ServerRunner:
    """Unified runner for MCP server with all available options."""
    ...
    def __init__(
        ...
        Args:
            server_type: Type of server ('sync', 'anyio', 'real', 'storage')
        ...
        if self.server_type == 'storage' and self.storage_config:
        ...
        logger.info(f"Initialized ServerRunner with {server_type} server")
...
def main():
    """Run the unified server with command-line arguments."""
    # Create argument parser
    parser = argparse.ArgumentParser(
        description="Unified Server Runner for IPFS Kit MCP",
    ...
    server_group.add_argument("--server-type", choices=["sync", "anyio", "real", "storage"], default="anyio",
                        help="Server type")
