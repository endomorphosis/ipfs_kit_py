"""
Metrics & Monitoring Extension for MCP Server

This extension integrates the MCP Monitoring System with the MCP server,
providing endpoints for monitoring, metrics collection, and health checks.
"""
...
from fastapi import APIRouter, HTTPException, Query, Path, Response, Depends
...
# Try to import Prometheus client for optional metrics export
try:
...
if MONITORING_AVAILABLE:
    try:
        ...
        config = {
            ...
            "prometheus_enabled": PROMETHEUS_CLIENT_AVAILABLE,
...
def create_metrics_router(api_prefix: str) -> APIRouter:
    ...
    async def metrics_status():
        ...
        return {
            ...
            "prometheus_enabled": PROMETHEUS_CLIENT_AVAILABLE and monitoring_system.config["prometheus_enabled"],
            "prometheus_port": monitoring_system.config["prometheus_port"] if PROMETHEUS_CLIENT_AVAILABLE and monitoring_system.config["prometheus_enabled"] else None,
    ...
    async def metric_history(
        metric_name: str,
        labels: Optional[str] = Query(None, description="Comma-separated list of label=value pairs")
    ):
        """
...
def update_metrics_status(storage_backends: Dict[str, Any]) -> None:
    ...
    storage_backends["monitoring"] = {
        ...
        "features": {
            ...
            "prometheus_export": PROMETHEUS_CLIENT_AVAILABLE
        } if MONITORING_AVAILABLE and monitoring_system is not None else {}
    }
