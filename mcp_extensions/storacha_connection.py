...
class StorachaConnectionManager:
    ...
    def __init__(
        ...
        self.endpoint_health = {endpoint: {"healthy": None, "last_checked": 0, "failures": 0} for endpoint in self.endpoints}
    ...
    def _validate_endpoints(self) -> None:
        ...
        logger.info(f"Endpoint validation complete. {healthy_count}/{len(self.endpoints)} endpoints healthy")
    ...
    def _get_endpoint(self) -> str:
        ...
        if not self.working_endpoint:
            if all(status["failures"] > 0 for status in self.endpoint_health.values()):
                ...
                candidates = [ep for ep, status in self.endpoint_health.items() 
        ...
        **kwargs
        ...
        while retry_count <= self.max_retries:
            try:
                ...
                last_exception = e
                logger.warning(f"Request to {current_endpoint} failed (attempt {retry_count+1}/{self.max_retries+1}): {e}")
