...
from fastapi import APIRouter, HTTPException, Form, Query, File, UploadFile, Body
...
def create_ipfs_router(api_prefix: str) -> APIRouter:
    ...
    async def ipfs_object_patch_rm_link(
        cid: str = Form(...),
        name: str = Form(...)
    ):
        """
    ...
    async def ipfs_dht_findprovs(
        cid: str,
        num_providers: int = Query(20, gt=0, le=100)
    ):
        """
