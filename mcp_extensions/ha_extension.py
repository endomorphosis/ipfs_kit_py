...
def initialize_config():
    ...
    try:
        ...
        this_node["api_url"] = f"{config['api']['protocol']}://{socket.gethostname()}:{config['api']['port']}"
...
def initialize_nodes():
    ...
    try:
        if os.path.exists(NODES_PATH):
            ...
            known_nodes = {
                node_id: node for node_id, node in nodes_data.items()
        ...
        if http_session is None:
            http_session = aiohttp.ClientSession(
                timeout=aiohttp.ClientTimeout(
                    total=config["discovery"]["timeout_seconds"]
                )
            )
...
# Helper functions
def add_event(event_type: EventType, node_id: str = None, region_id: str = None, details: Dict = None):
...
def update_ha_status():
    ...
    # Count active nodes
    active_nodes = sum(1 for node in known_nodes.values() 
                     if node.get("status") == NodeStatus.ACTIVE)
...
def elect_leader():
    ...
    # Get active nodes
    active_nodes = [node for node in known_nodes.values() 
                   if node.get("status") == NodeStatus.ACTIVE]
...
async def check_node_health(node_id: str, node_info: Dict) -> bool:
    ...
    for endpoint in config["health_check"]["endpoints"]:
        ...
        try:
            async with http_session.get(health_url, timeout=config["health_check"]["timeout_seconds"]) as response:
...
async def heartbeat_check():
    ...
    for node_id, node in known_nodes.items():
        ...
        if now - last_seen > config["heartbeat"]["timeout_seconds"]:
            ...
            if node["heartbeat"]["consecutive_failures"] >= config["heartbeat"]["required_successful"]:
        ...
        if node["status"] == NodeStatus.UNAVAILABLE and random.random() < 0.2:  # ~20% chance each round
...
def create_ha_router(api_prefix: str) -> APIRouter:
    ...
    async def list_regions():
        ...
        for region_id, region in regions.items():
            ...
            active_nodes = [node for node in region_nodes if node.get("status") == NodeStatus.ACTIVE]
    ...
    async def create_region(region: RegionConfig):
        ...
        return {
            "success": True,
            "message": f"Region {region.id} created successfully"
        }
    ...
    async def delete_region(region_id: str):
        ...
        add_event(
            EventType.REGION_REMOVED,
            region_id=region_id
        )
        
        return {
            "success": True,
            "message": f"Region {region_id} deleted successfully"
        }
    ...
    async def enable_ha():
        ...
        if config["enabled"]:
            return {
                "success": True,
                "message": "High availability is already enabled"
            }
        ...
        return {
            "success": True,
            "message": "High availability enabled successfully"
        }
    ...
    async def disable_ha():
        ...
        if not config["enabled"]:
            return {
                "success": True,
                "message": "High availability is already disabled"
            }
        ...
        return {
            "success": True,
            "message": "High availability disabled successfully"
        }
    ...
    async def enter_maintenance():
        ...
        if this_node["status"] == NodeStatus.MAINTENANCE:
            return {
                "success": True,
                "message": "Node is already in maintenance mode"
            }
