...
def initialize_content_registry():
    ...
    try:
        if os.path.exists(CONTENT_REGISTRY_PATH):
            ...
            logger.info(f"Loaded {len(content_registry)} content entries from {CONTENT_REGISTRY_PATH}")
        else:
            content_registry = {}
...
def initialize_metadata_registry():
    ...
    try:
        if os.path.exists(METADATA_REGISTRY_PATH):
            ...
            logger.info(f"Loaded {len(metadata_registry)} metadata entries from {METADATA_REGISTRY_PATH}")
        else:
            metadata_registry = {}
...
def get_unified_cid_from_backend(backend, backend_cid):
    ...
    if backend in content_map["backend_to_cid"] and backend_cid in content_map["backend_to_cid"][backend]:
...
def filter_content_by_query(query: ContentQuery):
    ...
    for cid, info in content_registry.items():
        ...
        # Filter by name
        if query.name_contains and (not info.get("name") or query.name_contains.lower() not in info.get("name", "").lower()):
            continue
...
async def store_content(content, request: StoreRequest):
    ...
    try:
        ...
        if not target_backend:
            try:
                from mcp_extensions.routing_extension import make_routing_decision, RoutingRequest, ContentAttributes
        ...
        if target_backend not in storage_backends or not storage_backends[target_backend].get("available", False):
...
async def retrieve_content(cid, preferred_backend=None):
    ...
    try:
        ...
        if (source_backend not in content_info.get("backends", {}) or
            source_backend not in storage_backends or
            not storage_backends[source_backend].get("available", False)):
            
            # If preferred backend not available, try to find another one
            available_backends = [
                b for b in content_info.get("backends", {}).keys()
...
def create_udm_router(api_prefix: str) -> APIRouter:
    ...
    @router.post("/store")
    async def store_content_api(
        request: StoreRequest = Form(...),
    ...
    async def retrieve_content_api(
        cid: str,
        preferred_backend: Optional[str] = None
    ):
