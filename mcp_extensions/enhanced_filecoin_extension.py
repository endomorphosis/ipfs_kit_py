...
from fastapi import APIRouter, HTTPException, Form, Query, File, UploadFile
...
LOTUS_GATEWAY_PATH = os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))), "bin", "lotus")
...
class EnhancedFilecoinGateway:
    ...
    def status(self) -> Dict[str, Any]:
        ...
        if not self.gateway_available:
            ...
            status_info["error"] = f"Gateway script not found or not executable: {self.gateway_path}"
            return status_info
        
        # Test connection by getting chain head
        try:
            result = subprocess.run(
                [self.gateway_path, "chain", "head"],
                capture_output=True,
                text=True
            )
            ...
            try:
                ...
                if height_result.returncode == 0:
                    try:
                        ...
                        deal_info["start_epoch"] = chain_data.get("Height") + 60  # Start 60 blocks in the future
