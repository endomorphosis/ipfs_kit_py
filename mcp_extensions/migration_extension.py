...
def get_backend_module(backend_name: str):
    ...
    try:
        if backend_name == "huggingface":
            from mcp_extensions.huggingface_extension import huggingface_operations
            return huggingface_operations
        elif backend_name == "s3":
            from mcp_extensions.s3_extension import s3_operations
            return s3_operations
        elif backend_name == "filecoin":
            from mcp_extensions.filecoin_extension import filecoin_operations
            return filecoin_operations
        elif backend_name == "storacha":
            from mcp_extensions.storacha_extension import storacha_operations
            return storacha_operations
        elif backend_name == "lassie":
            from mcp_extensions.lassie_extension import lassie_operations
...
async def estimate_migration_cost(source_backend: str, target_backend: str, cid: str) -> Dict[str, Any]:
...
async def perform_migration(migration_id: str, source_backend: str, target_backend: str, 
                            cid: str, metadata_sync: bool, remove_source: bool):
    """
    ...
    try:
        ...
        if hasattr(source_module, "get_content"):
            content = await source_module.get_content(cid)
        ...
        if hasattr(target_module, "store_content"):
            target_result = await target_module.store_content(content, cid=cid)
        elif target_backend == "ipfs":
        ...
        if metadata_sync:
            ...
            if hasattr(source_module, "get_metadata"):
                metadata = await source_module.get_metadata(cid)
            
            # Store metadata in target if available
            if metadata and hasattr(target_module, "set_metadata"):
                await target_module.set_metadata(target_result["cid"], metadata)
        ...
        if remove_source:
            if hasattr(source_module, "remove_content"):
                await source_module.remove_content(cid)
            elif source_backend == "ipfs":
...
def create_migration_router(api_prefix: str) -> APIRouter:
    ...
    async def verify_backends(source_backend: str, target_backend: str):
        ...
        if source_backend not in storage_backends:
            raise HTTPException(status_code=400, detail=f"Source backend '{source_backend}' not recognized")
        if target_backend not in storage_backends:
            raise HTTPException(status_code=400, detail=f"Target backend '{target_backend}' not recognized")
        
        if not storage_backends.get(source_backend, {}).get("available", False):
            raise HTTPException(status_code=400, detail=f"Source backend '{source_backend}' is not available")
        if not storage_backends.get(target_backend, {}).get("available", False):
            raise HTTPException(status_code=400, detail=f"Target backend '{target_backend}' is not available")
    ...
    async def get_migration_status(migration_id: str):
        ...
        if migration_id not in migrations:
            raise HTTPException(status_code=404, detail=f"Migration with ID {migration_id} not found")
    ...
    async def batch_migration(
        ...
        # Verify backends
        if source_backend not in storage_backends or not storage_backends.get(source_backend, {}).get("available", False):
            raise HTTPException(status_code=400, detail=f"Source backend '{source_backend}' not available")
        if target_backend not in storage_backends or not storage_backends.get(target_backend, {}).get("available", False):
            raise HTTPException(status_code=400, detail=f"Target backend '{target_backend}' not available")
