...
from filecoin_storage import FilecoinStorage, LOTUS_AVAILABLE, FILECOIN_LIBRARIES_AVAILABLE
...
def update_filecoin_status(storage_backends: Dict[str, Any]) -> None:
    ...
    storage_backends["filecoin"] = {
        ...
        "message": status.get("message", "Connected to Filecoin network via gateway") if status.get("gateway", False) else status.get("message", ""),
