...
DEFAULT_CONFIG = {
    ...
    "connection_management": {
        "enabled": True,
        "max_connections_per_backend": 20
    }
}
...
async def prune_cache() -> int:
    ...
    try:
        ...
        logger.info(f"Pruned {removed_count} cache entries, freed {freed_bytes / (1024*1024):.2f} MB")
...
def update_backend_stats(backend: str, latency_ms: float, success: bool = True):
    ...
    if success:
        ...
        if len(backend_stats[backend]["latency_history"]) > 50:
            backend_stats[backend]["latency_history"] = backend_stats[backend]["latency_history"][-50:]
    else:
        # Update error count
...
def select_backend(available_backends: List[str] = None) -> str:
    ...
    if available_backends is None:
        available_backends = [b for b in storage_backends 
                             if storage_backends[b]["available"]]
    ...
    if strategy == "round_robin":
        # Simple round-robin
        counts = stats["load_balancing"]["requests_per_backend"]
...
def create_performance_router(api_prefix: str) -> APIRouter:
    ...
    async def get_performance_status():
        ...
        return {
            ...
            "load_balancing": {
                ...
                "strategy": config["load_balancing"]["strategy"],
                "active_backends": len([b for b in storage_backends if storage_backends[b]["available"]])
            },
            "connection_management": {
    ...
    async def update_connection_config(conn_config: ConnectionConfig):
        ...
        config["connection_management"]["max_connections_per_backend"] = conn_config.max_connections_per_backend
    ...
    async def reset_backend_stats():
        ...
        return {
            "success": True,
            "message": "Backend statistics reset successfully"
        }
...
async def cache_middleware(request: Request, call_next):
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            cacheable_types = ["text/", "application/json", "image/", "application/octet-stream"]
