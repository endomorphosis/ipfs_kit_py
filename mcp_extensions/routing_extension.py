...
DEFAULT_PERFORMANCE_DATA = {
    ...
    "regional_adjustments": {
        "us-east": {
            "ipfs": {"latency_factor": 0.9, "availability_factor": 1.0, "cost_factor": 1.0},
            "s3": {"latency_factor": 0.8, "availability_factor": 1.0, "cost_factor": 1.0},
            "filecoin": {"latency_factor": 0.9, "availability_factor": 1.0, "cost_factor": 1.0}
        },
        "eu-west": {
            "ipfs": {"latency_factor": 0.95, "availability_factor": 1.0, "cost_factor": 1.1},
            "s3": {"latency_factor": 0.85, "availability_factor": 1.0, "cost_factor": 1.2},
            "filecoin": {"latency_factor": 1.0, "availability_factor": 0.99, "cost_factor": 1.1}
        },
        "ap-east": {
            "ipfs": {"latency_factor": 1.1, "availability_factor": 0.98, "cost_factor": 1.2},
            "s3": {"latency_factor": 1.0, "availability_factor": 0.99, "cost_factor": 1.3},
            "filecoin": {"latency_factor": 1.2, "availability_factor": 0.97, "cost_factor": 1.1}
        }
    },
...
def detect_content_type_category(content_type: str) -> str:
    ...
    if main_type in ["image"]:
        ...
        if any(subtype in content_type.lower() for subtype in ["pdf", "document", "word", "excel", "powerpoint"]):
            return "document"
        elif any(subtype in content_type.lower() for subtype in ["json", "xml", "yaml"]):
            return "text"
        elif any(subtype in content_type.lower() for subtype in ["model", "tensorflow", "pytorch", "onnx"]):
            return "model"
...
def get_region_latency(source_region: str, target_region: str) -> int:
    ...
    if source_region in geo_data["regions"] and target_region in geo_data["regions"].get(source_region, {}).get("latency_ms", {}):
...
def evaluate_policy_match(policy: Dict[str, Any], content_attributes: Optional[ContentAttributes]) -> bool:
...
def calculate_backend_scores(
    ...
    for backend in available_backends:
        ...
        if client_region and backend_region:
            ...
            elif (client_region in geo_data["regions"] and 
                  backend_region in geo_data["regions"] and
                  geo_data["regions"][client_region].get("continent") == 
        ...
        ) * content_type_factor * backend_factor * geo_factor
        ...
        result = BackendScore(
            ...
            metrics={
                ...
                "latency_ms": metrics.get("avg_read_latency_ms" if operation == "read" else "avg_write_latency_ms"),
                "throughput_mbps": metrics.get("read_throughput_mbps" if operation == "read" else "write_throughput_mbps"),
...
def make_routing_decision(request: RoutingRequest) -> RoutingDecision:
    ...
    try:
        ...
        if request.content_attributes and request.content_attributes.content_type:
            content_type_category = detect_content_type_category(request.content_attributes.content_type)
        ...
        available_backends = [name for name, info in storage_backends.items() 
                             if info.get("available", False)]
        
        # Add required backends if they're not already in the list
        for backend in request.required_backends or []:
            if backend not in available_backends and storage_backends.get(backend, {}).get("available", False):
        ...
        qualified_backends = [b for b in backend_scores 
                             if b.score >= routing_config["minimum_backend_score"]]
        ...
        if not primary_backend:
            primary_backend = "ipfs" if "ipfs" in storage_backends and storage_backends["ipfs"].get("available", False) else "local"
        ...
        routing_statistics["avg_decision_time_ms"] = (
            (routing_statistics["avg_decision_time_ms"] * (routing_statistics["total_decisions"] - 1) + decision_time) / 
        ...
        fallback_backend = "ipfs" if "ipfs" in storage_backends and storage_backends["ipfs"].get("available", False) else "local"
        ...
        return RoutingDecision(
            ...
            decision_time_ms=decision_time,
            client_region=client_region,
            content_type=request.content_attributes.content_type if request.content_attributes else None,
            operation=request.operation,
            decision_id=decision_id,
            timestamp=time.time()
        )
...
def create_routing_router(api_prefix: str) -> APIRouter:
    ...
    async def get_routing_status():
        ...
        return {
            ...
            "available_backends": [
                name for name, info in storage_backends.items() 
                if info.get("available", False)
            ],
            ...
            "stats": {
                ...
                "top_backends": sorted(
                    ...
                    key=lambda x: x[1],
                    reverse=True
                )[:3] if routing_statistics["backend_selections"] else [],
                "top_policies": sorted(
                    routing_statistics["policy_usage"].items(),
                    key=lambda x: x[1],
                    reverse=True
                )[:3] if routing_statistics["policy_usage"] else []
            }
        }
    ...
    async def get_routing_decision(request: RoutingRequest, client_request: Request = None):
        ...
        if not routing_config["enabled"]:
            ...
            return {
                "success": True,
                "primary_backend": "ipfs" if storage_backends.get("ipfs", {}).get("available", True) else "local",
                "replicas": [],
    ...
    async def delete_routing_policy(name: str):
        ...
        if name == routing_config["default_policy"]:
            raise HTTPException(
                status_code=400, 
                detail=f"Cannot delete the default policy. Change default_policy first."
            )
        ...
        return {
            "success": True,
            "message": f"Policy {name} deleted successfully"
        }
    ...
    async def get_performance_metrics():
        ...
        return {
            "success": True,
            "metrics": performance_data["backend_metrics"]
        }
    
    @router.get("/metrics/{backend}")
    async def get_backend_metrics(backend: str):
    ...
    async def analyze_content_type(
        ...
        if not routing_config["content_type_analysis"]:
            return {
                "success": False,
                "message": "Content type analysis is disabled"
            }
    ...
    async def simulate_routing(
        ...
        content_types: List[str] = Query(["text/plain", "image/jpeg", "video/mp4", "application/pdf"]),
