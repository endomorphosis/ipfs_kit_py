...
class HuggingFaceBackend(BackendStorage):
    ...
    def store(
        ...
        if not path:
            ...
            content_type = options.get("content_type") or options.get("metadata", {}).get(
                "content_type"
            )
            if content_type:
        ...
        try:
            ...
            if self.space_mode:
                ...
                result = self.huggingface.download_from_space(
                    space_name=repo_name, path_in_space=path, output_path=temp_file
                )
            else:
                # For regular repos
    ...
    def list(
        ...
        try:
            ...
            if self.space_mode:
                # For Spaces mode
                result = self.huggingface.list_space_files(space_name=repo_name, path=prefix or "")
            else:
                # For regular repos
                result = self.huggingface.list_repo_files(
                    repo_name=repo_name, path=prefix or "", branch=branch
                )
            ...
            if result.get("success", False):
                ...
                for file_info in result.get("files", []):
                    ...
                    if (
                        options.get("skip_directories", False)
                        and file_info.get("type") == "directory"
                    ):
                        continue
        ...
        try:
            ...
            if self.space_mode:
                ...
                result = self.huggingface.check_file_in_space(
                    space_name=repo_name, path_in_space=path
                )
            else:
                # For regular repos
                result = self.huggingface.check_file_exists(
                    repo_name=repo_name, path_in_repo=path, branch=branch
                )
