...
class LassieBackend(BackendStorage):
    ...
    def __init__(self, resources: Dict[str, Any], metadata: Dict[str, Any]):
        ...
        self.download_dir = resources.get("download_dir") or os.path.join(
            self.temp_dir, "lassie_downloads"
        )
    ...
    def _verify_lassie(self):
        ...
        try:
            ...
            result = subprocess.run([self.lassie_path, "--version"], capture_output=True, text=True)
    ...
    def _fetch_from_gateway(self, cid: str) -> Dict[str, Any]:
        ...
        if not self.use_ipfs_gateways:
            return {"success": False, "error": "IPFS gateway fallback disabled"}
            ...
            try:
                ...
                if result.get("success", False):
                    ...
                    return {
                        ...
                        "details": {"stored_with": "ipfs", "ipfs_result": result},
                    }
            except ImportError:
    ...
    def retrieve(
        ...
        try:
            ...
            retrieval_dir = os.path.join(
                self.download_dir, f"lassie_{int(time.time())}_{uuid.uuid4().hex[:8]}"
            )
                ...
                if result.get("success", False):
                    ...
                    if os.path.isfile(content_path):
                        ...
                        return {
                            ...
                            "backend": self.get_name(),
                            "identifier": cid
                            "details": {"retrieved_with": "lassie"},
                        }
                    else:
                        logger.error(
                            f"Lassie reported success but no content found at {content_path}"
                        )
        ...
        try:
            ...
            if cid in self._metadata_cache:
                ...
                return {
                    ...
                    "backend": self.get_name(),
                    "identifier": cid
                    "details": {"deleted_path": local_path},
                }
            else:
                ...
                if deleted_paths:
                    return {
                        ...
                        "backend": self.get_name(),
                        "identifier": cid
                        "details": {"deleted_paths": deleted_paths},
                    }
                else:
                    return {
                        "success": False
        ...
        try:
            ...
            if not os.path.exists(self.download_dir):
                return {
                    ...
                    "items": [],
                    "backend": self.get_name(),
                    "details": {"download_dir": self.download_dir},
                }
            ...
            return {
                ...
                "items": items
                "backend": self.get_name(),
                "details": {"download_dir": self.download_dir, "count": len(items)},
            }
                    ...
                    try:
                        ...
                        if response.status_code == 200:
                            ...
                            return {
                                ...
                                "backend": self.get_name(),
                                "identifier": cid
                                "details": {"source": "network", "gateway": gateway},
                            }
                    except Exception:
                        continue
        ...
        try:
            ...
            if result.returncode == 0:
                ...
                return {
                    ...
                    "error": result.stderr,
                    "backend": self.get_name(),
                    "details": {"code": result.returncode},
                }
    ...
    def clear_cache(
        self, identifier: Optional[str] = None, options: Optional[Dict[str, Any]] = None
    ) -> Dict[str, Any]:
        """
