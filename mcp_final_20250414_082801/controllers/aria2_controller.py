...
class Aria2Controller:
    ...
    async def add_torrent(self, torrent_file: UploadFile = File(...), options: str = Form(None)):
        ...
        try:
            ...
            raise HTTPException(
                status_code=400, detail="Invalid options format. Must be valid JSON."
            )
        except Exception as e:
            logger.error(f"Error in add_torrent: {e}")
            raise HTTPException(status_code=500, detail=f"Error processing torrent: {str(e)}")
    ...
    async def add_metalink(self, metalink_file: UploadFile = File(...), options: str = Form(None)):
        ...
        try:
            ...
            raise HTTPException(status_code=500, detail=f"Error processing metalink: {str(e)}")
    ...
    async def create_metalink(self, files_data: MetalinkFileModel):
        ...
        try:
            ...
            raise HTTPException(status_code=500, detail=f"Error creating metalink: {str(e)}")
