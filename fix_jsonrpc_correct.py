#!/usr/bin/env python3
"""
Corrected fix for the JSON-RPC functionality in the MCP server.
This script directly modifies the handler function without relying on external libraries
and uses the correct server variable name.
"""
...
def backup_file(file_path):
    ...
    backup_path = f"{file_path}.bak.corrected"
    ...
    if 'async def handle_jsonrpc(' in content:
        ...
        if start_idx is not None:
            ...
            new_function = [
                ...
                "            tools = []",
                "            for tool_name, tool in server.tool_manager.tools.items():",
                "                tools.append({",
                "                    'name': tool_name,",
                ...
                "",
                "            # Find the tool",
                "            tool = server.tool_manager.get_tool(tool_name)",
                "            if not tool:",
                "                return JSONResponse({",
                "                    'jsonrpc': '2.0',",
