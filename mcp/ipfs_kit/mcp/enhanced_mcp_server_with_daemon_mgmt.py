...
class IPFSKitIntegration:
    ...
    def _ensure_daemon_running(self) -> bool:
        ...
        # First check if daemon is already running and accessible
        if self._test_ipfs_connection():
            logger.info("✓ IPFS daemon already running and accessible")
            return True
        
        # Check if there are existing daemon processes that might be stuck
        existing_pids = self._find_existing_ipfs_processes()
        if existing_pids:
            logger.warning(f"Found existing IPFS daemon processes that are not responding: {existing_pids}")
            logger.info("Attempting to restart IPFS daemon...")
            
            # Kill existing daemons
            if not self._kill_existing_ipfs_daemons():
                logger.error("Failed to clean up existing IPFS processes")
                return False
            
            # Wait for processes to fully stop
            if not self._wait_for_daemon_stop():
                logger.warning("Timeout waiting for existing daemon to stop")
        
        # Try to start daemon using ipfs_kit if available (but skip the private method)
        if self.ipfs_kit:
            try:
                logger.info("Attempting to start daemon via IPFS Kit...")
                # Just test if IPFSKit can help, but don't rely on private methods
                if self._test_ipfs_connection():
                    logger.info("✓ IPFS Kit connection working")
                    return True
            except Exception as e:
                logger.warning(f"IPFS Kit connection test failed: {e}")
        ...
        try:
            ...
            # Start daemon in background with better error handling
            logger.info("Starting IPFS daemon directly...")
            
            # Use a more robust command with error output
            ...
            # Wait with progressive timeout and status checks
            ...
            # If we get here, daemon started but isn't responding
